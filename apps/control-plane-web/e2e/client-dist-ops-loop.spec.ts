...
/**
 * 客户端分发运营闭环 · mock 真浏览器冒烟（通用能力，不绑定单一需求编号）。
...
test.describe('客户端分发运营闭环', () => {
  ...
  test('安全中心：标题/事件行处置/导出/query 预填', async ({ page }) => {
    ...
    // 事件行至少有封禁 IP；有 key/channel 时有改 key 态 / 频道防护
