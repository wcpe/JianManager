 ...
 *
 * 布局为分栏（FR-423）：左右 50:50 —— 本页签两侧都是工作区，不同于本批其他页签的
 * 62:38「列表 + 详情」。原先是 `minmax(13rem,17rem)_1fr`：清单被钉死在 272px 只能排成
 * 一条细长单列（域多时纵向拖很远），而下发面板拿着 1600px 去放几个 8 行高的输入框。
 * 均分后清单按域横向铺开（`sm:grid-cols-2`），返回 JSON 也拿到够用的宽度。
 *
 * 注：spec 该行把「经济表」归左栏、「背包查询」归右栏，但二者是独立卡片类型
 * （`economy` / `inventory`，只在画布/导播台挂载），本页签只有能力清单 + 下发面板，故不涉及。
 */
...
export default function BusinessSegment({ instanceId }: BusinessSegmentProps) {
  ...
  return (
    <div className="flex min-h-0 min-w-0 flex-1 flex-col gap-3 p-4">
      {/* 头部：图标块 + 标题 + 刷新能力清单（flex-none：常驻可点，不随清单滚走） */}
      ...
      {!manifestQuery.isLoading && manifest?.available && domainEntries.length > 0 && (
        <div className="flex min-h-0 flex-1 flex-col gap-2 xl:flex-row">
          {/* 本页签取 50:50（不用本批 62:38 的默认比例）：两侧都是「工作区」而非
              「列表 + 详情」——左边要横向铺域块，右边的返回 JSON 一旦要横向滚就很难读。
              两者都需要宽度，均分比判定谁更重要更诚实。xl 以下退成纵向堆叠，
              权重只在 xl 生效——窄屏若仍按权重分高度，两栏会互挤到该滚时滚不动。 */}
          <div className="flex flex-none flex-col xl:min-h-0 xl:flex-1">
            {/* 内容驱动 + 栏高封顶：域少时贴合内容，多时撑到栏高上限后卡内滚（spec §3.2）。 */}
            <Panel
              className="max-h-full min-h-0 flex-none"
              title={t('business.capabilities')}
              icon={<Boxes className="size-3.5" />}
              bodyClassName="min-h-0 overflow-auto p-2"
            >
              {/* 域块横向铺开：单列细长清单只会把右边空着、把自己拖长。
                  items-start：动作数不等的域并排时，短域不被长域拉平（spec §3.2）。 */}
          ...
          {/* 右栏 50%：下发面板（表单字段 + 返回 JSON，JSON 超宽时 pre 内部横向滚） */}
          <div className="flex flex-none flex-col xl:min-h-0 xl:flex-1">
            <Panel
              className="max-h-full min-h-0 flex-none"
