    ...
    update.mutate(
      changed.map((it) => {
        ...
        return d.source === 'inline'
          ? { itemKey: it.itemKey, source: 'inline' as const, inlineValue: d.inlineValue }
          : { itemKey: it.itemKey, source: 'file' as const, filePath: d.filePath, fileKey: it.fileKey }
      }),
    )
  }
    ...
    <div className="flex flex-col gap-2 px-3 py-2.5 sm:flex-row sm:items-center" data-testid={`config-surface-row-${item.itemKey}`}>
      ...
      <div className="inline-flex shrink-0 rounded-full bg-muted p-0.5" role="group" aria-label={t('configSurface.sourceLabel')}>
        <button
          ...
          onClick={() => onChange({ source: 'inline' })}
          className={cn(
        ...
        {isInline ? (
          choices.length > 0 ? (
            <select
              ...
              onChange={(e) => onChange({ inlineValue: e.target.value })}
            >
              <option value="">—</option>
              {choices.map((c) => (
