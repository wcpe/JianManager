...
import { useTasks, ACTIVE_TASKS_REFETCH_MS, type Task } from './tasks'
...
describe('useTasks 自动轮询（FR-329）', () => {
  ...
  it('活跃任务每 2s 重取，任务终态后停止轮询', async () => {
    vi.useFakeTimers()
    mockedGet.mockResolvedValue({ data: [task('running')] })
    ...
    mockedGet.mockResolvedValue({ data: [task('succeeded')] })
    await act(async () => {
      await vi.advanceTimersByTimeAsync(ACTIVE_TASKS_REFETCH_MS)
    })
