...
- **滚动预发布**：push 到 `master` 触发，发布/**覆盖**固定 tag `nightly` 的预发布（`prerelease=true`），发布说明取 `CHANGELOG.md` 的 `[Unreleased]` 段。`nightly` **只保留本次构建产物**（同 tag 重发时替换既有同名资产，旧资产不残留）。**【superseded-by ADR-042】固定 tag 由 `nightly` 改名为 `latest`（语义不变，见 ADR-042 §6）；下文 §7 prerelease 渠道端点同步改为 `/releases/tags/latest`。**
...
- **渠道 ↔ GitHub 端点映射**（落地 §3 渠道契约的消费侧）：
  ...
  - `prerelease` → `GET https://api.github.com/repos/{repo}/releases/tags/nightly`（取 §3 的滚动 `nightly` 预发布，tag 名与 FR-173 workflow 强耦合，同属本 ADR 契约）。**【superseded-by ADR-042】tag 名 `nightly` 改为 `latest`，端点改 `/releases/tags/latest`。**
