...
5. **归属为独立仓 + JM 子模块**（Java / Gradle 工程），同构 ServerProbe / Beacon agent 这类独立交付物；服务端分发能力进 JM 主仓。楔子↔core 的 agentArgs 协议（gameDir / channel / 可选 endpoint）为两者接口契约，需定稿文档化。
...
- 客户端组件为 **Java / Gradle 独立仓 + JM 子模块**；客户端线全 JVM，无 Go 工具链引入。
- updater-core 仅用 JDK 自带能力（`java.net.http` / `MessageDigest`）+ 轻量 JSON，保持 jar 精简、零或极少三方依赖（与探针自写最小客户端的克制一致）。
