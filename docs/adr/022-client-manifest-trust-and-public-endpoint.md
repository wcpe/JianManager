...
> **后续修订（见 [ADR-053](053-trust-key-config-supply.md)，2026-07-01）**：决策 2/8 的**信任根公钥供给方式**由本 ADR 的「公钥编入 updater-core 编译期内置」补充为「编译期内置 **或** 随整合包 pin 的配置公钥」——updater-core 运行期从 `jm-updater.json` 的 `signPublicKey` 读信任公钥（缺省回退内置 dev 公钥），CP 一键生成带本机公钥的 `jm-updater.json`。整合包即信任分发载体，配置公钥与内置公钥信任等价；私钥仍服务端持有、投毒模型不变。本 ADR 的 8 条核心决策与 `accepted` 状态不受影响。
