...
- **日期**: 2026-06-23
- **状态**: accepted（§1「name 命中即重注册、不强制 token」立场 `superseded-by ADR-039`；§2「平台分发单脚本写配置 + 拉起注册」的安装编排立场 `superseded-by ADR-051`（改为「下载二进制 + Worker 免配置自启 setup」，脚本退化为取二进制 + 调 setup）；§4「分发来源」立场 `superseded-by ADR-036`，见各节小字与 ADR-036 §7、ADR-039、ADR-051；enrollment token 准入 / 凭据持久化 / 校验 / 自更新编排其余立场仍有效）
