...
| CP | `service.SelfUpdateService`（`resolveRelease`：GitHub Releases API / feed 回退 + CP 自升下载 + 升级前备份/回滚，FR-175/FR-182/ADR-036 §7/ADR-039） | 更新源解析（默认 GitHub Releases，feed 回退）+ CP 自身升级/回滚 |
