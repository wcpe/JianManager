...
**客户端信任根来源：内置 + 配置（FR-253，见 ADR-053）**：updater-core 验签 manifest 的信任根公钥来源双轨——①**编译期内置**（`Signatures.production()` 源码常量 `KEY_K1`，dev/兼容零配置回退）；②**运行期配置**（`jm-updater.json` 的 `signPublicKey`[X.509 SPKI DER base64] + `signKeyId`[默认 k1]）。楔子 `WedgeConfig` 解析上述字段、`Wedge` ctx 透传给 `Core.run`；`Core.run` 裁决：ctx 有 `signPublicKey` → `Signatures.fromConfig` 构造单公钥信任根，无/无效 → 回退内置并记日志（保守：坏配置退回内置，验签自然失败走 fail-static 放行本地版，不挡启动）。**信任等价**：整合包（含 updater-core.jar）即信任分发载体，公钥随包 pin 与编译期内置信任等价——能篡改 json 者亦能换 jar；签名保护后续 OTA 更新通道（服务端泄露/MITM），非初始整合包。私钥仍服务端持有、投毒模型不变。CP 一键生成带本机公钥的 `jm-updater.json`：`GET /client-channels/:id/updater-config`（JWT 平台管理员，复用 FR-248 签名器公钥），前端接入指引加「下载 jm-updater.json」按钮。**内嵌 updater-core.jar 需重编**（`make embed-client-updater`）使含「读配置公钥」能力——重编留整合阶段统一做。

**updater-core 默认随 CP 内嵌（服务端，FR-193 / ADR-045 改写）**：manifest 的 `agent.core` **不由运营管理**，由 **CP 内嵌的默认 updater-core（FR-107）自动产出**——装配处（`main.go`）从 `embed.UpdaterCoreJar()` 算 sha256/size + 单调整数版本（`embed.ClientUpdaterEmbeddedCoreVersion`，ldflags 可覆盖）构造 `embed.EmbeddedCore` 注入；`ClientVersionService.BuildManifest` 用它产出 `agent.core`（version + fan-out 填 `windows`/`macos`/`linux` 三键同制品），无内嵌 jar 时省略 `agent.core`（优雅降级、不破 FR-087/088）。随 CP 自更新（FR-081）跟进、运营不操作；`agent.core.version` 对客户端单调只升（FR-091）。楔子（wedge）同冻结。**已删**运营侧 core 管理（原 FR-193 初版的 `client_core_versions` 表 + pin/更新/回退端点 + 「更新器版本」Tab，见 ADR-045 改写）。接入指引（FR-107）只读展示内嵌更新器版本。
...
| client_channels (FR-086/193) | channel_id(slug, UNIQUE, 对外标识/URL 段), name, description, current_version(latest 内容版本指针，0=未发布，FR-088 编排), pinned_core_version(已废弃：FR-193 反转后改 CP 内嵌默认 updater-core 驱动 agent.core，本列不再使用、json 不暴露，见 ADR-045 改写), created_at, updated_at, deleted_at（客户端分发频道，每服一个，ADR-022） |
...
| client_versions (FR-087/088) | channel_id(所属频道 slug), version(单调递增, UNIQUE(channel_id,version), 防降级基准), files_json(文件清单快照), managed_dirs_json(托管目录), clean_exclude_json(运营自定义排除, FR-255), agent_json(自更新段, 可空；其 agent.core 自 FR-193 起由 CP 内嵌默认 updater-core 自动产出（ADR-045 改写），无内嵌 jar 时回退此手填透传/省略), note, created_by, created_at（版本快照，全保留供运营回滚/diff；manifest 即时组装+签名，回滚=以更高 version 重发旧内容，ADR-022） |
