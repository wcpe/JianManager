...
启动器经 `-javaagent:wedge.jar=<gameDir>` 注入。**楔子（wedge，Java 8，稳定件随基础包分发）** premain 自定位、读同目录 `jm-updater.json`、以独立 `URLClassLoader` **内存加载** updater-core（不锁原 jar）、反射 `Core.run(ctx)`、同步等待 + 超时，全程 **fail-open**（任何异常都放行游戏）。**updater-core（Java 17，fat jar 自含 zstd-jni）** 拉签名 manifest → Ed25519 验签 + 防降级 → 文件级 reconcile（增量/减量、托管区/玩家区隔离、CAS）→ 端点不可达 **fail-static** 带本地版本放行。
