...
- **系统更新 `/system-update`** (V2，FR-081/ADR-020): 侧栏「设置」组，仅平台管理员可见。检查更新（CP 自身 + 各节点版本对比）、CP 自更新、单节点升级、全网逐节点编排（rollout 运行中短轮询进度）；升级为危险操作走统一 `DangerConfirm`（scope=platform）二次确认
