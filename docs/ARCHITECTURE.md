...
`Bot.InstanceID` 始终表示被测目标实例，也是权限、指标与可选 ServerProbe 观测的归属；`Bot.ExecutorNodeID` 表示实际运行 Mineflayer 的 Worker。所有 Bot 创建、删除、停止、行为、命令、状态刷新与事件订阅统一经 `BotExecutorResolver` 解析：`ExecutorNodeID` 非空时优先，否则回退 `Instance.NodeID`，既有普通 Bot 与 V1 压测会话保持兼容；`WorkerID` 仅用于兼容展示，不作路由真源。
...
Control Plane 启动时会为当前已连接 Worker 恢复持久化活动批次所需的 Fleet 订阅，Worker 重连/重注册回调也只恢复该节点的活动订阅。恢复动作先重建 `GetBotFleetSnapshot` → `StreamBotFleetEvents` 归真链路；FR-365 起完整 snapshot reconcile 会按 CP `desired_state` 创建缺失 running、停止 session 内 orphan，并以 10s/90s 新鲜度巡检收敛幽灵 `connected`（`STATUS_STALE`/`RUNTIME_MISSING`）。
...
- 实例操作：CreateInstance, StartInstance, StopInstance, RestartInstance, KillInstance, SendCommand, GetInstanceStatus, ListInstances, DisposeOrphanRuntime, InspectManagedProcess, TerminateManagedProcess
  ...
  - `DisposeOrphanRuntime`（FR-326，Worker 侧实现）：CP 反向对账确认后下发；Worker Kill 进程树（若在跑）+ 移除注册 + `ReapDaemonForDelete` 清 PID/sock，**不删工作目录**。老 Worker `Unimplemented`，CP 记失败不崩
  - `InspectManagedProcess` / `TerminateManagedProcess`（FR-407/408）：仅面向受管实例当前根 PID 与子进程树。Worker 每次 RPC 都从 Manager 读取实例状态与根 PID、实时遍历树并校验目标 PID membership；PID 不在树内、实例未运行或目标是根进程（PID 级处置）时拒绝。根进程停机继续走实例级 stop/restart/kill 状态机；非根子进程可 `terminate`（温和终止，不支持的平台返回 UNSUPPORTED）或 `kill_tree`（强制终止目标子树）。CP 只通过实例 ID 鉴权后委托 Worker，不提供任意 OS 进程枚举或控制。
...
- 崩溃快照上报：`ReportCrashSnapshot`（FR-313；CP 侧实现，Worker 调用，与注册/心跳同信道）——进程非正常退出（退出码≠0 或 RUNNING/STARTING 态意外退出）时 Worker 组装崩溃现场（退出码/信号/时长 + 终端环形缓冲尾部 200 行/64KB）异步上报，凭 `node_uuid+node_secret` 鉴权且实例须属于该节点；CP 落 `instance_crash_snapshots` 并同事务按实例滚动只留最近 5 条。上报失败（网络/老 CP `Unimplemented`）Worker 记日志丢弃不阻塞状态机；daemon 模式退出码经 wrapper 控制通道事件帧（`TypeEvent`+`java_exit` JSON）上抛，老 wrapper 不发帧、老 Worker 忽略未知帧，新旧互不炸。**FR-470 起**同一事务内再同步执行根因归类（`crashdiag` 纯函数规则引擎，产出根因/指纹/证据/置信度并回填分类列）并 upsert 一条日粒度统计 `instance_crash_stats`——统计独立于 K=5 裁剪，趋势因此完整；docker 容器被 cgroup OOM killer 杀掉时（`State.OOMKilled`，退出码恒 137 且宿主侧无信号）Worker 补 `killed` 信号，使归类能判 `oom`。**统计 upsert 失败不回滚现场**：崩溃现场不可重放，故只降级该条的趋势计入（可由 `reclassify` 幂等补跑），现场与分类结果照常落库
- 实例工作目录占用：`GetInstanceResourceSnapshot`（FR-399；CP 侧调用，Worker 实现）响应 FR-467 起新增 `work_dir_bytes` / `work_dir_available`——运行期配额强制的磁盘维度与进程指标**不同源**（看目录树大小而非 RSS），故独立采集（独立 10s 超时预算，不拖垮 3s 的进程快照）且有独立可用标记：统计失败时 CP 需明确「磁盘维度不可用」，不能把 0 当成「没占空间」（0 在配额判定里等于未超限，会静默放过超配额实例）
- 文件操作：ListFiles, ReadFile, HashFile, WriteFile, UploadFile (client stream), DeleteFile, RenameFile（跨目录即移动）, DownloadFile (server stream), DownloadArchive (server stream), SearchFiles
  - `HashFile`（FR-468 §2.5）在 Worker 本地流式读盘算文件的 SHA-256，**只回传摘要**：供版本漂移检测比对「工作目录里实际文件」与绑定摘要，用于发现人工同名覆盖。**不复用 `ReadFile`**——后者带 10MiB 截断，对二进制会得出「前 10MiB 的摘要」这种看似成功实则错误的结论，且为校验把数十 MB 搬回 CP 纯属浪费带宽。`max_bytes>0` 时超限直接回 `too_large`（不计算），读盘错误落在 `error` 字段而非 gRPC error，使 CP 能把「文件不在/没权限」当成漂移信息呈现
...
- Bot Fleet（FR-351/352/399，加性协议，见 [ADR-074](adr/074-bot-distributed-load.md)）：`GetBotCapacity` 返回 ready/legacy/max/active/connecting、`capacity_generation`、worker epoch、bot-worker 版本/feature、**bot-worker RSS**、Go Worker 自身 RSS 与事件循环 p95；`GetInstanceResourceSnapshot` 按受管实例根 PID 聚合完整子进程树的 RSS/CPU/运行时长，缺失时给出可识别 unavailable 原因，不用零值冒充。`ApplyBotBatch` 每批最多 50 个 assignment，携 `batch_id`、`idempotency_key`、期望容量世代并返回逐 Bot accepted/skipped/error 回执；`GetBotFleetSnapshot` 建立完整 runtime 基线；`StreamBotFleetEvents` 持续发送 `runtime_snapshot | action_event`；`SignalBotActions` 每次最多 100 个通用动作信号并逐项回执。`BotAssignment.scenario_json` 承载 CP 已冻结的单 Bot 场景信封 `{seed,botOrdinal,runDeadlineUnixMs,scenario:{key,steps}}`，并与 `cohort_key`、`resume_step_id`、`connect_not_before_unix_ms`、`correlation_seed` 一并下发；`runDeadlineUnixMs` 以 `max(session.startedAt, connectNotBefore)` 为稳定基准，加上所属 cohort 全步骤的 duration/timeout、重试次数、退避与有界重生回跳预算，最后一个 Bot 仍获得完整观察窗；Worker 与 Bot Worker 不解析 YAML、不重新分组
- Bot 通用命令编排（FR-369，见 [ADR-075](adr/075-bot-command-orchestration.md)）：在 `WorkerService` 上加性新增 `ApplyBotCommandSchedules` / `ReleaseBotCommandSchedules` / `CancelBotCommandSchedules`，每批最多 100 个 Bot 项；逐项结果固定 `accepted|rejected|unknown`，最终 occurrence 终态仍只经 `StreamBotFleetEvents.action_event` 回传。Worker 与 Bot Worker 在 IPC 层加性新增 `command-schedule` / `command-schedule-accepted` / `command-schedule-release` / `command-schedule-release-result` / `command-schedule-result` / `command-schedule-cancel` / `command-schedule-cancel-result`；Bot Worker 维护集中 scheduler，每个 `scheduleRunId` 唯一管理 occurrence 计划；`bot.chat` 调用未同步抛错即记 `sent`、同步抛错按 V1 固定重试（≤3 次，250ms/500ms 退避），其余错误不重试；取消幂等，未开始 occurrence 立刻写 `cancelled`，同步 `bot.chat` 当前 occurrence 完成后再停。变量白名单仅 `botName`/`botOrdinal`/`cohortKey`/`runId`/`actionRunId`/`correlationToken`，CP 在 Apply 前冻结 occurrence plan 并按固定算法复算 jitter 偏移，Worker/Bot Worker 不再展开模板。`bot_stress_sessions.command_schedule_snapshot` 与 `bot_load_command_checkpoints` 持久化冻结计划与 occurrence 账本（稳定键 `runUuid+botUuid+stepId+commandId+occurrence`）。
  - 每个 `sessionUUID + executor node` 只有一条活动订阅；首次订阅与每次断流重连都严格先取 snapshot baseline、再开 stream。订阅槽带独立 generation，旧流迟到的 runtime 事件在进入协调器前丢弃，避免覆盖新 baseline；已经接收并进入统一 action 队列的事件跨 generation 排空，不能因切代永久丢失
...
Bot 场景保持 **Control Plane 单点解析**（FR-274/352/404）：新会话的 `scenario` 可提交 Scenario V2 JSON 对象或 YAML/JSON 字符串，两种输入都解析为显式 Go DTO、走同一 validator，并冻结为规范 JSON `ScenarioSnapshot`。MCP `loadtest_run_create.scenario` 将结构化对象原样交给同一创建链路，不另设解析或授权旁路。CP 按 seed 与稳定 Bot ordinal 确定性分配 cohort，把单 Bot `{seed,botOrdinal,scenario:{key,steps}}` 信封写入 Fleet assignment；Worker Node 只透传规范 JSON。旧 `orchestrationYaml` 与无编排 `behavior` 仍保留，CP 尽力转换为单 cohort V2 snapshot；转换失败时继续走原 `orchestrated`/legacy 行为，不覆盖原 YAML、不破坏既有会话。完整 Scenario V2 字段与动作语义见 [FR-352 规格](specs/bot-scenario-engine/spec.md)，跨节点执行边界见 [ADR-074](adr/074-bot-distributed-load.md)。当前代码没有 `commandSchedule` 请求字段或通用命令 scheduler；FR-358 的未来设计只存在于功能规格，落地前不属于当前架构能力。
...
- FR-399 的 `BotLoadMetricSampler` 每 5 秒以 `bot_load_batches` 确定本会话执行节点，写入 executor 与 `targetLegacy.targetResource` 资源样本；容量目录只提供当次 Worker 读取，历史曲线只以样本表为真源。资源未知统一写 JSON `null` 与 `unavailable` 原因。终态报告按阶段跳过前三帧、以第 4 帧为 baseline，分别给出实测峰值和 25% 安全余量建议。目标 SSH 主机为 64 GiB 时，报告要求保留 8 GiB 系统内存；不满足该约束、500 connected Bot、10 个实际执行节点或既有 verdict 时，只能声明最大稳定实测规模。
...
#### 6.4.2 场景执行、屏障与动作信号（FR-352）

- **Bot Worker 执行器**：每个 Bot 至多一个 `ScenarioRunner`，由 FleetController 的单一集中 scheduler 与旧行为共同驱动，不为每 Bot 创建无界 interval。Runner 统一持有当前 step/attempt/deadline/cancel token/锁定实体，切步前 dispose 旧动作并清理 pathfinder goal 与 timer；终态只上报一次 `action-event`。
- **动作能力**：Runner 已实现 `wait_spawn`、`roam_in_area`、`send_command`、`wait_probe_event`、`barrier`、`move_to_and_wait`、`find_entity`、`attack_until`、`wait`、`respawn_and_rejoin`。漫游目标由 seed+botOrdinal+stepId 确定性生成；移动需在半径内稳定后才推进；`attack_until` 支持锁敌、追击、节奏、确定性随机重选、`searchArea` 搜敌，以及死亡后的单次 `respawn()`→新 spawn 恢复。`minClientAttackAttempts` 只记录本地攻击调用活跃度，不替代可信 damage/kill/窗口断言，也不作为服务端伤害证据。真实 Mineflayer/pathfinder/服务端伤害证据仍须真机验证。
- **动作结果口径**：复用 §6.1 已定义的 `ActionResultService` 账本，不另建平行事件流；终态错误码冻结为 `CONNECT_TIMEOUT`、`CONNECT_ENDED`、`PATHFINDER_UNAVAILABLE`、`PATH_NOT_FOUND`、`MOVE_TIMEOUT`、`TARGET_NOT_FOUND`、`ATTACK_ASSERTION_UNMET`、`ATTACK_ACTIVITY_UNMET`、`RESPAWN_LIMIT_EXCEEDED`、`PROBE_EVENT_TIMEOUT`、`BARRIER_TIMEOUT`、`ACTION_CANCELLED`、`ACTION_INTERNAL_ERROR`。
- **屏障协调**：`BarrierCoordinator` 以 `runUuid+stageIndex+cohortKey+barrierKey+round` 为内存作用域，跨进程映射 proto `session_uuid`；数字 runId 仅用于数据库/API 引用。首次进入同时冻结 Bot UUID/current desired generation 与唯一 absolute deadline；后续 Bot 即使因不同到达时刻上报不同本地 deadline，也不能改变该 scope 的截止时间。到达按 Bot+generation 幂等，支持 all/count/percent（百分比向上取整）。正常达阈值时 `releaseAtUnixMs` 始终不晚于冻结 deadline；动态 lead 为 `min(1s, timeoutBudget/4, remaining)`，剩余时间小于冻结 lead 时立即释放，且预调度旧回执不能确认新的立即 releaseAt。
- **信号路由**：`ActionSignalRouter` 只路由仍为 running 且 run/Bot/actionRunId/correlationToken/generation 完整匹配的动作；等待动作与 Bot executor/generation 通过每 500 项一次的批量 JOIN 查询解析，禁止逐 Bot preload。按当前 `ExecutorNodeID`（为空回退目标实例节点）分组后，节点间最多 16 路并发、节点内每 100 项串行分块；一个节点 3 秒超时不会阻止其他节点立即收到，receipt 始终按原输入 index 返回。Worker 的 accepted/skipped/error 逐项保留；节点调用失败、缺失回执和单项错误均标为可重试，重试复用稳定 signalId。
- `barrier-arrived` 使用既有 `BotActionEvent.result_json` 承载 stage/cohort/barrier/round/release/deadline，不修改 proto。达到阈值后发 `barrier-release`（`round+releaseAtUnixMs`）；`release-arrived` 在冻结 deadline 前按动态 lead 预投递、统一以 deadline 释放，Runner 在同毫秒先执行已接收的屏障动作再裁决 timeout。`fail` 同样提前投递内部字符串信号 `barrier-fail`（`round+failAtUnixMs`，不改 proto），Bot Worker 到冻结 deadline 产出 `timed_out/BARRIER_TIMEOUT`；deadline 后慢到达同一已超时 scope 的 Bot 会立即收到同一关联失败信号，不再等待自己的本地 timeout。仅 accepted 或无错误 skipped 才从 pending delivery 移除；重复、迟到、重连和终态后重放均幂等。
- 内置纯数据预设 `tower-defense-core-v1` 生成 20% lobby（wait_spawn→roam observation）与 80% combat（wait_spawn→send_command→room_joined→barrier→game_started→move+area probe→attack observation）Scenario V2。固定随机种子、`RoomKey`、带 `{{roomKey}}` 与 `{{correlationToken}}` 的进房命令、主城/战斗坐标与半径、区域标识、怪物类型和锁敌半径均须由调用方参数提供；构建期校验 RoomKey 后只烘焙 `{{roomKey}}`，运行期关联占位符继续保留。通用 Scenario V2 没有 roomKey 值来源，提交未绑定 `{{roomKey}}` 会在 CP 创建期按叶子 path 拒绝，而不是拖到 Node 执行时报内部错误。

ServerProbe 仍只与目标实例**本机 Worker**通信（`/metrics` 回环抓取 + `/ws/plugin-bridge` 反向连接），不因 Bot 在其他 Worker 执行而跨节点直连；浏览器仍只访问 Control Plane。真实 ServerProbe 场景事件与塔防插件适配不属于取代 FR-353 的 FR-358，未来如需接入必须作为独立可选适配器，不改变 ADR-075 的通用命令成功语义。FR-352 当前只完成 fake/自动化关联验证，不宣称真 MC、真实 pathfinder 或 500 Bot 场景通过。
...
- **发布端点（运营，JWT 平台管理员，与 FR-086 频道管理同组）**：`POST /client-channels/:id/files`（上传制品入 FR-045 制品库 type=client-file）、大文件分块上传 `POST/PUT/DELETE /client-channels/:id/uploads/...`（FR-251，init→chunk→complete→abort，喂同一 CAS）、上传增效 `POST /client-channels/:id/files/precheck` + `POST /client-channels/:id/files/batch`（FR-346，批量 sha256 秒传预查命中免传 + ≤8 MiB 小文件聚合上传，落同一 CAS；上限 500 hash/次、200 文件且总量 ≤32 MiB/批）、`POST /client-channels/:id/versions`（发布版本、单调递增、切 latest 指针）、`POST /client-channels/:id/updater-core/versions`（手动上传 updater-core.jar hotfix，归档为 client-updater-core，可立即选为频道 core）。

**发布编排前端流程（`ClientPublishPage`，FR-191 独立路由页 → FR-250 延迟批量上传 → FR-346 上传增效）**：选文件/拖拽（散文件 + **文件夹** `webkitGetAsEntry` 递归保相对路径 + zip 前端 fflate 解包 + `webkitdirectory` 选择器）后文件以浏览器内 `File` **本地暂存**（草稿仅 File + name/size/相对 path，**无 sha256**）；文件树/路径/sync/platform 编排、删除全在本地零网络；点「发布」才调用 `lib/efficientUpload.ts`：本地 name+size 去重 → 安全上下文（HTTPS/localhost）下用 WebCrypto 串行计算 ≤256 MiB 文件的原始内容 sha256，普通 HTTP 无 WebCrypto 时仅对 ≤8 MiB 聚合小文件使用无依赖 JS SHA-256、较大文件直接分块 → hash 按 ≤500 项分批预查（命中免传；预查失败降级为 miss，不阻断发布）→ miss 中 ≤8 MiB 且有 hash 的文件按 ≤200 个且总量 ≤32 MiB 贪心装箱走聚合端点，较大或未 hash 文件走 FR-251 `uploadFileChunked`（有 hash 时 complete 携 `expectedSha256`）→ 聚合批与分块任务共用并发 4 的任务池，进度单调、不 NaN、支持取消，失败停批并保留草稿可重试 → 得各 sha256/md5/size 组 `ManifestFile[]` → `POST .../versions`。**选文件/拖拽/发布前删除的文件从不上传**；纯编排逻辑位于 `lib/client-publish-wizard.ts`、`lib/clientUploadPlan.ts` 与 `lib/efficientUpload.ts`。上传 codec=none（本期不压缩）。
...
| instances | uuid, node_id(FK), name, type, role(proxy/backend/universal/beacon, V2；后三者非群组服角色，FR-433 支持经 API 变更), process_type, status, start_command, work_dir(系统分配), env_vars(JSON), auto_start, auto_restart, jdk_id(FK, V2), launch_spec(JSON: jvm_args/core_jar/args/omit_nogui, V2), image(docker 模式镜像引用, FR-078), container_id(docker 模式最近容器 ID), cpu_limit/mem_limit_mb/disk_limit_mb(docker 资源限额，0=不限制，磁盘仅展示), throttle_cpu_limit/throttle_mem_limit_mb(FR-467 throttle 档登记的**待收紧限额**，0=无；启动容器时生效限额取 min(配置限额, 待收紧限额)——cgroup 限额只能在创建容器时注入，不落库这个意图就等于限流永不发生), forwarding_secret(V2, Velocity 转发), proxy_online_mode(V2, 代理正版校验), server_port/query_port, probe_port(V2, ServerProbe /metrics 端口, 29940 段), mc_*, tags(JSON), work_dir_in_place(FR-302, 就地导入标记=工作目录在托管区外、删除不删原目录) |
...
| instance_crash_snapshots (FR-313) | instance_id(FK, INDEX), occurred_at, exit_code(无法获知=-1), signal(Unix 信号名；Windows/非信号退出为空), duration_ms, tail_output(TEXT, ≤200 行/64KB Worker 侧截取), root_cause(varchar(32), INDEX；FR-470 根因枚举 oom/port_in_use/class_not_found/jvm_args/permission/segfault/corrupt_data/unknown；旧行为空), signature(varchar(255), INDEX；FR-470 归一化同类指纹), evidence(TEXT；命中证据原文 JSON 数组), confidence(float；归类置信度 0~1)（进程非正常退出现场；Worker 经 ReportCrashSnapshot 上报，写入同事务同步分类 + 按实例滚动只留最近 5 条，删实例级联清） |
| instance_crash_stats (FR-470) | instance_id(FK), bucket_day(char(10)，日粒度 UTC), root_cause(varchar(32)), signature(varchar(255)), count；UNIQUE(instance_id, bucket_day, root_cause, signature)（崩溃趋势汇总：**独立于 K=5 滚动裁剪**，每次快照入库 upsert 一条计数，使「连崩 >5 次照样看得到趋势」；日粒度使行数有界 = 实例数 × 天数 × 根因数，超 `crash.stat_retention_days`(90) 由后台巡检裁剪） |
| instance_binary_bindings (FR-468) | instance_id(FK, UNIQUE), current_asset_id(INDEX；0=无制品库版本，即 url/node_file 来源), previous_asset_id(0=无可回滚), current_filename, current_sha256, current_version, previous_filename, previous_sha256, start_command_at_binding（实例 ↔ 二进制版本的绑定：版本真源仍是制品库 asset（内容寻址），本表只记「该实例当前跑哪个 asset」；**重建读此绑定冻结版本**，不再重解析来源（ADR-090 补记），受控升级/一级回滚在此换绑定） |
| instance_snapshots (FR-466) | uuid(UNIQUE), instance_id(FK, INDEX), name, kind(manual/pre_rollback/scheduled), state(pending/running/completed/failed/rolled_back；仅 completed 与 rolled_back 可作回滚目标), root_backup_id(FK backups, INDEX), binary_name/binary_sha256(快照时刻启动二进制指纹), config_hash/config_summary(启动命令+环境变量摘要), triggered_by, triggered_by_rollback_id(该快照由哪次回滚触发，形成「回滚 → 退回」链), size_mb, failure_reason, note, deleted_at（整机快照 = 时间点语义的整机可回滚点，底层指向一条**全量** Backup 复用既有归档/回放；回滚前强制建 pre_rollback 快照，且 pre_rollback 不受条数裁剪。底链可用性 `rootBackupState` / `notRollableReason` 为**读时派生不落库**——底链会被人工删除或并发清理改变，持久化只会得到陈旧值，而陈旧值正是「列表显示可回滚、点击报找不到」的成因） |
| bot_stress_sessions | uuid, instance_id(FK), name, name_prefix, status(pending/running/stopped/error), bot_count, behavior, config(JSON), orchestration_yaml(TEXT), orchestration_summary(JSON), allocation_plan(TEXT，服务端保存的确定性分片正文；不由客户端回传), scenario_snapshot(TEXT/JSON，FR-352 已冻结的规范 Scenario V2 运行快照), succeeded, failed, last_error, started_at, ended_at |
...
| backups | uuid, instance_id(FK), name, file_path, file_size_mb, type(0/1), mode(0 全量/1 增量, V2), status(0/1/2/3), **origin(空=普通备份 / snapshot=快照底链)**, parent_id(FK self, 备份链, V2), manifest(JSON 文件清单, V2), storage_id(FK, V2), storage_key(远程对象键, V2), checksum/checksum_algo(归档完整性, FR-171)。`origin=snapshot` 的备份被 `backup.retention_days` **豁免**（其生命周期由 `snapshot.retention_days` 独占管理），否则默认 14 天会先于快照的 30 天裁掉底链，使快照全部变成指向软删行的死链 |
| process_metric_snapshots | node_uuid, instance_uuid, pid, name, cpu_percent, rss_bytes, read_bytes_per_sec, write_bytes_per_sec, user, command_summary(截断脱敏), sampled_at（受管实例进程 TOPN 短期快照，按 48h TTL 清理，FR-170；FR-407 诊断仅读最近窗口样本，不新增长期明细表） |
...
```
apps/bot-worker/src/
  ipc/          # stdin/stdout JSON 行协议、FleetController 与集中 scheduler
  bot/          # Mineflayer 连接、重连、生命周期
  behavior/     # 兼容行为引擎：follow, guard, patrol, idle, custom, orchestrated
  scenario/     # ScenarioRunner、动作工厂、取消/重试/信号关联与场景能力适配
  script/       # 脚本执行器 + 进度上报
  debug/        # 交互式调试会话
...
Node 测试入口固定为 `apps/bot-worker/test/run-tests.mjs`：用内置 `node:test.run()` 递归收集 `test/**/*.test.mjs`，`npm test` 先 build 再运行全部测试，避免 Node 版本或 shell glob 差异漏测；FR-352 自动化基线为 69/69。

默认准入容量仍为 **50 bots/worker**；FR-351 通过 Control Plane 将同一目标实例的 Bot 确定性分片到多个 Worker，而不是提高单 Bot Worker 默认容量。bot-worker 的 `worker-ready` 声明 `fleet-v1`、maxBots、worker epoch、worker epoch generation 与 capacityGeneration，heartbeat 上报 active/connecting、RSS 和事件循环 p95；runtime 的 generation/configHash/epoch/eventSeq 由 CP 归真，旧进程或乱序事件不得覆盖新 desired 状态。Bot Worker 内部只为 Fleet envelope 创建的 Bot 标记 `fleetManaged`，拒绝缺 requestId/generation 的 legacy stop 越权；停止先清 desired、取消 pending 重连、释放连接/行为/定时器并发出 stopped runtime，再从内存集合移除。FR-365：kicked/end/error 且 desired running 时按指数退避+抖动自动重连（base=1s、max=60s、jitter=20%，≥10 次降级固定 max），进程级 connecting 信号量默认 `min(10,maxBots/5)`；Worker Manager 内存保留 desired assignment，子进程崩溃后熔断（5 次/5 分钟开路 60s）并重放 `replay:<epoch>:<batch>`。
...
`orchestrated` 保留为 FR-274/V1 兼容组合行为，不直接解析 YAML：它只消费 Control Plane 已规范化的 `behaviorConfig`，按阶段创建并切换既有行为类。FR-352 的新会话走 `scenario` 目录下的 `ScenarioRunner`；旧 `orchestrationYaml`/behavior 可由 CP 转换为兼容 V2 snapshot，无法安全转换时仍回退原 `orchestrated`/legacy 路径，避免两端维护两套 YAML 语义并保持历史兼容。
