...
Bot 场景保持 **Control Plane 单点解析**（FR-274/352）：新会话的 `scenario` 可提交 Scenario V2 JSON 对象或 YAML/JSON 字符串，两种输入都解析为显式 Go DTO、走同一 validator，并冻结为规范 JSON `ScenarioSnapshot`。CP 按 seed 与稳定 Bot ordinal 确定性分配 cohort，把单 Bot `{seed,botOrdinal,scenario:{key,steps}}` 信封写入 Fleet assignment；Worker Node 只透传规范 JSON。旧 `orchestrationYaml` 与无编排 `behavior` 仍保留，CP 尽力转换为单 cohort V2 snapshot；转换失败时继续走原 `orchestrated`/legacy 行为，不覆盖原 YAML、不破坏既有会话。完整 Scenario V2 字段与动作语义见 [FR-352 规格](specs/bot-scenario-engine/spec.md)，跨节点执行边界见 [ADR-074](adr/074-bot-distributed-load.md)。当前代码没有 `commandSchedule` 请求字段或通用命令 scheduler；FR-358 的未来设计只存在于功能规格，落地前不属于当前架构能力。
...
- **动作能力**：Runner 已实现 `wait_spawn`、`roam_in_area`、`send_command`、`wait_probe_event`、`barrier`、`move_to_and_wait`、`find_entity`、`attack_until`、`wait`、`respawn_and_rejoin`。漫游目标由 seed+botOrdinal+stepId 确定性生成；移动需在半径内稳定后才推进；攻击支持锁敌、追击、节奏、重选与可信 damage/kill/窗口断言，截止未满足返回 `ATTACK_ASSERTION_UNMET`。真实 Mineflayer/pathfinder/服务端伤害证据仍须真机验证。
- **动作结果口径**：复用 §6.1 已定义的 `ActionResultService` 账本，不另建平行事件流；终态错误码冻结为 `CONNECT_TIMEOUT`、`CONNECT_ENDED`、`PATHFINDER_UNAVAILABLE`、`PATH_NOT_FOUND`、`MOVE_TIMEOUT`、`TARGET_NOT_FOUND`、`ATTACK_ASSERTION_UNMET`、`PROBE_EVENT_TIMEOUT`、`BARRIER_TIMEOUT`、`ACTION_CANCELLED`、`ACTION_INTERNAL_ERROR`。
