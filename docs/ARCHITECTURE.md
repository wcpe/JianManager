...
- Bot Fleet（FR-351/352，加性协议，见 [ADR-074](adr/074-bot-distributed-load.md)）：`GetBotCapacity` 返回 ready/legacy/max/active/connecting、`capacity_generation`、worker epoch、bot-worker 版本/feature、RSS 与事件循环 p95；`ApplyBotBatch` 每批最多 50 个 assignment，携 `batch_id`、`idempotency_key`、期望容量世代并返回逐 Bot accepted/skipped/error 回执；`GetBotFleetSnapshot` 建立完整 runtime 基线；`StreamBotFleetEvents` 持续发送 `runtime_snapshot | action_event`；`SignalBotActions` 每次最多 100 个通用动作信号并逐项回执。`BotAssignment.scenario_json` 承载 CP 已冻结的单 Bot 场景信封 `{seed,botOrdinal,runDeadlineUnixMs,scenario:{key,steps}}`，并与 `cohort_key`、`resume_step_id`、`connect_not_before_unix_ms`、`correlation_seed` 一并下发；`runDeadlineUnixMs` 以 `max(session.startedAt, connectNotBefore)` 为稳定基准，加上所属 cohort 全步骤的 duration/timeout、重试次数、退避与有界重生回跳预算，最后一个 Bot 仍获得完整观察窗；Worker 与 Bot Worker 不解析 YAML、不重新分组
  ...
  - `BotFleetEvent.action_event` 由 CP `ScenarioActionEventService` 接入，先经 `ActionResultService` 按执行节点、session、Bot 与 desired generation 强校验并写入 `bot_load_action_results`：V2 `correlationToken` 必须非空且为 UUID，非法 start/finish/barrier 在落账与屏障冻结前拒绝；actionRunId 幂等创建 running，首个终态条件更新胜出，重复/迟到事件不改账本；result JSON 最大 16KiB，超限改存带 `truncated/originalBytes/preview` 的可识别元数据。Worker gRPC 层为 action-event 维护当前进程内、非持久化的有界压缩 journal：按 `actionRunId` identity 只保留最新 running/waiting 或首个 terminal，terminal 替代同 identity 的非终态；容量上限 60,000 条，覆盖单节点合法最坏窗口 50 Bot × 100 step × 10 attempt=50,000 identities 并保留 20% 安全余量。新订阅在原子切点先按接收序号 replay、再接 live，1024 条 live backlog 溢出断流后可通过重订阅补发早期 terminal。Worker 进程重启后的恢复不在本 journal 保证内，归 FR-354。`barrier-arrived` 再进入 `BarrierCoordinator`，需要外部完成的动作由 `ActionSignalRouter` 经 `SignalBotActions` 投递。普通挥击事件不形成追加明细；真实 ServerProbe 事件来源与 MSPT p95 接真仍属于 FR-353
...
Bot 场景保持 **Control Plane 单点解析**（FR-274/352）：新会话的 `scenario` 可提交 Scenario V2 JSON 对象或 YAML/JSON 字符串，两种输入都解析为显式 Go DTO、走同一 validator，并冻结为规范 JSON `ScenarioSnapshot`。CP 按 seed 与稳定 Bot ordinal 确定性分配 cohort，把单 Bot `{seed,botOrdinal,scenario:{key,steps}}` 信封写入 Fleet assignment；Worker Node 只透传规范 JSON。旧 `orchestrationYaml` 与无编排 `behavior` 仍保留，CP 尽力转换为单 cohort V2 snapshot；转换失败时继续走原 `orchestrated`/legacy 行为，不覆盖原 YAML、不破坏既有会话。完整 Scenario V2 字段与动作语义见 [FR-352 规格](specs/bot-scenario-engine/spec.md)，跨节点执行边界见 [ADR-074](adr/074-bot-distributed-load.md)。
...
- **屏障协调**：`BarrierCoordinator` 以 `runId+stageIndex+cohortKey+barrierKey+round` 为内存作用域，首次进入同时冻结 Bot UUID/current desired generation 与唯一 absolute deadline；后续 Bot 即使因不同到达时刻上报不同本地 deadline，也不能改变该 scope 的截止时间。到达按 Bot+generation 幂等，支持 all/count/percent（百分比向上取整）。正常达阈值时 `releaseAtUnixMs` 始终不晚于冻结 deadline；动态 lead 为 `min(1s, timeoutBudget/4, remaining)`，剩余时间小于冻结 lead 时立即释放，且预调度旧回执不能确认新的立即 releaseAt。
...
ServerProbe 仍只与目标实例**本机 Worker**通信（`/metrics` 回环抓取 + `/ws/plugin-bridge` 反向连接），不因 Bot 在其他 Worker 执行而跨节点直连；浏览器仍只访问 Control Plane。真实 ServerProbe 场景事件与塔防插件适配由 FR-353 接入同一 `ActionSignalRouter`，FR-352 当前只完成 fake/自动化关联验证，不宣称真 MC、真实 pathfinder 或 500 Bot 场景通过。
