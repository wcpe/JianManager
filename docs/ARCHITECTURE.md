...
**updater-core 集中版本管理（服务端，FR-193 / ADR-045）**：manifest 的 `agent.core` 不再由发布向导手填透传，而由**频道 core pin 驱动**——运营上传 core jar 入制品库（`type=client-core`，一份 jar 三平台通用，FR-045 内容寻址）→ 登记为带版本号的 core 版本（`client_core_versions`，version 全局单调）→ 频道 `pinned_core_version`（0=自动用最新）选定要分发的 core 版本。`ClientVersionService.BuildManifest` 组装 latest 快照后用 pin 选定版本**覆盖** `agent.core`（version + fan-out 填 `windows`/`macos`/`linux` 三键同制品），无任何已登记 core 时回退快照手填透传（兼容）。**楔子（wedge）冻结、单版本、不纳管**（`agent.wedge` 仍来自快照、信息性）。**两条版本轴**：manifest `version`（内容、防降级）与 `agent.core.version`（updater-core 自身、对客户端单调**只升不降**，FR-091 证）正交。运营**「更新」**= pin 到更高 core 版本（客户端下次 promote）；**「回退」坏 core** = 以**更高** `agent.core.version` 重发旧 core 字节为新版并 pin（同 FR-088 内容回滚法，**绝不降版**——否则已升级客户端不降级、回退无效）。管理台频道详情「更新器版本」Tab：core 版本列表 + 当前 pin + 上传 + pin/更新/回退（DangerConfirm）+ 楔子只读冻结提示。客户端不改（楔子冻结成立，靠既有 FR-091 逻辑消费）。
...
| client_channels (FR-086/193) | channel_id(slug, UNIQUE, 对外标识/URL 段), name, description, current_version(latest 内容版本指针，0=未发布，FR-088 编排), pinned_core_version(频道 pin 的 updater-core 版本，0=自动用最新；驱动 manifest agent.core，FR-193/ADR-045), created_at, updated_at, deleted_at（客户端分发频道，每服一个，ADR-022） |
...
| client_versions (FR-087/088) | channel_id(所属频道 slug), version(单调递增, UNIQUE(channel_id,version), 防降级基准), files_json(文件清单快照), managed_dirs_json(托管目录), agent_json(自更新段, 可空；其 agent.core 自 FR-193 起由频道 pin 驱动覆盖，无 core 注册时回退此手填透传), note, created_by, created_at（版本快照，全保留供运营回滚/diff；manifest 即时组装+签名，回滚=以更高 version 重发旧内容，ADR-022） |
| client_core_versions (FR-193) | version(全局单调递增, UNIQUE, = manifest agent.core.version), artifact_sha256(core jar 制品自身 sha256, = 各平台 artifact.sha256, 一份 jar 三平台通用), artifact_size, codec, note, source_version(回退来源版本号, >0=某历史版本的重发), created_by, created_at（updater-core 集中版本注册；与频道无关，频道经 client_channels.pinned_core_version 选定；楔子冻结、不入表；回退=以更高 version 重发旧 core 字节，agent.core.version 对客户端只升不降，ADR-045） |
