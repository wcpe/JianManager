...
| FR-344 | 实例环境变量管理（双区展示 + 可编辑写入 .env）（feat，增强 FR-034 搭建/结构化启动，用户要求「加环境变量 tab、支持自配启动环境变量写入 .env」）：实例详情新增「环境变量」页签——上区可增删改自定义启动环境变量、保存写入工作目录 .env 文件、下次启动注入生效；下区展示运行中 JVM 实际完整环境（Worker 上报启动时所用 env，含继承 PATH/JAVA_HOME，只读，停机提示仅运行时可读）；env 编辑 API + .env 文件契约 + 启动管线注入 + 前端页签（需 spec）→ `docs/specs/instance-env-vars/spec.md` | P2 | 📋 计划 |
