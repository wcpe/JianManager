...
| FR-351 | 发版强制内嵌楔子与 updater-core（feat，增强 FR-193/发布管线）：`make embed-client-updater` / `release.yml` 在 jar 缺失或 0 字节时硬失败并中文报错；正式包禁止缺件；本地 dev 仍可缺 jar 启动并返回 `JAR_NOT_EMBEDDED`（免 spec） | P1 | 📋 计划 |
