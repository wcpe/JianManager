...
| FR-131 | 侧边栏可折叠图标轨 + 隐藏滚动条 + 布局持久化 | P2 | 🔨 开发中 |
| FR-132 | 侧栏底部控件图标化（主题/语言/退出）+ 三态直选 + 底部布局 | P2 | 🔨 开发中 |
| FR-133 | 实例树搜索/虚拟化/折叠保留/激活态/空态/a11y | P2 | 📋 计划 |
| FR-134 | 统一页头与面包屑组件 | P3 | 🔨 开发中 |
| FR-135 | 开源许可与依赖清单页（独立页，照参考图布局） | P2 | 🔨 开发中 |
| FR-136 | 实例列表汇总头 + 节点/端口列 + 角色徽标 + proxy↔backend inline | P2 | 🔨 开发中 |
...
| FR-144 | 节点页直观化 | P2 | 🔨 开发中 |
| FR-145 | 群组管理可寻址双栏 + proxy↔backend 拓扑 | P2 | 🔨 开发中 |
| FR-146 | 玩家管理增强 | P2 | 📋 计划 |
| FR-147 | Bot 规模化管理增强 | P2 | 🔨 开发中 |
| FR-148 | 趋势图增强 | — | ✅ 已交付 |
| FR-149 | 告警增强 | — | ✅ 已交付 |
| FR-150 | 日志中心增强 | P2 | 🔨 开发中 |
| FR-151 | 备份页增强 | P2 | 🔨 开发中 |
| FR-152 | 备份存储测试连接 + 容量展示 | P2 | 📋 计划 |
| FR-153 | 计划任务增强 | — | ✅ 已交付 |
| FR-154 | 模板应用到实例 + 变量填充预览 | P2 | 🔨 开发中 |
| FR-155 | 平台资产与更新页增强 | P2 | 📋 计划 |
| FR-156 | 用户与组管理能力补齐 | — | ✅ 已交付 |
| FR-157 | 认证体验增强 | — | ✅ 已交付 |
| FR-158 | 设置与审计页增强 | P2 | 🔨 开发中 |
| FR-159 | 共享对话框统一 | P2 | 📋 计划 |
| FR-160 | 共享基件统一（ref 重构） | P2 | 📋 计划 |
| FR-161 | 全局响应式与防翻屏基线 | P2 | 📋 计划 |
| FR-162 | 全局页眉/顶栏（基础信息 + 搜索占位） | P2 | 🔨 开发中 |
| FR-163 | 视觉底座与设计系统（统一 Panel/StatCard 组件、弃 shadcn Card 松散用法、靛蓝圆角灵动 + 响应式基线） | P1 | 🔨 开发中 |
| FR-164 | 全局双主题（靛蓝 / 青绿）+ 明暗模式（CSS 变量驱动 + 一处切换全站 + 持久） | P1 | 🔨 开发中 |
| FR-165 | 实例多级嵌套分组（前端树+列表+拖拽+批量标记+筛选；后端分组树 parent_id 自引用 + 实例-组 M:N） | P2 | 📋 计划 |
| FR-166 | 可组合卡片工作区（单实例可拖拽画布 + 6 功能卡 + 预设个人级持久化） | P2 | 📋 计划 |
| FR-167 | 跨实例超级工作台（任意实例卡片拼合 + 实例库拖拽） | P2 | 📋 计划 |
| FR-168 | 工作区导播台（多路 WS 预热 + 瞬切 + 定时轮播 + 并发上限 + 非激活降频） | P2 | 📋 计划 |
| FR-169 | 监控页升级（平台/节点/实例 6 指标 + 每图时间筛选 + brush 拖拽轴 + hover 浮窗 + 实时；进程粒度 TOP10 拆后端待登记，扩 FR-060/061） | P1 | 🔨 开发中 |
