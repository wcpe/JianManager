...
| FR-388 | Agent 入口可证安全闸（feat）：三入口同 Token 同输入行为契约表（CLI ↔ MCP ↔ curl）+ 真机复验矩阵（≥30 条断言：scope 外/Token 吊销/过期/写白名单外/硬拒绝面[用户写/删实例/DB 浏览/自更新/审计删除]/并发两入口同时写/速率限制）+ CI `release.yml` 增 agent-gate test 闸 + 契约入 `docs/API.md`（需 spec，依赖 FR-384/385/386 完成后起跑）→ `docs/specs/agent-safety-gate/spec.md` | P1 | 📋 计划 |
