...
| FR-433 | 实例角色可变更与 beacon 配套服务角色（feat，增强 FR-032/ADR-007）：`PUT /api/v1/instances/:id` 支持 `role`（非法值 400 拒绝，与 Create 的 grandfather 回落语义刻意不同），MCP `instance_update_config` 暴露 `role` 枚举；新增 `beacon` 角色标识配套服务实例（非 MC 群组服角色：不参与 proxy↔backend 拓扑、不适用 MC 探针与后端注册、不参与玩家查询）。role 变更不做隐式级联——原为 proxy 的 `server_registrations` 保留，避免误删运维数据 | P1 | 🔨 开发中·实现完成（待发版） |
| FR-434 | 群组服 Network 与代理注册的 MCP 工具（feat，增强 FR-032/335）：补齐 MCP 侧群组管理与拓扑能力——`network_list/get/create/update/delete`（删除需 `confirmName` 精确确认）、`network_add_members/remove_member`、`topology_get`（一次聚合代理注册 + 群组成员）、`registration_list/create/delete`；权限与实例分组同面（读 `instance.read`、写 `instance.write`），新增 `networkDomainActions` 独立 action 目录。使 MCP 客户端可完整维护群组服拓扑，不再依赖人工登录面板 | P1 | 🔨 开发中·实现完成（待发版） |
| FR-435 | 实例分组 MCP 工具（feat，增强 FR-165/ADR-033）：`instance_group_tree/members/create/update/delete/add_members/remove_members` 七个工具，让 MCP 客户端可按大区/小区等结构树做批量运维；分组仅承载运维归类，不承载权限或配额（读 `instance.read`、写 `instance.write`） | P1 | 🔨 开发中·实现完成（待发版） |
...
| FR-439 | MCP 会话并发上限造成永久 429（fix，缺陷修复 FR-389）：`MaxSessionsPerToken=4`/`MaxGlobalSessions=32` 在客户端「重连建新会话但不关旧会话」时会耗尽配额，此后连 `initialize` 都建不了会话，只能重启控制面清空内存会话恢复。现并发上限语义改为「0=不限制」且为默认（会话生命周期已由空闲 30m/绝对 24h 超时兜底），`Normalize()` 不再把 0 回落到正数，需要限流的环境可显式设正值；同时 MCP 会话失效响应保留规范要求的 404 + `SESSION_GONE` 错误码，便于与「路由不存在」区分排障 | P1 | 🔨 开发中·实现完成（待发版） |
