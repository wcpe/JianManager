...
- FR-326（CP↔Worker 实例反向对账，需 spec）→ `docs/specs/instance-reverse-reconcile/spec.md`（开发中）
- FR-362/363/365/369~372（500+ Bot 分布式（编号因 dev 已占用 FR-351～361 顺延）命令压测 2026-07-18；2026-07-20 由 FR-369～372 按 ADR-075 取代未实施的 FR-364/366/367/368）：FR-362 发压节点池 `docs/specs/bot-distributed-load/spec.md`（+ADR-074；已交付@v0.19.0，**缩比真机门禁已过**——≥2 真 Worker、跨节点分片预检、真连+stop+容量拒绝+故障区分；满规模 10×50/500 可选扩容）、FR-363 Scenario V2 历史兼容 `docs/specs/bot-scenario-engine/spec.md`（已交付@v0.19.0，缩比 Scenario V2 真连+契约校验已过；pathfinder/实体/500 cohort 可选）、FR-365 长稳恢复 `docs/specs/bot-runtime-recovery/spec.md`、FR-369 通用命令编排 `docs/specs/bot-load-probe-events/spec.md` + `docs/specs/bot-load-platform/super-spec.md`、FR-370 运行判定 `docs/specs/bot-load-runner/spec.md`、FR-371 创建向导 `docs/specs/bot-load-wizard/spec.md`、FR-372 实时观测 `docs/specs/bot-load-observability/spec.md`。编排命令成功仅表示 `bot.chat` 未同步抛错；ServerProbe/TPS/MSPT/业务事件仅为可选观测。执行序：FR-362 → FR-363 → [FR-365∥FR-369] → FR-370 → [FR-371∥FR-372]
- FR-384~388（Agent 安全操控后台 2026-07-23：CP 侧 Agent Token 鉴权真源 + 写白名单 + scope + 硬拒绝 + `jm-agent` 独立 Go CLI + 首版 `mcp-bridge` stdio + Token 管理 UI + 跨入口可证安全闸 + 文档/ADR-076/077；编号顺延至 384 起（373~383 已用于文件管理器批次）；计划见 `.tmp/plan-agent-control-plane-2026-07-23.md`）→ FR-384 `docs/specs/agent-token/spec.md`、FR-385 `docs/specs/jm-agent-cli/spec.md`、FR-386 `docs/specs/mcp-bridge/spec.md`（**由 FR-389/392 取代**：stdio 独立进程退役）、FR-387 免 spec、FR-388 `docs/specs/agent-safety-gate/spec.md`（矩阵扩展见 FR-389 批）。**原批不做**（已由下批推翻）：远程 SSE/HTTP MCP 端口——改由 FR-389 在 CP 内嵌落地
- FR-389~392（Agent MCP 长连接运维 + 调用观测 2026-07-23：CP 内嵌 Streamable HTTP + 兼容 SSE；可运维会话（列表/踢线/超时/并发上限）；`agent_call` 调用流水 + Token `lastUsedAt`/24h 活跃；观测 UI；删除 `apps/mcp-bridge` 并 supersede ADR-077；jmagent 带 `X-JM-Agent-Client` 同记流水；扩展 FR-388 矩阵为 MCP(HTTP/SSE)/CLI/curl。**不做**：mTLS、写操作面板审批流、统计大盘图表、tool 入参/出参全文落库、Control Plane 多副本会话粘滞以外的 HA。计划见 `.tmp/brainstorm-agent-mcp-obs-2026-07-23.md`）→ FR-389 `docs/specs/cp-mcp-server/spec.md`（+ 改写 ADR-077）；FR-390 `docs/specs/agent-call-log/spec.md`；FR-391 免 spec（UI 契约可附 FR-389/390 末节）；FR-392 `docs/specs/retire-mcp-bridge/spec.md`（或并入 FR-389 spec 退役章 + jmagent 契约增量）。执行序：FR-384 已具备 → [FR-389∥FR-390] → FR-391 → FR-392（可与 391 尾段并行）→ FR-388 矩阵补跑
- FR-373~378（文件权限底座 + 导入引导 + 资源管理器 Windows 对齐体验 + 多窗跨标签 DnD + 统一壳全场景接入 2026-07-23；增强 FR-302/070/071/008/031/083/213/261；**不做** root/chown/递归 chmod/Windows ACL；计划见 `.tmp/brainstorm-file-explorer-perm-ux-2026-07-23.md`）→ FR-373 `docs/specs/file-permission-foundation/spec.md`；FR-374 `docs/specs/import-permission-path-ux/spec.md`；FR-375 `docs/specs/file-explorer-windows-ux/spec.md`；FR-376 `docs/specs/file-explorer-multi-window/spec.md`；FR-377 `docs/specs/file-explorer-cross-window-dnd/spec.md`（+ 建议新 ADR 跨标签 DnD/剪贴板总线）；FR-378 `docs/specs/file-explorer-unified-shell/spec.md`。执行序：FR-373 → [FR-374∥FR-375] → FR-376 → [FR-377∥FR-378]
- FR-393~394（节点离线归档与彻底清理 2026-07-24：手动下线进归档、主列表仅活跃、NodesPage「归档」分段可查；归档「清理」硬删库记录，有实例 force 级联实例记录不碰远端文件；在线安全闸沿用 FR-048/309。**不做**在线一键下线、心跳自动归档、归档恢复回主列表、远端文件清理。交付验证：docker compose **仅 1 CP + 1 Worker**。计划见 `.tmp/brainstorm-node-archive-2026-07-24.md`）→ 共享 `docs/specs/node-offline-archive/spec.md`（FR-393 列表/UI + FR-394 清理章）。执行序：FR-393 → FR-394
...
| FR-326 | CP↔Worker 实例反向对账（feat，FR-310 孤儿评估缺口 (b)）：Worker 恢复/在跑的实例若 CP 已无记录会永远跑（正向对账已有：CP 以心跳为真源置 STOPPED；反向缺失）。Worker 心跳携带在管实例清单，CP 对无记录者下发处置指令（需防误杀护栏：宽限期+人工确认开关），proto+双侧改造（需 spec，后续批次）→ `docs/specs/instance-reverse-reconcile/spec.md` | P2 | 🔨 开发中 |
...
| FR-369 | 通用 Bot 命令编排与场景边界修订（feat，取代 FR-364，增强 FR-363/354）：新增 `command_schedule` 相对时间命令、稳定 commandId/occurrence checkpoint、绝对计划时间、本地集中 scheduler、取消、固定有限重试和结果聚合；`send_command`/命令发送成功仅表示 `bot.chat` 未同步抛错，不等待服务器回执。ServerProbe、塔防插件、TPS/MSPT 与业务事件仅为独立可选观测，不改变命令结果、preflight 或默认 verdict；既有 Scenario V2/塔防预设保留为显式 legacy，真实业务事件数据源不作为本批交付前提（需 spec + ADR-075，依赖 FR-362/352，可与 FR-365 非依赖部分并行）→ `docs/specs/bot-load-probe-events/spec.md`、`docs/specs/bot-load-platform/super-spec.md` | P0 | 🚧 开发中 |
| FR-370 | 通用命令压测运行状态机、负载曲线与自动判定（feat，取代 FR-366）：模板冻结 commandSchedule/loadProfile/thresholds；支持 stable/step/spike，聚合在线、命令发送、调度 lag、可选屏障和 Worker 健康，输出 verdict、maxStableBots、失败分类与 JSON/CSV 报告。默认适用指标均 ≥99%、schedule lag p95≤1 秒、非预期 crash=0；无屏障指标为 not_applicable，TPS/MSPT/业务事件不进入默认判定（需 spec，依赖 FR-362/352/354/358）→ `docs/specs/bot-load-runner/spec.md` | P0 | 🔨 开发中 |
| FR-371 | 通用命令压测模板与创建向导前端（feat，取代 FR-367）：Bot 管理拆为舰队/会话/命令模板；五步向导覆盖目标实例、连接配置、命令编排、负载曲线和阈值预检，支持结构化编辑/高级 YAML、模板 CRUD、容量分片与风险预览；不要求业务字段或 ServerProbe，5000 条 Mock 数据保持分页/虚拟化并通过中英、双主题、键盘与可访问性验收（需 spec，依赖 FR-362/352/359 API 契约）→ `docs/specs/bot-load-wizard/spec.md` | P1 | 🔨 开发中 |
| FR-372 | 通用命令压测实时观测、诊断与报告前端（feat，取代 FR-368）：会话级聚合 SSE 驱动概览、Bot/失败/事件分页、命令调度与发送进度、屏障、Worker 健康和报告；固定五类失败，支持 action trace、失败子集重试、停止/取消、JSON/CSV 导出。持续标注 `bot.chat` 成功边界，TPS/MSPT/ServerProbe 仅有值时作为 optional legacy 展示（需 spec，依赖 FR-370，可与 FR-371 非依赖部分并行）→ `docs/specs/bot-load-observability/spec.md` | P1 | 🔨 开发中 |
| FR-373 | 文件元数据与权限底座（feat）：`ListFiles`/`BrowseDir` 条目扩展 mode/readable/writable（相对 Worker 进程用户）及可取属主信息；写前 `CheckWritable`；单 path 非递归 `ChmodPath`（二次确认，不 chown/不 root）；permission denied 中文诊断（需 spec，跨 Worker-CP-Web）→ `docs/specs/file-permission-foundation/spec.md` | P0 | 🔨 开发中 |
| FR-374 | 导入现有服务器：手输绝对路径 + 权限引导 + 写配置预检（增强 FR-302）：DirectoryPicker 保留并加路径输入；inspect/读目录失败展示诊断与「尝试修复」(FR-373)/换路径/migrate；提交或写关键配置前可写预检，不过则阻断（需 spec，依赖 FR-373）→ `docs/specs/import-permission-path-ux/spec.md` | P0 | 🔨 开发中（DOM 测绿；真机闭环待用户节点） |
| FR-375 | 资源管理器单窗 Windows 对齐体验（增强 FR-070/071/141/008/031）：可编辑地址栏 + 前进后退历史栈 + 鼠标侧键 + 有栈时拦截浏览器后退；详情列头排序（名/时间/类型/大小/权限）+ 详情/列表/大图标视图切换与偏好记忆；列表/树滚动隔离不带动整页；权限列与锁标 + 编辑器可写态 + 保存/写路径统一预检与「尝试修复」（需 spec，依赖 FR-373）→ `docs/specs/file-explorer-windows-ux/spec.md` | P0 | 🔨 开发中（单元/DOM 测绿；侧键/整页滚动真机待确认） |
| FR-376 | 多实例壳：主区多标签 + 应用内浮动窗 + 浏览器新标签路由（feat）：标签多路径/多文件；弹出可拖拽缩放浮动窗并可收回；独立路由新开浏览器标签深链 path（需 spec，依赖 FR-375 体验契约）→ `docs/specs/file-explorer-multi-window/spec.md` | P1 | 🔨 开发中（单元/DOM 测绿；真机双标签+深链待确认） |
| FR-377 | 跨窗拖放 + 统一剪贴板总线（feat）：主区↔浮动窗↔跨浏览器标签 DnD 与剪切/复制/粘贴同一总线；与既有 rename/move/单文件 copy 语义对齐；系统文件拖入仍 HTML5 drop（需 spec + 新 ADR，依赖 FR-376）→ `docs/specs/file-explorer-cross-window-dnd/spec.md`、ADR-078 | P1 | 🔨 开发中（单元/DOM 测绿；跨标签真机待确认） |
| FR-378 | 统一文件管理器壳 + 能力注入，全场景接入（feat）：统一导航/视图/列表树/编辑槽/多窗宿主；Capability 注入（canWrite/canChmod/canUpload/标记清理/编排等）；接入实例文件与配置、平台存储、分发编排与预览、导入选目录等，**不删**分发标记等业务能力（需 spec，依赖 FR-375/376；可与 FR-377 部分并行）→ `docs/specs/file-explorer-unified-shell/spec.md` | P1 | 🔨 开发中（Capability+壳+存储/实例接入测绿；分发预览沿用 FileBrowser） |
...
| FR-385 | jm-agent 管理 CLI（feat，仿 jmctl）：独立 Go 二进制 `apps/jmagent/`，仅引入 HTTP 客户端与标准库，不链 gRPC/DB；子命令 MVP = `whoami/list nodes/list instances/instance status\|metrics\|logs/start\|stop\|restart/node maintenance enter\|leave`，写白名单仅含 `instance.life`+`node.maintenance`；非交互 `--output json` 给脚本/CI；403/硬拒绝文案稳定；Token 仅 env 与 `--token` 传入不落盘（需 spec + 新 ADR-079，依赖 FR-384；建议并行 worktree）→ `docs/specs/jm-agent-cli/spec.md` | P1 | 🔨 开发中 |
...
| FR-387 | Agent Token 管理 UI（feat）：设置 → Agent Token 页 = 列表 / 新建 / 详情 / 吊销 / 复制命令片段；非管理员 403；UI 契约仅附在 FR-384 spec 末尾（免 spec：纯前端消费 FR-384 API，无新模型/新契约/新 ADR，依赖 FR-384；建议并行 worktree） | P1 | 🔨 开发中 |
| FR-388 | Agent 入口可证安全闸（feat）：同 Token 同输入行为契约表（**CLI ↔ CP-MCP(Streamable HTTP/SSE) ↔ curl**，原 mcp-bridge 入口退役）+ 真机复验矩阵（≥30 条断言：scope 外/Token 吊销/过期/写白名单外/硬拒绝面[用户写/删实例/DB 浏览/自更新/审计删除]/并发两入口同时写/速率限制/会话踢线后 401）+ CI agent-gate 闸 + 契约入 `docs/API.md`（需 spec，依赖 FR-384/385/**389** 完成后起跑；扩展见 FR-389 批）→ `docs/specs/agent-safety-gate/spec.md` | P1 | 🔨 开发中 |
| FR-389 | CP 内嵌 MCP 长连接服务（feat，取代 FR-386 远程入口形态）：Control Plane 提供 **Streamable HTTP（主）+ 兼容 SSE** MCP 端点；鉴权复用 Agent Token（FR-384）；工具集与 jm-agent 对齐、硬拒绝不注册；**可运维会话**：列表（token 名/前缀、IP、已连时长、最近 tool、传输类型 streamable_http\|sse）、强制踢线、空闲超时、绝对超时、每 Token/全局并发上限；会话态内存为主（单 CP 进程，HA 粘滞不在本期）；平台管理员 API 管理会话（需 spec + **改写 ADR-077** 为「CP 内嵌 MCP 网关」；依赖 FR-384）→ `docs/specs/cp-mcp-server/spec.md` | P0 | 🔨 开发中（单测绿；待真机验） |
| FR-390 | Agent 调用流水与 Token 活跃（feat）：统一 `agent_call_logs`（或等价）记录 MCP tool 与 Agent HTTP API（含 jmagent）的 action、tokenId、client（`X-JM-Agent-Client`：mcp\|jmagent\|curl\|unknown）、IP、成功/失败、耗时、时间；读路径也记；保留窗口可配置（默认建议 7~30 天）；Token 鉴权继续 bump `lastUsedAt` 并提供 24h 调用次数聚合只读字段/接口；写操作可仍写既有 audit 并在 detail 标明 actor（需 spec，依赖 FR-384；可与 FR-389 并行）→ `docs/specs/agent-call-log/spec.md` | P0 | 🔨 开发中 |
| FR-391 | Agent 观测 UI（feat）：平台管理下 MCP 会话页（列表/踢线/超时说明）+ 调用流水页（按 token/action/client/成功失败/时间筛）+ Agent Token 列表展示 `lastUsedAt` 与 24h 次数 + 审计页可筛 actor=agent / action 前缀 `agent.`；复制「IDE 连接 CP MCP URL」片段；非管理员 403（免 spec：纯前端消费 FR-389/390 API，UI 契约附两 FR spec 末节；依赖 FR-389/390） | P1 | 🔨 开发中（页面/导航/i18n 已落地；审计页 action 前缀筛选沿用既有；远程 UI 随下次 dist 上线） |
| FR-392 | 退役 mcp-bridge + jmagent 客户端标识（feat/docs）：删除 `apps/mcp-bridge` 与 stdio 文档路径；README/接入指引改为 CP MCP URL；`jmagent` 全局发送 `X-JM-Agent-Client: jmagent`；构建/CI/发布物去掉 mcp-bridge；更新 FR-388 契约与 ADR-077 状态为 superseded by 改写版（需 spec 或并入 FR-389 退役章；依赖 FR-389 主路径可连后执行，可与 FR-391 尾段并行）→ 退役说明可落 `docs/specs/cp-mcp-server/spec.md` 退役章 | — | ✅ 已交付（开发版）·`apps/mcp-bridge` 已删 + CI 清理 + 契约刷新 |
| FR-393 | 节点离线归档列表与详情（feat，增强 FR-048 软删可发现性）：主节点列表仅活跃节点；`NodesPage` 增加「归档」分段/Tab，列出已软删下线节点并只读查看摘要（名/host/原状态/下线时间等）；在线下线与名下实例守卫沿用 FR-048/309（在线拒绝、未 force 的 409 清单）。验收：离线下线后主列表消失、归档可见；在线仍拒；compose 1CP+1Worker 真机走通（需 spec）→ `docs/specs/node-offline-archive/spec.md` | P1 | 🔨 开发中 |
| FR-394 | 归档节点彻底清理（feat，增强 FR-309 清理闭环）：归档区对已下线节点提供「清理」= 硬删除节点库记录；有残留实例记录须 force 确认后级联删实例平台记录（**不清理远端文件**，文案明示）；DangerConfirm 输入节点名；写审计。验收：清理后主列表与归档均不可见；未 force 有实例则拒绝；compose 真机验（需 spec，依赖 FR-393 UI 入口；硬删 API 可与 FR-393 后端并行）→ `docs/specs/node-offline-archive/spec.md`（同目录清理章） | P1 | 🔨 开发中 |
### 范围外（后续版本，暂不纳入 V1）
