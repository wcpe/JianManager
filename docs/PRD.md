...
- FR-354~355（对外部署与展示 2026-07-21：Docker/compose 打磨可跑 + Control Plane 一键下载安装脚本；同批 docs：README 安装步骤与 MSW mock 压缩截图入库、根目录 SECURITY.md、上游 sdd 规则对账；**不做** GHCR 发布 / K8s / CP systemd 服务化 / 重做面板 Worker 装机；计划见 `.tmp/brainstorm-deploy-readme-security-2026-07-21.md`）→ FR-354 `docs/specs/docker-compose-deploy/spec.md`（需 spec，计划）；FR-355 `docs/specs/cp-oneclick-install/spec.md`（需 spec，计划）；DOCS-README / DOCS-SECURITY / GOV-RULES 不占 §4
...
| FR-355 | Control Plane 一键下载安装脚本（feat）：仓库提供从 GitHub Releases latest 按 OS/arch 拉取 `control-plane` 的 curl\|bash（及可选 PowerShell）；职责=下载+可执行权限+启动提示；失败可诊断；不做 systemd/Windows 服务化；Worker 仍文档化面板一键装（需 spec）→ `docs/specs/cp-oneclick-install/spec.md` | P1 | 📋 计划 |
