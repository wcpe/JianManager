...
- FR-472~483（2026-09-20 Worker 日志平台底座受控规格）→ `docs/specs/worker-log-platform-contract/spec.md`、`worker-log-acquisition/spec.md`、`worker-log-normalizer/spec.md`、`worker-victorialogs-runtime/spec.md`、`worker-log-lifecycle/spec.md`、`worker-log-deep-archive/spec.md`、`worker-log-query-federation/spec.md`、`cp-log-query-coordinator/spec.md`、`log-ingest-cutover/spec.md`、`logs-center-tiered-ui/spec.md`；**Shared Contracts（FR-472）已冻结**；FR-473~483 实现规格状态为 **实现中（地基/接线）**，契约规格保持已冻结
- FR-472~483（2026-09-20 Worker 日志平台底座受控规格）→ `docs/specs/worker-log-platform-contract/spec.md`、`worker-log-acquisition/spec.md`、`worker-log-normalizer/spec.md`、`worker-victorialogs-runtime/spec.md`、`worker-log-lifecycle/spec.md`、`worker-log-deep-archive/spec.md`、`worker-log-query-federation/spec.md`、`cp-log-query-coordinator/spec.md`、`log-ingest-cutover/spec.md`、`logs-center-tiered-ui/spec.md`；**Shared Contracts（FR-472）已冻结**；FR-472~483 实现规格状态为 **实现中（地基/接线）**，契约规格保持已冻结
...
- FR-472~483（**Worker 日志平台底座** 2026-09-20）：正式版完整底座——采集/处理/持久/HOT VL/COLD Lifecycle/Deep+Rehydrate/Worker 层内联邦/CP 跨节点联邦/停实例日志入 CP/日志中心改造。能力齐套、三层按配置启用；Paper 默认 FILE_PRIMARY；CP 仅 platform+audit+Legacy 只读。**不做**：CH/Loki 实现、日志告警引擎、浏览器直连 VL。架构审核：范围与 FR 划分保留。**门禁重分类（2026-09-20）**：FR-472 冻结=「契约与治理可开工」——退出条件为 Shared Contracts 齐备 + 依赖层证据（VL 行为/supervisor-equivalent smoke/纯逻辑）+ **ADR-094 accepted** + **proto/能力协商终审** + **VL 资产正式审批** + **性能判据方法与初始阈值入契约**；**Worker 生产组件级集成测不是冻结前置**（否则与「冻结前不实现 434+」死锁），改为 FR-473（Runbook A WAL/恢复分段）/ FR-475（Runbook C supervisor wiring）/ FR-476（Runbook B Catalog 崩溃与旧 owner 过滤）的开发验收条件。计划见 `.tmp/brainstorm-worker-log-platform-foundation-2026-09-20.md` 与 `.tmp/fr433-contract-freeze-checklist-2026-09-20.md`（均不入库）→ FR-472～483 见 §4（状态：**FR-472 契约已冻结**；FR-472~483 **开发中·地基/接线**；438/442 发布必需）；FR-482 docs **开发中·文档对账**
- FR-483（**canonical 事件体落盘 compact**，2026-09-24）：FR-472 契约 §4.3/§5.3 的「VL 数据根重建权威事件集合」原先内联在 `ingest.state.json` 并常驻内存，随单源留存事件数线性增长（真机 64 源实测 state ≈143–189MB、Worker 稳态 ≈674MiB，1GiB 仅够 ≈40 源）。量测归因显示主因不是事件体常驻（+95MiB）而是 `persist()` 整份 `MarshalIndent` 的瞬时分配（+620MiB 且 `sys` 不归还）。本 FR 把事件体改为**追加式磁盘段**（NDJSON + MANIFEST 原子替换、只容忍尾段截断、中间段损坏硬失败），**集合内容与语义完全不变**（仍为全部已登账事件、仍不可按 reclaim 裁剪）。**不做**：内存裁剪（已被三个回归测试否决）、新压缩格式、改动事件身份/投影代次/覆盖水位。→ `docs/specs/worker-log-canonical-eventstore/spec.md` + ADR-095（状态：**实测通过**，文档对账进行中）
...
| FR-472 | Worker 日志平台 Shared Contracts 与 ADR（feat，开发前闸门）：新 ADR（Worker 本地 VictoriaLogs + CP 联邦；与 ADR-005/013/066/081 对账）；**冻结正式版基线**：事件身份与采集账本、投递语义与重复对账、checkpoint/WAL/ACK/回收四水位、Partition Catalog 与唯一查询所有权、IngestRouter、PublishedProjection、canonical projection 与 `closed_visible_seq`、`logs.lifecycle` 配置分层与合法部署组合、**完整 gRPC 消息契约**、权限覆盖全查询/导出面、Worker 日志资源预算与**性能判据方法及初始阈值**、能力协商、Legacy/导出/覆盖语义；`stream_fields` 白名单。**冻结退出=契约+治理+依赖层证据**（ADR-094 accepted、proto/协商终审、VL 资产正式审批、性能判据入契约）；**Worker 级 Runbook A/B/C 不作为冻结前置**，分别归属 FR-473/436/437 验收 → `docs/specs/worker-log-platform-contract/spec.md` | P0 | 📅 契约已冻结·开发前闸门（ADR-094 accepted；不等于 FR-472 已交付） |
| FR-473 | Worker 日志采集与持久化通道（feat）：FileTailer + StdIO + ArchiveImporter **共用采集账本**；源分段身份与 `latest.log→轮转→.gz` 关联；cursor/WAL/分水位回收/坏记录隔离；STDIO 受管 Raw 分段；容量耗尽与缺口可见；**hash+size 仅识别归档对象**；不引入 Filebeat 等。**验收含 Runbook A**（Worker WAL 强杀/ACK 丢失/reclaim/恢复分段释放与 hold 保护真实 PASS）→ `docs/specs/worker-log-acquisition/spec.md` | P0 | 🔨 开发中·生产采集/投影/ACK恢复/reclaim 已通过；**Runbook A 真机（强杀 `-9` 重启）通过并修复重启时序缺陷**；磁盘容量**真实填盘验收通过**（容器内受限 tmpfs：真实越过 80%→`DEGRADED_STORAGE`、91.67%→`PAUSED` 且必记缺口；经真实 `Pipeline.Ingest` 落账本 `AcquirePaused`+缺口+未投递；**真实 ENOSPC** 填至 100% 观测到 `no space left on device`；见 `worker-log-acquisition/acceptance-real.md`） |
| FR-474 | 日志 Multiline 与归一化（feat）：Java/MC 多行归并；level/time/logger/instance_id；Multiline 上限/超时/半条恢复；时区跨午夜；解析失败保留原文+状态；parser_version；**事件数 vs 行数**口径 → `docs/specs/worker-log-normalizer/spec.md` | P0 | 🔨 开发中·**真机验收通过**（真 VL v1.52.0：Multiline 5 行归并为 1 事件、跨午夜回拨、半条事件 Flush 闭合、账本 ready；**修复损坏编码致 canonical hash 与 VL 正文不一致的缺陷**——净化 + `encoding_sanitized` 审计标记，VL 侧重算 hash 一致）；7 条新回归 + 2 项变异验证；Windows 平台证据待补 |
| FR-475 | VictoriaLogs 资产分发与 Worker 运行时（feat）：**CP 只内嵌/分发**经校验官方 OSS 资产（linux-amd64 + windows-amd64）；Worker 本地 supervisor 管理 HOT/COLD/Rehydrate（localhost + 本地鉴权）；资产审批=tag/包与解包校验/许可/兼容；安全停机；资源总预算；失败日志防递归；cache≠RSS。**验收含 Runbook C**（wiring/防递归/预算降级/老 Worker Unimplemented）→ `docs/specs/worker-victorialogs-runtime/spec.md` | P0 | 🔨 开发中·v1.52.0 资产审批、supervisor/CP runtime 面、远程三实例、预算采样与 80/90 降级评估、**CP 资产上传→签名下载→Worker 安装真机闭环**、GOMEMLIMIT 内存调优均已通过；**Windows 包校验与解包已补真机证据**（真实包经 GitHub asset API 取得：双层哈希与审批值一致、`InstallApprovedPackage` 真实 zip 解包 PASS、CP `Cache`/`Open` 校验通过并拒绝篡改；见 `worker-victorialogs-runtime/acceptance-windows.md`）——**Windows 运行时行为仍未覆盖**（需 Windows 主机执行 `.exe`） |
| FR-476 | 分区 Catalog 与冷热 Lifecycle（feat）：Partition Catalog（owner/generation/journal）；迁移状态机（冻结路由→排空→snapshot→staging→**QueryPlanner 排除**→原子切 owner→租约排空→detach→清理）；迟到日志按 Catalog 路由；retention 协同；`cold.enabled=false` 单层合法；**detach≠完成**。**验收含 Runbook B**（Catalog 适配层各状态崩溃恢复 + re-attach 后旧 owner 过滤）→ `docs/specs/worker-log-lifecycle/spec.md` | P0 | 🔨 开发中·catalog 状态机单测绿·**Runbook B 真机迁移链已通过**（FROZEN→…→CLEANED，数据物理迁移、HOT 空/COLD 持有） |
| FR-477 | Deep Archive 与 Rehydrate（feat，**发布必需**）：Provider local/S3/MinIO；VL 分区按 namespace+UTC 日+generation；manifest 含 schema/parser/engine；受管 Raw Archive；Rehydrate 任务化（租约/合并/超时/预留/generation 隔离）；幂等重恢复 → `docs/specs/worker-log-deep-archive/spec.md` | P1·发布必需 | 🔨 开发中·**真机 RustFS S3 验收通过**（登记/幂等/manifest/对象往返/Rehydrate 租约+复用+预留拒绝+完成/发布 generation 清理矩阵）；manifest 记录真实 VL build_id |
| FR-478 | Worker 日志查询服务（feat）：Catalog 驱动不重叠权威副本；Search/Stats/Tail/Fields/Facets/StorageHealth/Rehydrate/ArchiveStatus；覆盖状态与 Cursor/水位；取消与预算；**完整验收依赖 437/438** → `docs/specs/worker-log-query-federation/spec.md` | P0 | 🔨 开发中·全查询 RPC/受管 VL 已接线，远程 Search/Stats/Facets/Export 通过；**CP 联邦真机查询通过**（`complete=true`、目标 success、返回事件） |
| FR-479 | CP 跨 Worker 日志查询协调器（feat）：**全部授权目标 + 覆盖汇总**；「仅在线」为显式选项；历史持有者；稳定复合排序 K-way merge；可合并聚合；Facet 截断语义；统一时间桶；禁止全量原始日志入 CP；导出复用有界 Search/同一快照，**缺口/截断/超预算不交付成功附件** → `docs/specs/cp-log-query-coordinator/spec.md` | P0 | 🔨 开发中·logcoord+HTTP+生产 Assemble（inst/node 解析+真实 Readiness）；**真机联邦查询与延迟取证通过**（串行 p95 28.5ms、8 并发 p95 15.2ms） |
| FR-480 | 日志入库切换与 Legacy 只读（feat）：停止**实例与 Worker/Node**日志新入 CP `logs`；platform/audit + **LegacyLogReader**；Legacy 独立保留预算与到期规则；容量预检不足不切换；逐 Worker 切换水位；控制台订阅与持久化推送分离；历史 Multiline 可追溯原文；未统一口径禁止拼精确趋势 → `docs/specs/log-ingest-cutover/spec.md` | P0 | 🔨 开发中·地基/接线·cutover+Legacy+DualPath + **HTTP 管理面已注册**（默认关闭）；Legacy/cutover 行为由 service/router 单测覆盖；DualPath federated 侧由 `/logs/federation` 承担（无生产调用方，保留 API 不加投机接线） |
| FR-481 | 日志中心与仪表盘（feat，**发布必需**）：范围/级别/关键字/时间/列表/Tail/Hits/Stats/Facets；partial/离线/缺数据/截断/积压/Rehydrate 失败等非成功态；Legacy 标识；导出失败不交付成功附件；nextCursor=null≠历史完整；失败不得呈完整零结果 → `docs/specs/logs-center-tiered-ui/spec.md` | P1·发布必需 | 🔨 开发中·**真浏览器验收通过**（真实联邦列表/Stats·Facets 一致、失败态引导；修了 Stats `agg.count` 读法缺陷）；失败态 DOM 矩阵 19 项 |
| FR-482 | 日志平台文档对账（docs）：Shared Contracts/ADR 迁入受控文档；资产清单与许可；恢复手册；数据损失边界；资源预算；合法配置组合；切换与故障处理；ARCHITECTURE/API/CHANGELOG | — | 🔨 开发中·docs（已对账 ARCHITECTURE/API/CHANGELOG/PRD/规格状态；已补受控文档 `worker-victorialogs-runtime/asset-inventory.md`（资产清单/许可）+ `worker-log-platform-contract/recovery-manual.md`（恢复与故障处置）+ `worker-log-platform-contract/acceptance-record.md`（**持久真机验收记录**，缓解 `.tmp` 证据不入库）） |
| FR-483 | canonical 事件体落盘 compact（feat）：把 FR-472 §4.3/§5.3「VL 数据根重建的权威事件集合」由 state.json 内联+内存常驻改为追加式磁盘段存储（NDJSON + MANIFEST 原子替换），**内容与语义不变、仍不可按 reclaim 裁剪**；含旧格式迁移、崩溃/截断恢复语义、`persist()` 流式写元数据；并加界三处跨轮无界累积结构（`n.events`/`linePos`/`delivered`）收敛采集期峰值。目标单 Worker 150 源稳态 RSS ≤300MiB → `docs/specs/worker-log-canonical-eventstore/spec.md` + ADR-095 | P1 | 🔨 开发中·**真机验收通过**（node-main + 真 VictoriaLogs v1.52.0：64/128/150 源稳态 RSS 22.5/22.3/20.0MiB、峰值 56.0/62.0/61.8MiB，判据 ≤300MiB；state.json 77 803KiB→177.9KiB 并与事件数解耦；**342 源逐一核对 1 026 000 条事件零丢失零重复**）；五个门禁回归 + 幂等回归 + 四条累积结构回归（均含变异验证）全绿；**曾发布错误读数已更正**（未保活致 GC 回收） |
