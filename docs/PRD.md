...
| FR-391 | Agent 观测 UI（feat）：平台管理下 MCP 会话页（列表/踢线/超时说明）+ 调用流水页（按 token/action/client/成功失败/时间筛）+ Agent Token 列表展示 `lastUsedAt` 与 24h 次数 + 审计页可筛 actor=agent / action 前缀 `agent.`；复制「IDE 连接 CP MCP URL」片段；非管理员 403（免 spec：纯前端消费 FR-389/390 API，UI 契约附两 FR spec 末节；依赖 FR-389/390） | P1 | 📋 计划 |
