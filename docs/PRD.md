...
| FR-326 | CP↔Worker 实例反向对账（feat，FR-310 孤儿评估缺口 (b)）：Worker 恢复/在跑的实例若 CP 已无记录会永远跑（正向对账已有：CP 以心跳为真源置 STOPPED；反向缺失）。Worker 心跳携带在管实例清单，CP 对无记录者下发处置指令（需防误杀护栏：宽限期+人工确认开关），proto+双侧改造（需 spec，后续批次）→ `docs/specs/instance-reverse-reconcile/spec.md` | P2 | 📋 计划 |
...
| FR-384 | Agent Token 模型 + CP 侧鉴权（feat）：`agent_tokens` 表 + `POST/GET/DELETE /api/v1/agent/tokens` 颁发/吊销/列表 + `agent_token` 中间件（独立于人类 JWT）+ 策略引擎真源（默认只读 + 写白名单 + scope 收敛 + 硬拒绝面）+ 审计 `actor_kind=agent` 贯通；CP 为唯一真源，jm-agent/mcp-bridge/curl 三入口共享同一闸（需 spec + 新 ADR-076，跨 middleware/service/router/model）→ `docs/specs/agent-token/spec.md` | P1 | 🔨 开发中 |
| FR-385 | jm-agent 管理 CLI（feat，仿 jmctl）：独立 Go 二进制 `apps/jmagent/`，仅引入 HTTP 客户端与标准库，不链 gRPC/DB；子命令 MVP = `whoami/list nodes/list instances/instance status\|metrics\|logs/start\|stop\|restart/node maintenance enter\|leave`，写白名单仅含 `instance.life`+`node.maintenance`；非交互 `--output json` 给脚本/CI；403/硬拒绝文案稳定；Token 仅 env 与 `--token` 传入不落盘（需 spec + 新 ADR-076，依赖 FR-384；建议并行 worktree）→ `docs/specs/jm-agent-cli/spec.md` | P1 | 🔨 开发中 |
