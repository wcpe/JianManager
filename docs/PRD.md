...
| FR-343 | 实例系统级基础指标（无需探针，走系统直取）（feat，增强 FR-170/142，真机反馈：实例未连 ServerProbe 时 TPS/MSPT/CPU/内存无源、概览显示占位）：把 Worker 侧已采集的 gopsutil 进程 CPU%/RSS 内存/运行时长（FR-170 进程指标）暴露到实例详情 API + 控制台概览，作为不依赖探针的基础指标底座；无探针时仍显真实 CPU/内存/运行时长，TPS/MSPT/在线数标注「需探针」而非假值（需 spec）→ `docs/specs/instance-system-metrics/spec.md` | P1 | 📋 计划 |
