...
| FR-068 | 探针在线更新（推送即就位 + 下次重启生效） | — | ✅ 已交付@v0.7.0·全真栈验收（真机复验发现运行中实例 Windows classpath 锁致更新 422，已修：同内容跳过 + 锁定降级 + Windows 独占锁自动化回归，待真机复验绕锁生效） |
...
| FR-074 | 跨文件全文搜索与持久倒排索引 | — | ✅ 已交付@v0.9.0·验收经 FR-113 覆盖 |
| FR-075 | 归档浏览与反编译 | — | ✅ 已交付@v0.9.0·全真栈验收 |
| FR-076 | 全量 Bukkit 状态探查（异步非侵入）+ WS 按需查询 | — | ✅ 已交付@v0.9.0·验收经 FR-142 覆盖 |
| FR-077 | 「服务器状态」专属 tab | — | ✅ 已交付@v0.9.0·验收经 FR-142 覆盖 |
...
| FR-140 | 终端体验增强 | P2 | 🔨 开发中（重连/全屏/字号/读写徽标/菜单 a11y 与 Ctrl/Cmd+F 搜索计数、当前定位、上下切换和可见区高亮已落地；走查发现重连复用一次性 token 致 401 已修：Terminal 不再静态持有 token，改由 TerminalPane 暴露 fetchToken 回调每次连接前现取新一次性 token（自动重试+手动重连均覆盖），DOM 回归锁定，待真机复验；首连约 54s 即断已加固：CP 终端代理（browser/worker 两侧）与 Worker WS 桥补 WS ping/pong 心跳（~30s ping + ~70s 读超时续期），空闲终端经反代/LB 不被按空闲超时断连，单元测试复现「无心跳空闲断连」并回归「有心跳保活」，待真机复验） |
| FR-141 | 资源管理器与配置编辑器增强 | P2 | 🔨 开发中（CodeMirror 暗色跟随、目录树 a11y 键盘导航/虚拟化、二进制与超大文件下载降级、搜索片段高亮与目录/扩展名范围、版本 diff 增删着色、批量粘贴/移动进度、配置表单分组与即时校验已落地；版本 diff 增删着色已补齐（配置版本抽屉复用共享 UnifiedDiff 组件、+/- 语义色 token 着色随明暗自适配/双主题正交，DOM 测试覆盖，待真机复验）） |
