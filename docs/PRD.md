...
- FR-346~350（客户端分发上传增效与外置对象存储 2026-07-17：上传秒传/聚合/并发 + BlobStore 抽象与 S3[rustfs] 存储渠道 + 存量迁移 + 一致性对账 + 编排器多级目录/右键定点添加；关联新 ADR[外置对象存储与 302 预签名分发] + 增强 FR-250/251/088/191；BUG「0 字节文件上传被拒 totalSize 须为正」走 sdd-fix-bug 不占 §4）→ FR-346 `docs/specs/client-upload-efficiency/spec.md`、FR-347 `docs/specs/artifact-external-storage/spec.md`（+新 ADR）、FR-348 `docs/specs/artifact-storage-migration/spec.md`、FR-349 `docs/specs/artifact-s3-reconcile/spec.md`（均需 spec，开发中创建）；FR-350 免 spec（纯前端）；执行序：波1[BUG+346+350 并行] → 波2[347 单跑] → 波3[348∥349]；计划见 `.tmp/brainstorm-client-dist-storage-2026-07-17.md`
...
| FR-346 | 客户端分发上传协议增效：秒传预查 + 小文件聚合 + 有限并发（feat，增强 FR-250/251，真机反馈：数千文件整合包发布逐文件串行 init→chunk→complete 请求过多）：新增批量 sha256 预查端点（前端上传前算哈希、一请求查 N 个、命中制品库 CAS 者跳过上传直接引用既有制品）+ 小文件聚合上传端点（一请求携带多个小文件，免三段协议）+ 前端文件级有限并发与进度整合；大文件保留 FR-251 分块协议（需 spec）→ `docs/specs/client-upload-efficiency/spec.md` | P1 | 🔨 开发中 |
| FR-347 | 制品库外置对象存储底座：BlobStore 抽象 + 本地/S3 适配器 + 存储渠道配置页 + 302 预签名下载（feat，增强 FR-088 修订 ADR-011 存储节，用户诉求：制品上传自部署 rustfs 做数据分发、CP 不存大对象）：抽 BlobStore 接口（Put/Open/Stat/Delete/List/Presign，后续新后端只加适配器）——本地适配器封装现 CAS、S3 适配器复用 worker SigV4 纯标准库实现并补 presign；新表制品存储渠道 + 「文件存储配置」管理页（CRUD/连通测试/设活跃渠道；凭证面板直填可逆加密存储，沿 FR-192 先例）；Asset.StorageBackend 按记录路由下载——local 走 ServeContent、s3 返 302 预签名短时效 URL（updater 已开跟随重定向，真机必验）（需 spec + ADR）→ `docs/specs/artifact-external-storage/spec.md` | P1 | 🔨 开发中 |
| FR-348 | 制品库存量迁移工具（feat，依赖 FR-347）：存储渠道页发起「迁移到渠道 X」后台任务——逐制品校验搬运→更新 Asset 存储标注→删源；进度可见/失败明细/中断幂等续跑，迁移后下载按新位置路由（需 spec）→ `docs/specs/artifact-storage-migration/spec.md` | P1 | 🔨 开发中 |
| FR-349 | 制品索引可视化与 S3 一致性对账（feat，依赖 FR-347）：制品列表加存储位置/对账状态列；对账任务（手动触发 + 定期默认每日）比对 Asset 索引与 S3 对象清单，产出缺失（索引有 S3 无）/孤儿（S3 有索引无）差异报告；处置走显式按钮——缺失标记失效并提示重传、孤儿二次确认后一键清理，不做全自动修复（需 spec）→ `docs/specs/artifact-s3-reconcile/spec.md` | P2 | 🔨 开发中 |
| FR-350 | 发布编排器文件树定点增强：多级目录一次建 + 右键定点添加（feat，增强 FR-191/250/261）：「新建文件夹」支持输入 `a/b/c/d` 自动创建整条层级、支持多行一次建多个；右键目录菜单增「上传文件到此 / 上传文件夹到此」，所选内容落到该目录路径下（免 spec，纯前端——FileExplorer 右键基座与 emptyDirs 机制已有） | P2 | 🔨 开发中 |
