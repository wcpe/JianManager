...
- **状态**: ✅ done（已交付@v0.9.0；自动化验收全绿[go build/vet/test 含 router 端点测试 + 前端 tsc/lint/build]；真机[建索引/搜命中/改文件增量/忽略规则]待用户环境补）
- **优先级**: P2
...
#### FR-075: 归档浏览与反编译
- **状态**: ✅ done（已交付@v0.9.0；自动化验收全绿；真机[打开真 plugin jar 看内部、反编译 class 出源码，需节点 JDK]待用户环境补）
- **优先级**: P1
...
- **状态**: ✅ done（已交付@v0.9.0；自动化验收全绿；深层真机[真 Paper 按需拉全状态、TPS 无可感下降、classloader 专区有数据]待用户环境补）
- **优先级**: P2
...
#### FR-077: 「服务器状态」专属 tab
- **状态**: ✅ done（已交付@v0.9.0；自动化验收全绿；真机[「服务器状态」tab 渲染真探针数据]待用户环境补）
- **优先级**: P2
...
- **状态**: ✅ done（已交付@v0.9.0；自动化验收全绿；真机[CP/Worker 二进制热升级、版本号为证、daemon 下游戏服不掉]待用户环境补）
- **优先级**: P1
