...
| FR-378 | 文件元数据与权限底座（feat）：`ListFiles`/`BrowseDir` 条目扩展 mode/readable/writable（相对 Worker 进程用户）及可取属主信息；写前 `CheckWritable`；单 path 非递归 `ChmodPath`（二次确认，不 chown/不 root）；permission denied 中文诊断（需 spec，跨 Worker-CP-Web）→ `docs/specs/file-permission-foundation/spec.md` | P0 | 📋 计划 |
| FR-379 | 导入现有服务器：手输绝对路径 + 权限引导 + 写配置预检（增强 FR-302）：DirectoryPicker 保留并加路径输入；inspect/读目录失败展示诊断与「尝试修复」(FR-378)/换路径/migrate；提交或写关键配置前可写预检，不过则阻断（需 spec，依赖 FR-378）→ `docs/specs/import-permission-path-ux/spec.md` | P0 | 📋 计划 |
| FR-380 | 资源管理器单窗 Windows 对齐体验（增强 FR-070/071/141/008/031）：可编辑地址栏 + 前进后退历史栈 + 鼠标侧键 + 有栈时拦截浏览器后退；详情列头排序（名/时间/类型/大小/权限）+ 详情/列表/大图标视图切换与偏好记忆；列表/树滚动隔离不带动整页；权限列与锁标 + 编辑器可写态 + 保存/写路径统一预检与「尝试修复」（需 spec，依赖 FR-378）→ `docs/specs/file-explorer-windows-ux/spec.md` | P0 | 📋 计划 |
| FR-381 | 多实例壳：主区多标签 + 应用内浮动窗 + 浏览器新标签路由（feat）：标签多路径/多文件；弹出可拖拽缩放浮动窗并可收回；独立路由新开浏览器标签深链 path（需 spec，依赖 FR-380 体验契约）→ `docs/specs/file-explorer-multi-window/spec.md` | P1 | 📋 计划 |
| FR-382 | 跨窗拖放 + 统一剪贴板总线（feat）：主区↔浮动窗↔跨浏览器标签 DnD 与剪切/复制/粘贴同一总线；与既有 rename/move/单文件 copy 语义对齐；系统文件拖入仍 HTML5 drop（需 spec + 新 ADR，依赖 FR-381）→ `docs/specs/file-explorer-cross-window-dnd/spec.md` | P1 | 📋 计划 |
| FR-383 | 统一文件管理器壳 + 能力注入，全场景接入（feat）：统一导航/视图/列表树/编辑槽/多窗宿主；Capability 注入（canWrite/canChmod/canUpload/标记清理/编排等）；接入实例文件与配置、平台存储、分发编排与预览、导入选目录等，**不删**分发标记等业务能力（需 spec，依赖 FR-380/381；可与 FR-382 部分并行）→ `docs/specs/file-explorer-unified-shell/spec.md` | P1 | 📋 计划 |
