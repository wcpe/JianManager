...
| FR-360 | updater-core 遥测与启动画像字段补齐 + 隐私契约（feat，增强 FR-094/264）：客户端上报补齐 core 版本、arch、javaVendor、locale/timezone、memoryTier 等运营诊断字段；明确 required vs diagnostic；隐私 notice/opt-out 与面板展示脱敏规则；允许 schema/落库扩展（需 spec）→ `docs/specs/client-telemetry-enrich-privacy/spec.md` | P1 | 📋 计划 |
