...
| FR-345 | 未运行/历史日志查看（终端回放 DB 历史 + 独立历史增强）（feat，增强控制台终端与日志能力，真机反馈：手动关服后终端变黑看不到关服过程、停止态无历史）：实例停止/崩溃时终端自动从 DB 回放最近 N 行历史（可上滚、重连/切页签/刷新不丢），关服不清屏；实例级历史日志查看支持时间筛选/搜索/导出（在现有 Logs 能力上加实例入口）；软依赖 FIX-3（重启优雅关服使关服日志先存在）（需 spec）→ `docs/specs/instance-log-history/spec.md` | P2 | 📋 计划 |
