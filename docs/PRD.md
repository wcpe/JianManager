...
| FR-346 | 客户端分发上传协议增效：秒传预查 + 小文件聚合 + 有限并发（feat，增强 FR-250/251，真机反馈：数千文件整合包发布逐文件串行 init→chunk→complete 请求过多）：新增批量 sha256 预查端点（前端上传前算哈希、一请求查 N 个、命中制品库 CAS 者跳过上传直接引用既有制品）+ 小文件聚合上传端点（一请求携带多个小文件，免三段协议）+ 前端文件级有限并发与进度整合；大文件保留 FR-251 分块协议（需 spec）→ `docs/specs/client-upload-efficiency/spec.md` | P1 | 📋 计划 |
...
| FR-350 | 发布编排器文件树定点增强：多级目录一次建 + 右键定点添加（feat，增强 FR-191/250/261）：「新建文件夹」支持输入 `a/b/c/d` 自动创建整条层级、支持多行一次建多个；右键目录菜单增「上传文件到此 / 上传文件夹到此」，所选内容落到该目录路径下（免 spec，纯前端——FileExplorer 右键基座与 emptyDirs 机制已有） | P2 | 📋 计划 |
