...
| FR-284 | devmock 抽为独立包（ref，免 spec）：`web/src/mocks`→`packages/devmock`，依赖反转（只依赖 API 契约，不再 `import '@/…'` app 内部）；主应用与测试改引 `@jianmanager/devmock`；生产构建仍不含 devmock（ADR-047 不变量保持）。依赖 FR-283 先落 | P2 | 🔨 开发中 |
| FR-285 | Go 入口迁入 apps（ref，免 spec）：`cmd/*`→`apps/*`（control-plane/worker/jmctl）；`internal/` 不动；同步 `Makefile`/`release.yml`/`Dockerfile.*` 的 go build 路径；go:embed 目标目录不变。main 包无 importer，行为不变（关联 ADR-064） | P2 | 🔨 开发中 |
| FR-286 | bot-worker 迁入 apps（ref，免 spec）：`bot-worker`→`apps/bot-worker`；同步 Go worker 运行时 spawn 路径解析与打包；须真机验 bot 真入服 spawn 成功（关联 ADR-064）。依赖 FR-285 | P2 | 🔨 开发中 |
| FR-287 | 统一命令面 go-task（feat，需 spec）：引入 `Taskfile.yml` 作唯一命令入口，各语言暴露 build/test/lint/dev 同套动词委托 go/pnpm/gradlew，Windows+Linux 双跑通；`Makefile` 收敛为被委托层（关联 ADR-064）→ `docs/specs/monorepo-unification/spec.md`。依赖 FR-283/285 路径落定 | P2 | 🔨 开发中 |
| FR-288 | 前端版本接上单一真源（feat，需 spec）：版本唯一真值 = `internal/version/version.go`（ADR-065 既定）；把前端 `__APP_VERSION__` 从独立记版的 `web/package.json` 改为构建期从真源派生注入，改 `version.go` 一处→前端展示+Go `/version`+产物路径（ADR-036）一致，消除前端漂移（实证：真源已 `0.15.0-dev`、前端仍报 `0.14.0`）（关联 ADR-064/065）→ `docs/specs/monorepo-unification/spec.md`。依赖 FR-283 | P2 | 🔨 开发中 |
