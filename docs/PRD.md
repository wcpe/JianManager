...
| FR-387 | Agent Token 管理 UI（feat）：设置 → Agent Token 页 = 列表 / 新建 / 详情 / 吊销 / 复制命令片段；非管理员 403；UI 契约仅附在 FR-384 spec 末尾（免 spec：纯前端消费 FR-384 API，无新模型/新契约/新 ADR，依赖 FR-384；建议并行 worktree） | P1 | 📋 计划 |
