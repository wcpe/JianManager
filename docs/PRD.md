...
- FR-351~353（客户端更新器内嵌硬门禁 + 接入/版本面板信息补齐 + 楔子/core 本地日志轮换 2026-07-21：发版构建缺 wedge/updater-core 硬失败；guide 与版本旁路展示内嵌版本；`.jm-updater/logs` 按 10MiB/跨日轮换 gzip 保留 5；增强 FR-107/193/260/090；计划见 `.tmp/brainstorm-client-updater-embed-logrotate-2026-07-21.md`）→ FR-351 免 spec；FR-352 `docs/specs/client-updater-guide-embed-info/spec.md`（开发中创建）；FR-353 `docs/specs/updater-local-log-rotate/spec.md`（开发中创建）
- FR-354~355（对外部署与展示 2026-07-21：Docker/compose 打磨可跑 + Control Plane 一键下载安装脚本；同批 docs：README 安装步骤与 MSW mock 压缩截图入库、根目录 SECURITY.md、上游 sdd 规则对账；**不做** GHCR 发布 / K8s / CP systemd 服务化 / 重做面板 Worker 装机；计划见 `.tmp/brainstorm-deploy-readme-security-2026-07-21.md`；验收证据 `.tmp/acceptance-deploy-readme-security-2026-07-21.md`）→ FR-354 `docs/specs/docker-compose-deploy/spec.md`（需 spec，开发中）；FR-355 `docs/specs/cp-oneclick-install/spec.md`（需 spec，开发中）；DOCS-README / DOCS-SECURITY / GOV-RULES 不占 §4
- FR-356~361（客户端分发安全/遥测/统计闭环 2026-07-21：KPI 口径统一 + 统计监控加厚钻取 + 安全研判处置 + 跨页深链 + updater-core 遥测隐私字段 + CSV 导出；允许新聚合 API/必要 schema；**不做** 告警联动/CDN 多节点防护/重写安全状态机；计划见 `.tmp/brainstorm-client-dist-obs-security-loop-2026-07-21.md`）→ FR-356 `docs/specs/client-dist-kpi-semantics/spec.md`（开发中）；FR-357 `docs/specs/client-dist-stats-monitor-enrich/spec.md`（计划，已落盘草拟 spec）；FR-358 `docs/specs/client-dist-security-ops-loop/spec.md`（计划，已落盘草拟 spec）；FR-359 免 spec（深链，无契约）；FR-360 `docs/specs/client-telemetry-enrich-privacy/spec.md`（开发中）；FR-361 `docs/specs/client-dist-csv-export/spec.md`（计划，已落盘草拟 spec）
...
| FR-351 | 发版强制内嵌楔子与 updater-core（feat，增强 FR-193/发布管线）：`make embed-client-updater` / `release.yml` 在 jar 缺失或 0 字节时硬失败并中文报错；正式包禁止缺件；本地 dev 仍可缺 jar 启动并返回 `JAR_NOT_EMBEDDED`（免 spec） | P1 | 🚧 开发中 |
| FR-352 | 接入指引补齐内嵌更新器信息 + 版本面板旁路摘要（feat，增强 FR-107/260）：`client-channels?tab=guide` 展示展示版本串、core 整数版本、wedge/core available/size 与下载；未内嵌明确失败态；频道版本列表/详情/发布页旁路展示「内嵌更新器 vX · core N」摘要；本批不含 commit/buildTime 元信息（需 spec）→ `docs/specs/client-updater-guide-embed-info/spec.md` | P1 | 🚧 开发中 |
| FR-353 | 楔子与 updater-core 本地诊断日志轮换压缩（feat，增强 FR-090）：`wedge.log` / `updater.log` 各自在打开时若 ≥10MiB 或跨自然日则轮换，旧文件时间戳命名并 gzip，保留最近 5 个归档；轮换失败 fail-open 不挡启动；阈值代码常量、不做运营可配（需 spec）→ `docs/specs/updater-local-log-rotate/spec.md` | P1 | 🔨 开发中 |
...
| FR-355 | Control Plane 一键下载安装脚本（feat）：仓库提供从 GitHub Releases latest 按 OS/arch 拉取 `control-plane` 的 curl\|bash（及可选 PowerShell）；职责=下载+可执行权限+启动提示；失败可诊断；不做 systemd/Windows 服务化；Worker 仍文档化面板一键装（需 spec）→ `docs/specs/cp-oneclick-install/spec.md` | P1 | 🔨 开发中 |
| FR-356 | 客户端分发 KPI 口径与共享语义（feat，增强 FR-095/217/218/219/265）：统一定义「下载健康 / 更新健康 / 活跃客户端精确|近似 / 安全微标」字段名与计算公式；频道统计 Tab 与分发监控统计 Tab 共用语义（可抽共享组件）；文案区分请求成功 vs 更新成功；允许只读聚合补全缺字段（需 spec）→ `docs/specs/client-dist-kpi-semantics/spec.md` | P1 | 🔨 开发中 |
| FR-357 | 分发统计与监控信息加厚 + 分布钻取（feat，增强 FR-095/218/219/249/265）：KPI 绝对数、下载 bytes 趋势、错误码 TopN、失败样例、日志列加厚（player/machine/core/bytes/耗时/可读 err）、详情结构化、分布点击过滤日志/客户端；空态区分无流量/未开遥测/窗外；依赖 FR-356 口径（需 spec）→ `docs/specs/client-dist-stats-monitor-enrich/spec.md` | P1 | 📋 计划 |
| FR-358 | 安全中心研判处置闭环 + 频道安全摘要（feat，增强 FR-096/264）：画像详情全量字段与风险时间线、事件行一键封 IP/改 key 态/频道防护（DangerConfirm）、频道工作台安全摘要条链到安全中心；安全日志 detail 结构化；不可信字段统一角标；允许频道安全摘要聚合 API（需 spec）→ `docs/specs/client-dist-security-ops-loop/spec.md` | P1 | 📋 计划 |
| FR-359 | 分发三页跨页深链与统一筛选状态（feat，增强 FR-265/264）：`channelId`/`ip`/`machineId`/`errCode`/`version`/`tab` 等 query 在频道工作台、分发监控、安全中心互通；分布/榜单/事件一键跳转保留筛选；不改后端业务语义（免 spec） | P1 | 📋 计划 |
| FR-360 | updater-core 遥测与启动画像字段补齐 + 隐私契约（feat，增强 FR-094/264）：客户端上报补齐 core 版本、arch、javaVendor、locale/timezone、memoryTier 等运营诊断字段；明确 required vs diagnostic；隐私 notice/opt-out 与面板展示脱敏规则；允许 schema/落库扩展（需 spec）→ `docs/specs/client-telemetry-enrich-privacy/spec.md` | P1 | 🔨 开发中 |
| FR-361 | 分发统计与安全日志 CSV 导出（feat，增强 FR-095/249/264）：管理端导出当前筛选窗口的统计汇总/事件日志/安全日志 CSV；仅平台管理员；导出列遵守 FR-360 隐私脱敏；限流防拖库（需 spec）→ `docs/specs/client-dist-csv-export/spec.md` | P2 | 📋 计划 |
| FR-362 | Bot 发压节点池与分布式调度底座（feat，增强 FR-038/042/274，500+ Bot 地基）：将目标游戏实例与执行 Bot 的 Worker 节点解耦，CP 发现具备 Node/bot-worker 运行时的可用发压节点及容量，启动前生成跨 Worker 分片计划并做容量预检；批量、幂等地下发创建/停止与逐批回执，容量不足或节点失联时明确拒绝/降级，不产生无主 Bot 记录。真机门禁（2026-07-21 用户确认缩比）：≥2 真 Worker、>50 跨节点分片预检、缩比真实 connected（非仅 DB 行）、容量不足 ready=false、stop 与故障区分；满规模 10 Worker/500 真连为可选扩容（需 spec + ADR-074）→ `docs/specs/bot-distributed-load/spec.md` | P0 | 🔨 开发中（自动化+缩比真机门禁已过；满规模 10×50/500 可选扩容；待发版标已交付） |
| FR-363 | 可验证 Bot 场景引擎与塔防预设（feat，增强 FR-274）：结构化场景支持按比例分组、固定随机种子、同步屏障，以及 `wait_spawn` / 主城区域漫游 / `send_command` / 等待探针事件 / `move_to_and_wait` / 锁敌追击 / `attack_until` / 死亡重生与重新进房；每步具备超时、重试、取消和成功/失败结果，兼容代理跨服与单服多房间。塔防预设支持「主城行走→命令进房→同步开局→抵达区域→持续普通攻击」；首版不含技能、放塔、商店和复杂背包（需 spec，依赖 FR-362）→ `docs/specs/bot-scenario-engine/spec.md` | P0 | 🔨 开发中（自动化+缩比 Scenario V2 真连/契约已过；pathfinder·实体·500 cohort 可选 partial；待发版标已交付） |
