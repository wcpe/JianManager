...
| FR-386 | mcp-bridge 独立进程（feat）：新 Go 二进制 `apps/mcp-bridge/`，stdio 默认（远端 IDE 经 HTTPS CP）；工具集仅暴露 CLI 等价的 action 子集；硬拒绝面永不对 MCP 暴露；CP 403 → MCP `isError=true` 含中文拒绝原因；不引入数据库/gRPC/Worker；不暴露独立网络监听（需 spec + 新 ADR-077，依赖 FR-384；建议并行 worktree）→ `docs/specs/mcp-bridge/spec.md` | P1 | 📋 计划 |
