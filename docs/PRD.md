...
| FR-358 | 安全中心研判处置闭环 + 频道安全摘要（feat，增强 FR-096/264）：画像详情全量字段与风险时间线、事件行一键封 IP/改 key 态/频道防护（DangerConfirm）、频道工作台安全摘要条链到安全中心；安全日志 detail 结构化；不可信字段统一角标；允许频道安全摘要聚合 API（需 spec）→ `docs/specs/client-dist-security-ops-loop/spec.md` | P1 | 🔨 开发中 |
| FR-359 | 分发三页跨页深链与统一筛选状态（feat，增强 FR-265/264）：`channelId`/`ip`/`machineId`/`errCode`/`version`/`tab` 等 query 在频道工作台、分发监控、安全中心互通；分布/榜单/事件一键跳转保留筛选；不改后端业务语义（免 spec） | P1 | 🔨 开发中 |
...
| FR-361 | 分发统计与安全日志 CSV 导出（feat，增强 FR-095/249/264）：管理端导出当前筛选窗口的统计汇总/事件日志/安全日志 CSV；仅平台管理员；导出列遵守 FR-360 隐私脱敏；限流防拖库（需 spec）→ `docs/specs/client-dist-csv-export/spec.md` | P2 | 🔨 开发中 |
