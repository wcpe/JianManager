...
| FR-294 | 页眉集群统计浮窗（增强 FR-162/179，复用 FR-216 铃铛浮窗范式）：在线节点/运行中服务器/崩溃服务器三徽标点击改弹缩略浮窗——节点行=名称+状态点+运行实例数、服务器行=名称+节点+在线人数、崩溃行=名称+节点+崩溃原因（statusReason）；行点击进对应服控制台/节点页，底部「查看全部」保留原筛选页跳转；数据仅浮窗打开时拉取，行数上限 8 超出提示剩余数；崩溃空态友好文案（免 spec） | P2 | 📋 计划 |
