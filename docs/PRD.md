...
| FR-348 | 制品库存量迁移工具（feat，依赖 FR-347）：存储渠道页发起「迁移到渠道 X」后台任务——逐制品校验搬运→更新 Asset 存储标注→删源；进度可见/失败明细/中断幂等续跑，迁移后下载按新位置路由（需 spec）→ `docs/specs/artifact-storage-migration/spec.md` | P1 | 📋 计划 |
| FR-349 | 制品索引可视化与 S3 一致性对账（feat，依赖 FR-347）：制品列表加存储位置/对账状态列；对账任务（手动触发 + 定期默认每日）比对 Asset 索引与 S3 对象清单，产出缺失（索引有 S3 无）/孤儿（S3 有索引无）差异报告；处置走显式按钮——缺失标记失效并提示重传、孤儿二次确认后一键清理，不做全自动修复（需 spec）→ `docs/specs/artifact-s3-reconcile/spec.md` | P2 | 📋 计划 |
