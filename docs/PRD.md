...
- FR-362~368（500+ Bot 分布式压测 2026-07-18：发压节点池与跨 Worker 分片 / 可验证场景引擎与塔防预设 / ServerProbe 断言桥 / 长稳重连归真 / 固定长稳+阶梯升压+突发洪峰 / 完整创建向导 / 实时观测诊断报告；离线测试环境，手动运行+模板，兼容代理跨服与单服多房间，首版不含技能/放塔/复杂背包；**编号因 dev 已占用 FR-351～361 顺延**）→ FR-362 `docs/specs/bot-distributed-load/spec.md`（+ADR-074，Go/Node 自动化门与 FR-362 定向 router race 已通过；全量 router race 受既有无关测试基建限制在 600 秒超时；因 `.tmp/bot-load-acceptance/environment.json` 不存在，10+ Worker / 500 Bot 真实连接、故障降级与 stop 全链路待真机），FR-363 `docs/specs/bot-scenario-engine/spec.md`、FR-364 `docs/specs/bot-load-probe-events/spec.md`、FR-365 `docs/specs/bot-runtime-recovery/spec.md`、FR-366 `docs/specs/bot-load-runner/spec.md`、FR-367 `docs/specs/bot-load-wizard/spec.md`、FR-368 `docs/specs/bot-load-observability/spec.md`；后续计划状态与执行序保持：FR-362 → FR-363 → [FR-364∥FR-365] → FR-366 → [FR-367∥FR-368]
...
| FR-363 | 可验证 Bot 场景引擎与塔防预设（feat，增强 FR-274）：结构化场景支持按比例分组、固定随机种子、同步屏障，以及 `wait_spawn` / 主城区域漫游 / `send_command` / 等待探针事件 / `move_to_and_wait` / 锁敌追击 / `attack_until` / 死亡重生与重新进房；每步具备超时、重试、取消和成功/失败结果，兼容代理跨服与单服多房间。塔防预设支持「主城行走→命令进房→同步开局→抵达区域→持续普通攻击」；首版不含技能、放塔、商店和复杂背包（需 spec，依赖 FR-362）→ `docs/specs/bot-scenario-engine/spec.md` | P0 | 🔨 开发中（自动化完成：Bot Worker 69/69，相关 Go 普通/race/build/vet 通过；真实 ServerProbe 事件依赖 FR-364，真 MC/pathfinder 与 500 Bot 环境验收阻塞，待真机） |
| FR-364 | ServerProbe 压测断言桥与塔防事件适配（feat，增强 FR-065/066）：定义通用压测事件契约，由可修改的塔防插件发布进入子服/房间、房间准备、开局、波次、伤害、击杀、死亡、重生、结束等事件，ServerProbe 经既有反向 WS→Worker→CP 链路上报并关联运行/Bot/房间/请求；命令、到达和攻击仅在收到可信回执后算成功，重复/乱序/迟到事件不得误判（需 spec，依赖 FR-363；不以聊天文本解析作为正式判据）→ `docs/specs/bot-load-probe-events/spec.md` | P0 | 📋 计划 |
| FR-365 | Bot 长稳重连、进程恢复与状态归真（feat，可靠性增强）：Bot 被踢/断线后按指数退避+随机抖动自动重连；bot-worker 子进程崩溃后恢复本节点期望 Bot 集合，Worker 重启/短暂离线后由 CP desired-state reconcile；状态带序号与时间，进程死亡后立即收敛而非残留旧 `connected`，恢复后重走当前阶段幂等入口。真机验收覆盖批量踢出、杀 bot-worker、重启 Worker、服务端离线后恢复，且无重连风暴、幽灵在线、无界定时器和持续内存增长（需 spec，依赖 FR-362；可与 FR-364 并行）→ `docs/specs/bot-runtime-recovery/spec.md` | P0 | 📋 计划 |
| FR-366 | 压测运行状态机、三类负载曲线与自动判定（feat）：分离可复用场景模板与每次运行记录，支持固定 500+ 长稳、阶梯升压找最大稳定并发、突发登录/开局/攻击洪峰；运行状态覆盖预检/启动/运行/降级/停止/完成/失败/取消，聚合 Bot 在线率、连接耗时、动作成功率、重连与错误，以及 Worker CPU/RSS、节点资源和目标服 TPS/MSPT，按阈值自动给出 verdict、最大稳定容量和安全停止。默认严格验收：500+ 连续 60 分钟，在线/进房/抵达/攻击成功率均 ≥99%，TPS≥19、MSPT p95≤50ms、Bot/Worker/目标服无崩溃；阶梯与洪峰报告含 p50/p95/p99 并区分目标服/发压端/网络/动作失败（需 spec，依赖 FR-362~354）→ `docs/specs/bot-load-runner/spec.md` | P0 | 📋 计划 |
| FR-367 | 压测场景模板与创建向导前端（feat，完整 Bot 压测配置体验）：Bot 管理信息架构拆为舰队/压测会话/场景模板；五步向导覆盖目标与节点池、连接配置、分组与行为、负载曲线、阈值与预检，结构化编辑为主并保留高级 YAML，支持保存/复制/恢复模板、容量分片预览、预计时长和风险提示。验收要求无需手写 YAML 即可配置塔防场景，500 Bot 容量不足时启动前明确缺口，5000 条 Mock 数据不全量铺开，并通过中英 i18n、暗亮主题、键盘与可访问性验收（需 spec，依赖 FR-362/352/355 API 契约）→ `docs/specs/bot-load-wizard/spec.md` | P1 | 📋 计划 |
...
- **500+ Bot 分布式塔防压测**（FR-351~357）：离线测试环境中以多 Worker 分片向同一目标服/房间发起固定长稳、阶梯升压和突发洪峰三类测试；正式通过至少要求 500+ Bot 连续 60 分钟在线/进房/抵达/攻击成功率均 ≥99%、TPS≥19、MSPT p95≤50ms，ServerProbe/塔防插件事件证明真实游戏动作，Bot/Worker/目标服无崩溃；前端必须完成配置、预检、实时观测、失败定位和报告闭环。
