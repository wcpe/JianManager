...
| FR-362 | Bot 发压节点池与分布式调度底座（feat，增强 FR-038/042/274，500+ Bot 地基）：将目标游戏实例与执行 Bot 的 Worker 节点解耦，CP 发现具备 Node/bot-worker 运行时的可用发压节点及容量，启动前生成跨 Worker 分片计划并做容量预检；批量、幂等地下发创建/停止与逐批回执，容量不足或节点失联时明确拒绝/降级，不产生无主 Bot 记录。真机验收：不少于 10 个 Worker、单 Worker 不超过声明容量，500 个 Bot 实际同时连接同一目标服而非仅落 500 条 DB 记录（需 spec + 新 ADR，保持三进程模型并部分修订 ADR-006 的单 Worker 归属假设）→ `docs/specs/bot-distributed-load/spec.md` | P0 | 🔨 开发中（自动化完成，待 10+ Worker / 500 Bot 真机验收） |
| FR-363 | 可验证 Bot 场景引擎与塔防预设（feat，增强 FR-274）：结构化场景支持按比例分组、固定随机种子、同步屏障，以及 `wait_spawn` / 主城区域漫游 / `send_command` / 等待探针事件 / `move_to_and_wait` / 锁敌追击 / `attack_until` / 死亡重生与重新进房；每步具备超时、重试、取消和成功/失败结果，兼容代理跨服与单服多房间。塔防预设支持「主城行走→命令进房→同步开局→抵达区域→持续普通攻击」；首版不含技能、放塔、商店和复杂背包（需 spec，依赖 FR-362）→ `docs/specs/bot-scenario-engine/spec.md` | P0 | 🔨 开发中（自动化完成；成功边界与 legacy 交付门禁由 FR-369 修订，待真 MC/pathfinder 与 500 Bot 真机） |
