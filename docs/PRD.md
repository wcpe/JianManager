...
| FR-165 | 实例多级嵌套分组（前端树+列表+拖拽+批量标记+筛选；后端分组树 parent_id 自引用 + 实例-组 M:N） | P2 | 🔨 开发中 |
| FR-166 | 可组合卡片工作区（单实例可拖拽画布 + 6 功能卡 + 预设个人级持久化；取代 ADR-030，并入 FR-129/130） | P2 | 🔨 开发中 |
| FR-167 | 跨实例超级工作台（任意实例卡片拼合 + 实例库拖拽） | P2 | 🔨 开发中 |
| FR-168 | 工作区导播台（多路 WS 预热 + 瞬切 + 定时轮播 + 并发上限 + 非激活降频） | P2 | 🔨 开发中 |
