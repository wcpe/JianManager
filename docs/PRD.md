...
- **验收标准**:
  - [ ] `GET /logs` 支持筛选（instanceId/nodeId/level/keyword/from/to）+ 分页，DB 侧过滤不全量序列化
  - [ ] 前端日志中心页：筛选器 + 分页表 + 实时/历史切换
  - [ ] 关键字全文检索（基于 DB 能力，必要时建索引）
  - [ ] 可导出当前筛选结果
  - [ ] 权限：组成员仅见有权实例日志（沿用 RBAC）
  - [ ] i18n + 主题正常
