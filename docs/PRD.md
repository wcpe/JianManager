...
| FR-373 | 文件元数据与权限底座（feat）：`ListFiles`/`BrowseDir` 条目扩展 mode/readable/writable（相对 Worker 进程用户）及可取属主信息；写前 `CheckWritable`；单 path 非递归 `ChmodPath`（二次确认，不 chown/不 root）；permission denied 中文诊断（需 spec，跨 Worker-CP-Web）→ `docs/specs/file-permission-foundation/spec.md` | P0 | 📋 计划 |
