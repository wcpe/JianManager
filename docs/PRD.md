...
| FR-353 | 楔子与 updater-core 本地诊断日志轮换压缩（feat，增强 FR-090）：`wedge.log` / `updater.log` 各自在打开时若 ≥10MiB 或跨自然日则轮换，旧文件时间戳命名并 gzip，保留最近 5 个归档；轮换失败 fail-open 不挡启动；阈值代码常量、不做运营可配（需 spec）→ `docs/specs/updater-local-log-rotate/spec.md` | P1 | 📋 计划 |
