...
| FR-372 | 通用命令压测实时观测、诊断与报告前端（feat，取代 FR-368）：会话级聚合 SSE 驱动概览、Bot/失败/事件分页、命令调度与发送进度、屏障、Worker 健康和报告；固定五类失败，支持 action trace、失败子集重试、停止/取消、JSON/CSV 导出。持续标注 `bot.chat` 成功边界，TPS/MSPT/ServerProbe 仅有值时作为 optional legacy 展示（需 spec，依赖 FR-370，可与 FR-371 非依赖部分并行）→ `docs/specs/bot-load-observability/spec.md` | P1 | 📋 计划 |
