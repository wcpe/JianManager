...
| FR-371 | 通用命令压测模板与创建向导前端（feat，取代 FR-367）：Bot 管理拆为舰队/会话/命令模板；五步向导覆盖目标实例、连接配置、命令编排、负载曲线和阈值预检，支持结构化编辑/高级 YAML、模板 CRUD、容量分片与风险预览；不要求业务字段或 ServerProbe，5000 条 Mock 数据保持分页/虚拟化并通过中英、双主题、键盘与可访问性验收（需 spec，依赖 FR-362/352/359 API 契约）→ `docs/specs/bot-load-wizard/spec.md` | P1 | 📋 计划 |
