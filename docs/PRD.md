...
| FR-357 | 分发统计与监控信息加厚 + 分布钻取（feat，增强 FR-095/218/219/249/265）：KPI 绝对数、下载 bytes 趋势、错误码 TopN、失败样例、日志列加厚（player/machine/core/bytes/耗时/可读 err）、详情结构化、分布点击过滤日志/客户端；空态区分无流量/未开遥测/窗外；依赖 FR-356 口径（需 spec）→ `docs/specs/client-dist-stats-monitor-enrich/spec.md` | P1 | 🔨 开发中 |
