...
> **验收档位图例**：`·全真栈验收`=真 UI+真 CP/Worker+真外部进程（MC/Docker/MinIO 等）端到端；`·四档验收`=单测/集测/单机截图/真浏览器截图（后端 mock 基底）；**无后缀=开发交付，未做独立验收**。证据台账见 `.tmp/acceptance/ACCEPTANCE-LEDGER.md`。
...
| FR-059 | 危险操作保护体系化 | — | ✅ 已交付@v0.10.0·四档验收 |
| FR-060 | 时序监控与历史曲线 | — | ✅ 已交付@v0.5.0·全真栈验收 |
