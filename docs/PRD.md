...
| FR-479 | Worker 日志查询服务（feat）：Catalog 驱动不重叠权威副本；Search/Stats/Tail/Fields/Facets/StorageHealth/Rehydrate/ArchiveStatus；覆盖状态与 Cursor/水位；取消与预算；**完整验收依赖 437/438** → `docs/specs/worker-log-query-federation/spec.md` | P0 | 🔨 开发中·全查询 RPC/受管 VL 已接线，远程 Search/Stats/Facets/Export 通过；**CP 联邦真机查询通过**（`complete=true`、目标 success、返回事件） |
| FR-480 | CP 跨 Worker 日志查询协调器（feat）：**全部授权目标 + 覆盖汇总**；「仅在线」为显式选项；历史持有者；稳定复合排序 K-way merge；可合并聚合；Facet 截断语义；统一时间桶；禁止全量原始日志入 CP；导出复用有界 Search/同一快照，**缺口/截断/超预算不交付成功附件** → `docs/specs/cp-log-query-coordinator/spec.md` | P0 | 🔨 开发中·logcoord+HTTP+生产 Assemble（inst/node 解析+真实 Readiness）；**真机联邦查询与延迟取证通过**（串行 p95 28.5ms、8 并发 p95 15.2ms） |
...
| FR-482 | 日志中心与仪表盘（feat，**发布必需**）：范围/级别/关键字/时间/列表/Tail/Hits/Stats/Facets；partial/离线/缺数据/截断/积压/Rehydrate 失败等非成功态；Legacy 标识；导出失败不交付成功附件；nextCursor=null≠历史完整；失败不得呈完整零结果 → `docs/specs/logs-center-tiered-ui/spec.md` | P1·发布必需 | 🔨 开发中·**真浏览器验收通过**（真实联邦列表/Stats·Facets 一致、失败态引导；修了 Stats `agg.count` 读法缺陷）；失败态 DOM 矩阵 19 项 |
| FR-483 | 日志平台文档对账（docs）：Shared Contracts/ADR 迁入受控文档；资产清单与许可；恢复手册；数据损失边界；资源预算；合法配置组合；切换与故障处理；ARCHITECTURE/API/CHANGELOG | — | 🔨 开发中·docs（已对账 ARCHITECTURE/API/CHANGELOG/PRD/规格状态；已补受控文档 `worker-victorialogs-runtime/asset-inventory.md`（资产清单/许可）+ `worker-log-platform-contract/recovery-manual.md`（恢复与故障处置）+ `worker-log-platform-contract/acceptance-record.md`（**持久真机验收记录**，缓解 `.tmp` 证据不入库）） |
