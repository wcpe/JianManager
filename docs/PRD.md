...
| FR-471 | 在管实例运行态对齐与外来运行时接管（feat，增强 FR-302/326/456）：外部启动（tmux/脚本）的服务器进程不被平台接管——DB 记 STOPPED 而磁盘在跑、面板说谎且无出口；启动预检只查 java/目录/jar，不含现场占用，故此时点「启动」会双开撞 `world/session.lock` 与端口。现新增三件事：①孤儿扫描增第 4 相，按**已注册实例的 workDir**（不限受管 `serversDir`，覆盖就地导入）探测未纳管活进程，经心跳 `foreign_pid/foreign_cmdline` 上报，CP 落 `instances.runtime_drift_*` 供面板提示（只观测不自动处置）；②启动预检补 `work_dir_busy` 与 `port_free` 两项，把双开拦在转 STARTING 之前；③新增接管入口 `POST /instances/:id/adopt-runtime`——先做归属复核（防 PID 回收误杀），再 SIGTERM 进程树等其优雅退出（超时升级 SIGKILL），随后以受管方式拉起。**验收**：外部启动的农场逐台报出漂移；点「接管」后状态转 RUNNING 且与磁盘进程一一对应；对运行中实例点「启动」被预检拒绝而非双开 | P1 | 🔨 开发中·代码与单测已过，待真机验收（农场漂移识别 / 接管闭环 / 双开拦截） |
