...
| FR-436 | daemon 实例 wrapper 已死而 Java 仍活时的孤儿接管（fix，缺陷修复 FR-325 同源场景）：接管扫描原逻辑只检查 wrapper，见到 wrapper 不存活即删 PID 文件——而 wrapper 死、其托管 Java 仍活时（父进程变 init），活着的 Java 从此不可发现，继续占服务端口与 Paper `session.lock`、面板却显示 STOPPED 且实例再起不来（真机事故：CP 重启后 63 个实例集体失联）。现改为按 PID 记录强杀 Java 树，死透才清理、杀不死则保留 PID 文件待下次兜底 | P0 | 🔨 开发中·实现完成（待发版） |
| FR-437 | 压测容量快照陈旧导致的伪容量不足（fix，缺陷修复 FR-398）：①容量快照老化阈值 15s 仅比 bot-worker 10s 心跳多 5s 余量，一次 GC/负载抖动即判 `CAPACITY_SNAPSHOT_STALE`；②预检与容量查询工具走 15s TTL 缓存而非实时快照。二者叠加使节点明明有空闲容量仍报「可用容量 0」，需反复重试偶中。现阈值放宽到 30s（3 倍心跳），并让预检与 `loadtest_node_capacity` 优先经 `RefreshBotLoadCapacity` 取实时值 | P1 | 🔨 开发中·实现完成（待发版） |
| FR-438 | bot-worker 大规模舰队的事件循环过载（fix，缺陷修复 FR-398）：Mineflayer 为每个 Bot 起 50ms 物理模拟定时器，数百 Bot 时单 Node 进程事件循环被占满（实测 500 容量下 p95 达 5s），keepalive 与心跳均无法调度——服务器集体判 `Timed out` 踢线，容量快照亦停止推进。现按行为判定关闭无需本地碰撞的 Bot 的物理模拟（`idle` 等不自行移动的行为默认关闭，`follow/patrol/guard` 保持开启），并把默认视距降到 `tiny`；另新增 `JM_BOT_WORKER_MAX_BOTS` 配置让容量上限可经 worker.yml 下发 | P0 | 🔨 开发中·实现完成（待发版） |
