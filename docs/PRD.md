...
| FR-356 | 客户端分发 KPI 口径与共享语义（feat，增强 FR-095/217/218/219/265）：统一定义「下载健康 / 更新健康 / 活跃客户端精确|近似 / 安全微标」字段名与计算公式；频道统计 Tab 与分发监控统计 Tab 共用语义（可抽共享组件）；文案区分请求成功 vs 更新成功；允许只读聚合补全缺字段（需 spec）→ `docs/specs/client-dist-kpi-semantics/spec.md` | P1 | 📋 计划 |
