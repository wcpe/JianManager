...
| FR-342 | 搭建失败损毁态 + 一键重建（feat，增强 FR-319 异步搭建，真机反馈：断网/下载核心失败后实例只留 STOPPED+statusReason「搭建未完成」，无重建入口、只能删了重填参数）：新增实例状态 DAMAGED（损毁），任何搭建阶段失败（下载/校验/配置写入/探针部署/Forge 安装器）都进损毁态并保留原始搭建参数；新增「重建」动作复用已存配置重跑搭建（成功→STOPPED、再失败→仍 DAMAGED），零重填；状态机加 DAMAGED 转移 + 重建 RPC/端点 + 前端损毁态徽章与重建按钮（需 spec）→ `docs/specs/provision-damaged-rebuild/spec.md` | P1 | 🔨 开发中 |
| FR-343 | 实例系统级基础指标（无需探针，走系统直取）（feat，增强 FR-170/142，真机反馈：实例未连 ServerProbe 时 TPS/MSPT/CPU/内存无源、概览显示占位）：把 Worker 侧已采集的 gopsutil 进程 CPU%/RSS 内存/运行时长（FR-170 进程指标）暴露到实例详情 API + 控制台概览，作为不依赖探针的基础指标底座；无探针时仍显真实 CPU/内存/运行时长，TPS/MSPT/在线数标注「需探针」而非假值（需 spec）→ `docs/specs/instance-system-metrics/spec.md` | P1 | 🔨 开发中 |
| FR-344 | 实例环境变量管理（双区展示 + 可编辑写入 .env）（feat，增强 FR-034 搭建/结构化启动，用户要求「加环境变量 tab、支持自配启动环境变量写入 .env」）：实例详情新增「环境变量」页签——上区可增删改自定义启动环境变量、保存写入工作目录 .env 文件、下次启动注入生效；下区展示运行中 JVM 实际完整环境（Worker 上报启动时所用 env，含继承 PATH/JAVA_HOME，只读，停机提示仅运行时可读）；env 编辑 API + .env 文件契约 + 启动管线注入 + 前端页签（需 spec）→ `docs/specs/instance-env-vars/spec.md` | P2 | 🔨 开发中 |
| FR-345 | 未运行/历史日志查看（终端回放 DB 历史 + 独立历史增强）（feat，增强控制台终端与日志能力，真机反馈：手动关服后终端变黑看不到关服过程、停止态无历史）：实例停止/崩溃时终端自动从 DB 回放最近 N 行历史（可上滚、重连/切页签/刷新不丢），关服不清屏；实例级历史日志查看支持时间筛选/搜索/导出（在现有 Logs 能力上加实例入口）；软依赖 FIX-3（重启优雅关服使关服日志先存在）（需 spec）→ `docs/specs/instance-log-history/spec.md` | P2 | 🔨 开发中 |
