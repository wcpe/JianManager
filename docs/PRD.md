...
| FR-362 | Bot 发压节点池与分布式调度底座（feat，增强 FR-038/042/274，500+ Bot 地基）：将目标游戏实例与执行 Bot 的 Worker 节点解耦，CP 发现具备 Node/bot-worker 运行时的可用发压节点及容量，启动前生成跨 Worker 分片计划并做容量预检；批量、幂等地下发创建/停止与逐批回执，容量不足或节点失联时明确拒绝/降级，不产生无主 Bot 记录。真机验收：不少于 10 个 Worker、单 Worker 不超过声明容量，500 个 Bot 实际同时连接同一目标服而非仅落 500 条 DB 记录（需 spec + 新 ADR，保持三进程模型并部分修订 ADR-006 的单 Worker 归属假设）→ `docs/specs/bot-distributed-load/spec.md` | P0 | 📋 计划 |
