...
| FR-293 | 侧栏常驻服务器列：收藏置顶+最近打开（增强 FR-240，补当年搁置的 Part B 常驻实例列）：「选择服务器」按钮下方常驻两区=收藏（置顶）+最近打开（LRU ≤8），行=状态点+名称，点击进该服控制台；行内星标收藏/取消，与搜索弹窗收藏互通（同一 localStorage）；已收藏不重复出现在最近区；双空显示引导；侧栏折叠图标轨态不显示；不为侧栏引入高频轮询（免 spec） | P2 | 📋 计划 |
