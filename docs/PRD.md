...
| FR-352 | 接入指引补齐内嵌更新器信息 + 版本面板旁路摘要（feat，增强 FR-107/260）：`client-channels?tab=guide` 展示展示版本串、core 整数版本、wedge/core available/size 与下载；未内嵌明确失败态；频道版本列表/详情/发布页旁路展示「内嵌更新器 vX · core N」摘要；本批不含 commit/buildTime 元信息（需 spec）→ `docs/specs/client-updater-guide-embed-info/spec.md` | P1 | 📋 计划 |
