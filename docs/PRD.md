...
- FR-384~388（Agent 安全操控后台 2026-07-23：CP 侧 Agent Token 鉴权真源 + 写白名单 + scope + 硬拒绝 + `jm-agent` 独立 Go CLI + `mcp-bridge` 独立 Go 进程 + Token 管理 UI + 跨入口可证安全闸 + 文档/ADR-076/077；编号顺延至 384 起（373~383 已用于文件管理器批次）；执行方式=dev 分支登记 + FR-385/386/387 三 worktree 并行；计划见 `.tmp/plan-agent-control-plane-2026-07-23.md`）→ FR-384 `docs/specs/agent-token/spec.md`、FR-385 `docs/specs/jm-agent-cli/spec.md`、FR-386 `docs/specs/mcp-bridge/spec.md`、FR-387 免 spec（消费 FR-384 API，仅一节 UI 契约附在 FR-384 spec 末尾）、FR-388 `docs/specs/agent-safety-gate/spec.md`；新 ADR-076「agent-token-and-mcp-authority」与 ADR-077「mcp-bridge-as-agent-frontend」在对应 FR 落地时创建。**不做**：SSE/HTTP MCP 端口、mTLS 客户端证书、agent 内置审批流、业务高危写给 agent、GHCR、面板 Worker 装机重做
...
| FR-386 | mcp-bridge 独立进程（feat）：新 Go 二进制 `apps/mcp-bridge/`，stdio 默认（远端 IDE 经 HTTPS CP）；工具集仅暴露 CLI 等价的 action 子集；硬拒绝面永不对 MCP 暴露；CP 403 → MCP `isError=true` 含中文拒绝原因；不引入数据库/gRPC/Worker；不暴露独立网络监听（需 spec + 新 ADR-077，依赖 FR-384；建议并行 worktree）→ `docs/specs/mcp-bridge/spec.md` | P1 | 🔨 开发中 |
...
| FR-388 | Agent 入口可证安全闸（feat）：三入口同 Token 同输入行为契约表（CLI ↔ MCP ↔ curl）+ 真机复验矩阵（≥30 条断言：scope 外/Token 吊销/过期/写白名单外/硬拒绝面[用户写/删实例/DB 浏览/自更新/审计删除]/并发两入口同时写/速率限制）+ CI `release.yml` 增 agent-gate test 闸 + 契约入 `docs/API.md`（需 spec，依赖 FR-384/385/386 完成后起跑）→ `docs/specs/agent-safety-gate/spec.md` | P1 | 🔨 开发中 |
### 范围外（后续版本，暂不纳入 V1）
