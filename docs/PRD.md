...
| FR-385 | jm-agent 管理 CLI（feat，仿 jmctl）：独立 Go 二进制 `apps/jmagent/`，仅引入 HTTP 客户端与标准库，不链 gRPC/DB；子命令 MVP = `whoami/list nodes/list instances/instance status\|metrics\|logs/start\|stop\|restart/node maintenance enter\|leave`，写白名单仅含 `instance.life`+`node.maintenance`；非交互 `--output json` 给脚本/CI；403/硬拒绝文案稳定；Token 仅 env 与 `--token` 传入不落盘（需 spec + 新 ADR-076，依赖 FR-384；建议并行 worktree）→ `docs/specs/jm-agent-cli/spec.md` | P1 | 📋 计划 |
