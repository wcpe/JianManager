...
- **验收标准**:
  - [ ] 实例树每个实例显示 bot 聚合徽标（在线/总数），不展开为逐个 bot（数据来自 `GET /bots/summary`）
  - [ ] 工作区提供「终端 | Bot」切换；Bot 段顶部为状态概览卡片（总计/在线/连接中/异常）
  - [ ] Bot 段支持搜索、按状态/行为筛选与分组，列表分页加载，不一次性渲染全部
  - [ ] 可对当前筛选集或选中集批量设行为/停止/删除（调 `POST /bots/batch`）
  - [ ] 单 bot 行内可设行为/停止/重连/删除
  - [ ] 从该实例新建 bot 时，实例 id 预填、连接地址用「所在节点 host + 默认端口」预填且可改（FR-032 端口分配落地后可自动填实际 server-port）
...
- **验收标准**:
  - [ ] 导航「Bot」按钮位于「实例」与「告警」之间，点击进入全局 Bot 页
  - [ ] 页顶全局概览卡片（总计/在线/连接中/异常）+ 分布（X 实例·Y 节点）
  - [ ] 默认按实例分组的总览（每行：实例/节点/健康条/总数/批量），可切换分组维度（实例/节点/状态/行为）
  - [ ] 支持全局搜索与按节点/状态筛选；展开某实例才按行为/状态细分并分页
  - [ ] 每行可批量操作；每行「在控制台打开」跳到该实例的 Bot 段
  - [ ] 顶部提供「新建 Bot」「压测」入口（压测复用 FR-009 既有后端；完整编排 UI 见 FR-042）
...
- **验收标准**:
  ...
  - [ ] FHS 式子目录：`bin/`、`etc/`（平台配置）、`opt/jdks/<vendor>-<ver>/`（便携 JDK）、`var/servers/<slug>-<shortid>/`（服务器工作目录）、`var/log/`、`cache/cores/`（核心 jar 缓存）
  ...
  - [ ] 下载的 Paper/核心 jar 落入 `cache/cores/`，按「类型-MC版本-构建号」命名去重；建服优先命中缓存，缺失才下载（FR-034 复用）
  - [ ] 缓存的核心可登记为模板来源，与 FR-014 模板打通
...
- **备注**: 与 FR-032/033/034（均 in-progress，FR-033 约 80%、JDK 当前装在 `<serversDir>/jdks`）重叠；本 FR 只承接「自包含便携 + FHS 布局 + 核心缓存」这层底座，三者存储路径统一收口到本数据根，不重复其功能
