...
- **响应**: `200 { "message": "已发送" }`；表示 Worker 已接受并写入 `send-command` IPC，不代表 Bot Worker 已调用 `bot.chat`，也不证明服务器接受、权限通过或产生业务效果
- **错误**: 400 `INVALID_REQUEST`（缺 command）；404 `NOT_FOUND`（Bot 不存在/无权访问）；503 `COMMAND_FAILED`（Worker 未连接或 IPC 委托失败）。当前端点不等待 Bot Worker 执行回执，`bot.chat` 同步抛错不会回传到本次 HTTP 请求

### POST /api/v1/bots/stress-sessions
- **描述**: 创建持久化 Bot 压测会话；FR-352 在原端点加性支持 Scenario V2，**不新增独立场景 HTTP endpoint**。
- **关联 FR**: FR-352
- **历史来源**: FR-042 / FR-274（仅追溯原端点与旧编排，不表示当前所有权）
...
- **YAML 输入**: 同一 `scenario` 字段也可为 YAML 字符串，例如 `"version: 2\nseed: 20260719\ncohorts: ..."`；它不是新的 content-type 或新端点。旧请求仍可使用 `behavior` 或 `orchestrationYaml`。
  ...
  - `scenario`: 可选；可直接提交 Scenario V2 JSON 对象，也可提交包含 JSON 或 YAML 的字符串。Control Plane 将两种输入规范为同一节点树，严格拒绝顶层、cohort、动作及嵌套结构中的未知字段，再解码到同一 DTO 并执行同一 validator；JSON/YAML 的同类字段错误返回相同叶子 `path`。每个 cohort 必须恰有一个 `roam_in_area` 或 `attack_until` 持续动作标记 `observationStep:true`。`attack_until` 可加性使用 `selector.priority:"random"`、`searchArea`、`respawn:{maxAttempts,retryBackoffMs,timeoutMs}` 与 `stop.minClientAttackAttempts`；后者只核验 Mineflayer 客户端攻击调用次数，不证明服务端伤害或击杀。`legacy_behavior` 仅供 V1 内部转换，HTTP 新建 V2 不接受。`scenario` 与 `orchestrationYaml` 不可同时提交。
...
- **当前边界**：本端点当前没有 `commandSchedule` 请求字段，也没有通用命令计划解析、校验或执行能力；调用方不得提交或依赖该字段。未来设计仅见 `docs/specs/bot-load-probe-events/api.md`，实际落地后才回写本正式 API。
- **响应**: `201`
  ```json
  {
    "id": 1,
...
- **关联 FR**: FR-352
- **历史来源**: FR-042 / FR-274（仅追溯原列表能力，不表示当前所有权）
...
- **关联 FR**: FR-351 / FR-352
- **历史来源**: FR-042 / FR-274（仅追溯原详情与旧编排，不表示当前所有权）
...
- **描述**: 为尚未启动或可重试的压测会话执行预检；重新校验冻结的 ScenarioSnapshot 和执行节点容量，再生成跨 Worker 确定性分片、短期软预留和签名 `planToken`。预检不要求 ServerProbe，不验证连接配置、服务器接受、游戏服命令权限或业务效果；场景非法时在容量查询、计划持久化与 Worker 下发之前失败
- **关联 FR**: FR-351 / FR-352
...
- **响应** (200):
  ...
  ```
  - 容量不足或指定节点不可用时仍返回 200，`ready=false`、无 `planToken`，原因进入 `blockers[]`；容量 blocker code 为 `BOT_LOAD_CAPACITY_INSUFFICIENT`，节点 blocker code 为 `BOT_LOAD_NODE_UNAVAILABLE`
  - `probe` 是当前响应保留的兼容字段；通用预检固定按 `required=false` 解释，不执行 ServerProbe 连接校验，`connected=false` 或 ServerProbe 缺失均不形成 warning/blocker，也不改变 `ready`
  - 当前 service 内仍保留一个仅测试/历史调用可触达的 `Required=true` dormant 分支及 `BOT_LOAD_PROBE_REQUIRED` 常量，但公开 handler 从不传 true，故不属于公开 API；FR-359 接入新 preflight 前必须删除或私有隔离该分支，禁止新调用复活 Probe 硬阻断
...
- **关联 FR**: FR-351
- **历史来源**: FR-042 / FR-274（仅追溯原启动入口，不表示当前所有权）
...
- **关联 FR**: FR-351
- **历史来源**: FR-042 / FR-274（仅追溯原停止入口，不表示当前所有权）
...
- **CP → Worker → Bot Worker（FR-369）**：`ApplyBotCommandSchedules` 每批最多 100 项，下发 CP 已展开 occurrence plan（含 `durationMs/jitterMs/occurrences[commandId,occurrence,declarationIndex,baseAtMs,jitterOffsetMs,actionRunId,command]`）与 `start.mode=absolute|barrier`、`runDeadlineUnixMs`、`jitterSeed`、`skipOccurrences`；`ReleaseBotCommandSchedules` 以共同 `releaseAtUnixMs` 启动 prepared 命令计划；`CancelBotCommandSchedules` 下发 `unresolvedOccurrences` 用于无活动计划/tombstone 时的合成 cancelled。Worker 派生稳定 IPC requestId = `UUIDv5(requestId, "apply|release|cancel" + NUL + botUuid + NUL + generation + NUL + scheduleRunId)`，Bot Worker 集中 scheduler 每个 `scheduleRunId` 维护唯一 plan：occurrence 已发立即写入 sent，未开始 occurrence 在取消时立即写 cancelled，正在执行的同步 `bot.chat` 不强制中断。`command-schedule-result` 经 Worker 映射为既有 `BotActionEvent`（`sent→succeeded`、`failed/timed_out/cancelled` 原样透传错误码），最终仍只经 `StreamBotFleetEvents.action_event` 回传并经 ActionResultService 首终态胜出。
- **动作终态错误码**：`CONNECT_TIMEOUT`、`CONNECT_ENDED`、`PATHFINDER_UNAVAILABLE`、`PATH_NOT_FOUND`、`MOVE_TIMEOUT`、`TARGET_NOT_FOUND`、`ATTACK_ASSERTION_UNMET`、`ATTACK_ACTIVITY_UNMET`、`RESPAWN_LIMIT_EXCEEDED`、`PROBE_EVENT_TIMEOUT`、`BARRIER_TIMEOUT`、`ACTION_CANCELLED`、`ACTION_INTERNAL_ERROR`。其中 `ATTACK_ASSERTION_UNMET` 表示 `attack_until` 截止时可信伤害/击杀/事件或证据窗口断言未满足；`ATTACK_ACTIVITY_UNMET` 只表示本地攻击调用次数未达配置下限；二者均不得记为成功。
...
- **成功边界**：`send_command` 调用 `bot.chat` 且未同步抛错即为命令动作成功；调用前路由、IPC、参数处理失败或调用同步抛错仍为动作失败。ServerProbe/业务事件只可作为独立观测，不改变该结果。
- **真机边界**：FR-352 自动化完成不代表真 MC/pathfinder 或 500 Bot 真机通过；通用 `command_schedule` 与集中调度扩展由 FR-369 实施（已交付自动校验与 Bot Worker 集中 scheduler；500 Bot 真机由后续版本验证）。
...
| `LogRehydrate` | `LogRehydrateRequest` → `LogTaskResponse` | Deep Archive 回灌（FR-478） | Worker Registry/Rehydrate 已接线；CP 节点归档管理面可触发 |
...
实例注册消息 `CreateInstanceRequest` 另增可选字段 `log_target_id=19`、`log_acquire_mode=20`、`log_source_generation=21`，由 CP 统一创建/重连规格下发。Worker 将它们绑定到实例的受管日志采集面：Java 文件主源、通用进程 stdout/stderr 主源；缺字段保持旧 CP 兼容，不凭 UUID 猜测授权数字 ID。采集绑定失败时注册返回 `success=false`；已有实例 Resync 的绑定失败返回 `FailedPrecondition`。绑定持久化及无绑定实例禁止 cutover，见 FR-474 spec。

### 日志联邦查询端点（FR-479/480/482，**HTTP 已注册**）

- **`GET /api/v1/logs/federation`** — 前端门面（FR-482）：内部调用 Search，返回 `{ok,sourceTag,items,coverage,quality,view,notes,…}`；coverage/quality 为 FR-473 snake_case
- **`GET /api/v1/logs/federation/search|stats|facets`** / **`POST /api/v1/logs/federation/export`** — FR-480 精确端点；导出 incomplete 时不返回 NDJSON 附件
...
> 服务层真源：`internal/controlplane/logcoord` + `internal/worker/logs/query`。联邦侧经过 Worker-local View、Catalog、coverage 和权限校验；能力不可用时仍返回结构化 not-ready，禁止空成功。**CP 为浏览器唯一入口**；覆盖语义继承 FR-473。
...
- **工具目录**（全量注册，永久禁区不注册）：
  ...
  - Bot 舰队（FR-398，`bot.read`/`bot.manage`）：`bot_list`、`bot_get`、`bot_create`、`bot_set_behavior`、`bot_send_command`、`bot_delete`
