...
> updater-core 经 manifest `agent.core` 做集中版本管理：运营上传 core jar、按频道 pin / 更新 / 回退。**楔子（wedge）冻结、单版本、不纳入管理**。
> **两条版本轴**：manifest `version`（内容、防降级，ADR-022）与 `agent.core.version`（updater-core 自身、由频道 pin 驱动、对客户端单调**只升不降**）正交。
> **「回退」= 以更高 `agent.core.version` 重发旧 core 字节**（同 FR-088 内容回滚法），绝不降版（客户端 core 只升不降，否则回退无效）。core jar 与频道无关（一份 jar 三平台通用，ADR-021）：上传一份，manifest 生成时 fan-out 填 `windows`/`macos`/`linux` 三键。

#### POST /api/v1/client-core-versions/upload
- **描述**: 上传 updater-core jar 制品（入 FR-045 制品库 `type=client-core`，按制品自身 sha256 内容寻址去重）。返回的 `sha256` 即 manifest `agent.core.platforms[os].artifact.sha256`，随后用 `POST /client-core-versions` 登记版本
- **关联 FR**: FR-193 | **鉴权**: **JWT，平台管理员**（运营操作）
- **请求**: `multipart/form-data` — `file`（必）、`codec`（可，`zstd`|`none`）、`expectedSha256`（可）
- **响应** (201): `{ "sha256", "md5", "size", "codec" }`
- **错误**: 400 `INVALID_REQUEST`（缺 file）| 422 `CHECKSUM_MISMATCH`
- **审计**: `client_core.upload`

#### POST /api/v1/client-core-versions
- **描述**: 登记 core 版本（`version` 服务端**全局单调递增分配**，与频道无关）。制品须已由上传端点入 `client-core` 库
- **关联 FR**: FR-193 | **鉴权**: **JWT，平台管理员**
- **请求**: `{ "artifactSha256": "…", "artifactSize": 0, "codec": "zstd", "note": "可选" }`
- **响应** (201): `{ "id", "version", "artifactSha256", "artifactSize", "codec", "note", "sourceVersion", "createdBy", "createdAt" }`
- **错误**: 400 `INVALID_REQUEST` / `CORE_ARTIFACT_NOT_FOUND`（制品未上传）
- **审计**: `client_core.register`

#### GET /api/v1/client-core-versions
- **描述**: 列出全部已登记 core 版本（版本号 DESC）+ 冻结楔子版本（只读展示）
- **关联 FR**: FR-193 | **鉴权**: **JWT，平台管理员**
- **响应** (200): `{ "versions": [ { "version", "artifactSha256", "artifactSize", "codec", "note", "sourceVersion", "createdBy", "createdAt" } ], "wedge": { "version": "0.1.0", "frozen": true } }`

#### GET /api/v1/client-channels/:id/core-pin
- **描述**: 取频道 core pin 现状。`pinnedCoreVersion=0` 表示自动用最新；`effectiveVersion` 为解析后的有效版本（无任何已登记 core 时为 0）
- **关联 FR**: FR-193 | **鉴权**: **JWT，平台管理员**
- **响应** (200): `{ "channelId", "pinnedCoreVersion", "effectiveVersion" }` | **错误**: 404 `CHANNEL_NOT_FOUND`

#### PUT /api/v1/client-channels/:id/core-pin
- **描述**: 设 / 更新频道 core pin。`version=0` 恢复「自动用最新」；`>0` 须为已登记版本（即「更新」到该 core，客户端下次启动 promote）
- **关联 FR**: FR-193 | **鉴权**: **JWT，平台管理员**
- **请求**: `{ "version": 3 }`
- **响应** (200): `{ "channelId", "pinnedCoreVersion": 3 }`
- **错误**: 400 `INVALID_REQUEST` | 404 `CHANNEL_NOT_FOUND` / `CORE_VERSION_NOT_FOUND`
- **审计**: `client_core.pin`

#### POST /api/v1/client-channels/:id/core-rollback
- **描述**: 回退坏 core——取历史 core 版本 `sourceVersion` 的字节，**以更高版本号重发为新版并 pin**（不降 `agent.core.version`、不违反客户端防降级，ADR-045 决策 4）
- **关联 FR**: FR-193 | **鉴权**: **JWT，平台管理员**
- **请求**: `{ "sourceVersion": 1, "note": "可选，空则生成「回退至 core vN」" }`
- **响应** (201): `{ "channelId", "version": 3, "sourceVersion": 1, "note": "回退至 core v1" }`
- **错误**: 400 `INVALID_REQUEST` | 404 `CHANNEL_NOT_FOUND` / `CORE_VERSION_NOT_FOUND`
- **审计**: `client_core.rollback`
