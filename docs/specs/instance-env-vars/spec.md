...
> 状态：🔨 开发中（v0.18.0）· 类型：feat（增强 FR-034 搭建/结构化启动、ADR-008）
...
- [ ] 环境变量页签上区可增删改自定义启动 env，保存持久化
- [ ] 启动后工作目录出现 `.env` 文件、含所配 env（`KEY=VALUE`）；进程内该 env 生效（真机）
- [ ] 下区展示运行中 JVM 实际完整环境（含继承 PATH/JAVA_HOME）；停机/受限显提示
- [ ] Go build/test 绿、前端 tsc/lint/vitest 绿
- [ ] **真机验**：配一个自定义 env → 启动 → `.env` 存在且进程内生效 → 下区可见
