...
- [x] **Fix 5**: `register.go` Config 加 `GrpcPort`，`collectSystemInfo` 填入 `RegisterRequest`，worker `main.go` 传参
- [x] **Fix 1**: `handler.go` 加 `pool` 字段，`Register()` 中调 `pool.Connect()`，新节点生成 `Secret`。CP `main.go` 传 pool

### Phase 2: 进程管理器 I/O 修复

- [x] **Fix 3**: `manager.go` Instance 加 `Stdin io.WriteCloser` 字段，`Start()` 前创建管道存到实例，`SendCommand` 用存储管道
- [x] **Fix 4**: `manager.go` Manager 加 `onOutput` 回调 + `instanceWriter` 适配器，替换 `os.Stdout`/`os.Stderr`。Worker `main.go` 桥接 TerminalServer

### Phase 3: 实例创建 Worker 注册

- [x] **Fix 2**: `instance.go` 新增 `registerOnWorker()` 方法，Create 事务成功后调 `Worker.CreateInstance`
