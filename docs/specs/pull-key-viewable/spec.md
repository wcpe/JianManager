...
- **删「轮换」（2026-06-28 用户定）**：移除轮换按钮 + `POST .../keys/:id/rotate` 端点 + `useRotateClientKey`——轮换换 key 会让已分发客户端断更，与「密钥发出后永久使用」矛盾。
- **可编辑密钥值（2026-06-28 用户定）**：管理员可**手动设置/修改**密钥明文值——创建时可填自定义值（留空则自动生成）；既有密钥可改值。改值同步重算 `KeyHash`（鉴权）+ 重写 `KeyEnc`（可查看）。新增 `PUT .../keys/:keyId`（值/名称，管理员 + 审计），create 接受可选自定义值。运营据此自控这把永久 key。
- **保留「吊销」+ 强警告（2026-06-28 用户定）**：吊销保留，但加明确警告「吊销=使用此密钥的已分发客户端将无法再更新」+ `DangerConfirm` 二次确认。
- **迁移边界**：仅新建/改值过的密钥有 `KeyEnc` 可查看；**存量「只有哈希」的老密钥不可查**（哈希单向，救不回）——前端对无 `KeyEnc` 的密钥「查看」禁用 + 提示「此密钥创建于可查看功能之前、明文不可找回；可『改值』为已知值后即可查看（注意改值会使持旧值的客户端失效）」。
