...
本 FR 建独立运行详情页，用一条会话级聚合 SSE 驱动实时概览，Bot/失败/历史指标按需分页；用户可从 verdict 下钻到发压 Worker、Bot、命令步骤、屏障状态和错误，并重试失败子集或导出报告。ServerProbe 等业务观测只能作为可选扩展，不进入通用失败链必需条件。页面与报告必须持续说明：命令发送成功仅表示 Bot Worker 调用 `bot.chat` 时未同步抛错，不证明服务器接受、权限校验通过或产生预期业务效果。
