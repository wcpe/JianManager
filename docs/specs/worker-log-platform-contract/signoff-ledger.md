...
**根因**（已定位到代码）：[apps/worker/main.go:809](apps/worker/main.go#L809) 的输出处理器对**全部实例**无条件调用 `AppendInstanceOutput`，但日志绑定只对有 `workDir` 的实例注册（[instances.go:40](internal/worker/logs/ingest/instances.go#L40) 要求 `workDir` 非空）。涉事实例 `beacon-main`（`process_type=daemon`）无 workDir → 无绑定 → 每次输出都命中 [instances.go:140](internal/worker/logs/ingest/instances.go#L140) 的 ERROR 分支。

**影响面：仅日志噪音，功能无缺陷**。三条输出路径中，终端 WS 广播（`main.go:807`）与 CP 事件流（`main.go:808`）都在报错点**之前**分流，不受影响；受管 Raw 持久化是报错路径，但该实例本就无绑定、**无数据可丢**（VL 实测 `inst:153` 计数为 0，符合预期）。涉事实例仅 1 个（153 个 daemon 类型中唯一在输出者），且因该服务启动后即静默故错误已自然停止。

**性质**：属「正常行为的实例产生 ERROR 噪音、会误导运维判断」的缺陷，非数据面故障。**建议修法**：对无绑定的实例静默跳过（或降为 debug 级），使日志如实反映真实异常。此条**不在 FR-473~484 的验收范围内**（非任一 FR 的验收标准），登记为独立待办，**不影响本次签字**。
