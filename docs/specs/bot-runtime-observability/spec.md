...
> 状态：草拟　·　关联 PRD：FR-401　·　优先级：P1　·　依赖：FR-400　·　下游：FR-402
...
- [ ] 为 sampler 正常、缺失、陈旧、多节点隔离、Start/Stop 幂等写红测。
- [ ] 新增指标键与 30 秒 CP sampler，接入主程序生命周期。
- [ ] 实现关联查询、档位选择、共享运行时声明与权限收敛。
- [ ] 增加节点监控图卡、空态和中英文文案。
- [ ] 更新 API、ARCHITECTURE、PRD、CHANGELOG；回归 ADR-013 卷积与 TTL。
...
6. 服务、路由、前端、降采样回归测试通过；真 CP+Worker+Bot Worker 环境中创建/停止 Bot 后曲线变化、断隧道后曲线断开，需用户确认。
