...
```json
{
  ...
  "health": {"nodeCount": 3, "onlineNodeCount": 2, "staleNodeCount": 1, "offlineNodeCount": 0, "runningInstanceCount": 5, "crashedInstanceCount": 1, "stoppedInstanceCount": 2},
  ...
  "exceptions": [{"kind": "node_stale", "nodeId": 1, "title": "node-a 心跳陈旧", "href": "/monitoring?node=node-a-uuid"}]
}
```

响应只含受管资源与链接所需最小标识；不可用数值为 `null`，不以零替代。

`bots.eventLoopP95Ms` 为可用节点事件循环 P95 的最大值（最差节点），不是跨节点全局分位数；无可用节点时为 `null` 并由 `unavailable` 解释。`exceptions.href` 为既有页面的可直接下钻链接。
