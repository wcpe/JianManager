...
FR-351～354 提供分布式 Bot、确定性场景、可信探针事件和恢复能力，但尚缺少可复用模板、完整运行状态机、固定/阶梯/洪峰调度、时序指标、阈值判定、安全停止和正式报告。当前 BotStressSession 只有 pending/running/stopped/error 和查询时聚合，不能回答“最大稳定并发是多少、哪一层先过载、是否达到严格标准”。
...
- 每 5 秒聚合一次 Bot、动作、发压端和目标服指标，不逐 Bot 写时序。
- 阈值 evaluator 持续判断，区分 verdict 阈值和安全停止阈值。
- step 模式输出 maxStableBots；spike 模式输出连接/屏障/攻击峰值延迟。
- 失败分类必须区分 target/executor/network/scenario/probe/internal。
- 终态生成 JSON 报告和 CSV 导出。
...
- 内置塔防 preset 不作为 DB 内置不可删行；前端“使用预设”产生普通模板，避免迁移和本地化名称问题。
...
- 事件输入：Bot 状态、ActionResult、ProbeEvent、Worker capacity/heartbeat、目标指标、人工 intent、tick。
...
从 DB 增量/内存聚合读取：状态数、连接时间、ActionResult、ReconnectCount。避免每 5 秒扫描全 bots 表：维护 run 级聚合器，CP 重启时可从 DB 重建。
...
复用现有实例系统指标和 ServerProbe TPS/在线数；MSPT 使用 FR-353 接真的 `MSPTP95Millis`（ServerProbe 最近60秒原始 Tick p95），按目标 InstanceID 取最近值。禁止对 MSPT avg 样本二次取 p95。探针断开或老探针无 p95 时字段为 null，并触发 PROBE_METRICS_MISSING，不用 0/avg 代替。
...
- stable 预热：connected/stageTargetBots≥99%，且每个 cohort 进入 `observationStep:true` 的 Bot/该 cohort stageExpectedBotSet≥99%，两者连续60秒；ramp结束后10分钟内必须达到。step 每级差量派发后同样预热。
- 观察窗每5秒采样；online 取窗口内最小 `connected/stageTargetBots`，不是累计平均。
- room/arrival/attack 取 `可信 succeeded / 冻结 expectedBotSet`；clientAttackAttempts 不计攻击成功。
- TPS 取观察窗有效 1m TPS 样本最小值；MSPT 取 ServerProbe 直接报告的 60s Tick p95 最大值。
- 样本覆盖率≥99%，连续缺样>30秒失败；p95 连续缺失>60秒失败 PROBE_METRICS_MISSING。
- step 每级按同一公式；spike 额外计算连接/屏障/攻击开始延迟。
- percentile 纯函数只用于连接/动作延迟；不得对 MSPT avg 再算 p95。
...
- [ ] 新增固定 acceptance harness：`JM_BOT_LOAD_ACCEPTANCE=1 JM_BOT_LOAD_ENV=.tmp/bot-load-acceptance/environment.json go test -tags=botloadacceptance ./internal/e2e -run '^TestBotLoadAcceptance$' -count=1 -timeout=4h`；按超级规格 schema 探测10+ Worker/探针/塔防适配，输出 passed/failed/blocked 机器可读证据。
...
- [ ] 固定测试向量锁定：预热起点、720样本窗口、固定分母、expectedBotSet、最小在线率、最小TPS、最大Tick MSPT p95、缺样覆盖率和 crash 计数。
- [ ] 阈值缺样本为 Pending，连续缺样/安全窗口触发失败或 safety，单点毛刺不触发。
- [ ] p50/p95/p99 仅用于连接/动作延迟，固定向量结果准确。
- [ ] 报告能区分 target/executor/network/scenario/probe/internal。
...
- [ ] JSON/CSV 报告与真机观察、ServerProbe 指标一致。
