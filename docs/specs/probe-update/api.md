...
- **未部署/未连入优雅提示**：探针未连入时仍可推送（jar 落盘即生效于下次重启），响应里 `probeConnected=false`
  ...
  批量整体 422（无可推送内容）。
- **运行中实例的依赖缓存绕锁（FR-114 集成，见 CHANGELOG 修复）**：在线更新面向运行中实例，而随 jar 下发的
  运行库缓存 `libraries_zip` 会解压覆盖实例根 `libraries/`——运行中 JVM 独占锁定这些 classpath 依赖 jar，
  Windows 覆盖被锁文件必败（`Access is denied`）。Worker 解压时两级绕锁：① 目标已存在且大小+CRC32 与 zip
  条目一致即跳过覆盖（依赖版本极少变，绝大多数条目命中，不触碰被锁文件）；② 内容确有变化但目标被锁时降级为
  「跳过并告警」（旧版留存、下次重启后可再更新生效）。只要 jar+config 与其余条目写成功即视为部署成功，
  不因单个被锁依赖使整个更新请求失败——契合「推送即就位、下次重启生效」的语义。
