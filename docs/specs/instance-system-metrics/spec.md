...
> 2026-09-07 注（展示形态演进，语义不变）：实例控制台**顶栏**（FR-412 指标条演进为分段状态条）已将
> TPS/MSPT/在线的逐项「需探针」聚合为一枚「ServerProbe 未连接」芯片（悬停见不可用项），避免三个灰词
> 比数据还抢眼；**概览 KPI 卡仍逐项诚实标注**，本 spec 的「诚实标记、不以假值冒充」语义不变。
