...
- **描述**: 去掉 emoji 主题/语言按钮、纯文字退出，统一改 lucide 图标 + 文字（与导航项同款式）；主题三态可直选（非盲循环）；切语言同步 `<html lang>`；底部重排为「版本左下 · 开源许可入口右下」（开源许可页见 FR-135）
- **验收**:
  - [ ] 主题用 Sun/Moon/Monitor 图标 + 文字、可直选 light/dark/system；语言用图标 + 语言名
  - [ ] **退出登录**用 `LogOut` 图标 + 文字，与导航项一致（非纯文字）
...
- **描述**: 控制台外壳新增全局顶栏（位于内容区上方，侧栏保持全高）：左=面包屑/当前页标题，中=常驻搜索框，右=集群概览徽标 + 告警铃铛 + 用户/角色/账户菜单。本期搜索仅占位（UI + 聚焦快捷键），搜索逻辑留后续 FR
- **验收**:
  - [ ] 顶栏布局：左面包屑/标题、中常驻搜索框、右集群概览徽标 + 告警铃铛 + 账户菜单；搜索框本期占位（聚焦快捷键就绪、输入暂不联动检索）
  - [ ] 集群概览徽标（在线节点 / 运行实例 / 崩溃数）可点跳转对应筛选；告警铃铛接现有告警体系（只读最近 + 未读计数）；账户菜单显用户名 / 角色
