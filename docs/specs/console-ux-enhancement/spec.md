# 功能规格：控制台体验与可寻址性增强（FR-128~162）

> 状态：开发中　·　关联 PRD：FR-128~162 + BUG-013~023　·　分支：feature/console-ux-*
>
> 本 spec 承载该程序全部 FR 的详细描述与验收（PRD §4 仅留一行索引）。

## 1. 背景与目标

源于 2026-06-24 三轮前端真机走查（导航 / 实例操作 / 详情页 + 节点 / 群组 / 玩家 / Bot / 监控 / 告警 / 日志 / 备份 / 计划 / 模板 / 运行时 / 分发 / 更新 / 账户 / 设置 / 审计全量页）+ 7 个改造方向，共 131 个优化点。决策（sdd-brainstorming）：按主题聚合为 34 FR（FR-128~161）+ 11 BUG（BUG-013~023）+ 2 ADR（030/031），坏的走 BUG-###、大改造配新 ADR。**2026-06-26 增补 FR-162（全局页眉/顶栏），并扩 FR-132（含退出图标化 + 底部布局）、FR-135（照参考图丰富度）。** 计划全文 `.tmp/brainstorm-console-ux-2026-06-24.md`（不入库）。

- **横切约束**：每条 FR 验收含 **i18n(FR-016) 完整 + 暗 / 亮色(FR-026) 正常 + 真机验证**。
- **波次**：**Wave0** 地基 / 基件先行减返工（ADR-030/031 + FR-160/159）；**Wave1** 架构主线（FR-128 解锁可寻址 → FR-130 → FR-129）；**Wave2** 页面增强大并行（FR-131~158）；BUG / 归真随时并行（sdd-fix-bug）。

## 2. 需求与验收（逐 FR）

### 地基 · 可寻址性与工作区（配 ADR-030/031）

#### FR-128: 导航与视图状态可寻址化 + 滚动位置恢复
- **优先级**: P1 | **依赖**: 无（地基起点） | **关联 ADR**: ADR-031 | **来源**: 走查 #109,110,111,112,113,43
- **描述**: 修订「打开实例存 store、不进 URL」取舍，让导航/视图状态可寻址，支持「返回上一个位置」与鼠标侧键前进/后退（ADR-031）
- **验收**:
  - [x] 先写 **ADR-031**；实例列表/卡片/侧栏树入口直接跳 `/instances/:id`，`openInstanceId` 兼容桥已移除
  - [~] 列表筛选/分组/排序/pageSize 与详情激活 Tab 已进 URL；群组/节点下钻仍待全量梳理
  - [~] 实例列表虚拟滚动容器已接入滚动位置恢复；**真机** 鼠标侧键截图验收待补

#### FR-129: 实例工作区分屏面板化
- **优先级**: P2 | **依赖**: FR-130 | **关联 ADR**: ADR-030 | **来源**: 走查 #53,42,61,59,58
- **描述**: 兑现 FR-037「分屏/导播台禁用占位」，工作区从固定单 Tab 演进为可拖拽分屏的面板（终端/资源/插件/监控/Bot），状态外提为 panel 树（ADR-030）
- **验收**:
  - [ ] 先写 **ADR-030**；面板可水平/垂直拆分、比例可拖拽，可同时打开多个面板；多文件以面板内 Tab 承载
  - [ ] 非激活/未打开面板惰性挂载（不再六 Tab 全预渲染建 WS/discover）；资源面板去固定 h-600 随容器自适应
  - [ ] 各面板自管 dirty/未保存草稿；**真机**：左终端右配置同屏协同

#### FR-130: 文件与配置合并为统一资源面板
- **优先级**: P2 | **依赖**: 无 | **关联 ADR**: ADR-030 | **来源**: 走查 #52,58
- **描述**: 删除割裂的「文件」「配置」双 Tab，合为一个资源面板（ConfigExplorer 已是 ResourceExplorer 薄封装，合并近零成本）
- **验收**:
  - [ ] 「文件」「配置」合为一个标签/面板；配置发现/收藏作为该面板可选视图
  - [ ] 既有文件浏览/编辑/上传下载/版本回滚能力不丢；**真机**：单面板内完成原两 Tab 全部操作

### 侧栏 / 导航

#### FR-131: 侧边栏可折叠图标轨 + 隐藏滚动条 + 布局持久化
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #51,10,13,54
- **描述**: 侧栏可折叠为仅图标轨（hover tooltip 显 label）、导航区滚动条隐藏（保留滚动）、折叠态/选中节点持久化
- **验收**:
  - [ ] 折叠开关：展开 w-60 / 折叠仅图标 + tooltip；导航滚动条隐藏不占位
  - [ ] 折叠态、选中节点、导航分组折叠态持久化 localStorage，刷新不重置

#### FR-132: 侧栏底部控件图标化（主题/语言/退出）+ 三态直选 + 底部布局
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #56,57,16,17 + 2026-06-26 需求（退出图标化 + 底部布局）
- **描述**: 去掉 emoji 主题/语言按钮，主题/语言统一改 lucide 图标 + 文字（与导航项同款式）；主题三态可直选（非盲循环）；切语言同步 `<html lang>`；**退出登录迁至全局顶栏账户菜单（FR-162），底部不再保留退出按钮**；底部重排为「版本左下 · 开源许可入口右下」（开源许可页见 FR-135）
- **验收**:
  - [ ] 主题用 Sun/Moon/Monitor 图标 + 文字、可直选 light/dark/system；语言用图标 + 语言名
  - [ ] **退出登录**移交 FR-162 顶栏账户菜单承载（本 FR 底部不再放退出按钮）
  - [ ] 切语言同步 document.documentElement.lang
  - [ ] 底部布局：版本号居左下、「开源许可」入口居右下（FR-135 提供目标页）

#### FR-133: 实例树搜索/虚拟化/折叠保留/激活态/空态/a11y
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #8,9,11,12,14,15
- **描述**: 侧栏实例树加名称搜索、长列表虚拟化、折叠「实例」组时树不消失、激活态正确、空态 CTA、分组维度 a11y
- **验收**:
  - [~] 实例分组树已补名称搜索、匹配上下文展示、虚拟渲染与键盘 a11y；侧栏常驻实例树不承载 1000+ 的边界见实例列表重设计 spec
  - [x] 分组树已补 role=tree/treeitem、aria-selected/expanded、Enter/Space、空态 CTA 与侧栏服务器分组激活态

#### FR-134: 统一页头与面包屑组件
- **优先级**: P3 | **依赖**: 无 | **来源**: 走查 #18,131
- **描述**: 各页统一页头（标题/副标题/字号一致 + 可点面包屑），消除三页标题字号不一与二级页无「当前在哪」
- **验收**:
  - [ ] 统一页头组件被各页复用，标题字号一致；二级页有可点面包屑

### 全局外壳 / 页眉

#### FR-162: 全局页眉/顶栏（基础信息 + 搜索占位）
- **优先级**: P2 | **依赖**: 无（布局改 `DashboardPage`；面包屑与 FR-134 协同） | **来源**: 2026-06-26 需求（补充页眉区域 + 基础信息 + 搜索功能）
- **描述**: 控制台外壳新增全局顶栏（位于内容区上方，侧栏保持全高）：左=面包屑/当前页标题，中=常驻搜索框，右=集群概览徽标 + 告警铃铛 + 用户/角色/账户菜单（含退出登录，接管 FR-132）。本期搜索仅占位（UI + 聚焦快捷键），搜索逻辑留后续 FR
- **验收**:
  - [ ] 顶栏布局：左面包屑/标题、中常驻搜索框、右集群概览徽标 + 告警铃铛 + 账户菜单；搜索框本期占位（聚焦快捷键就绪、输入暂不联动检索）
  - [ ] 集群概览徽标（在线节点 / 运行实例 / 崩溃数）可点跳转对应筛选（运行/崩溃→实例列表按状态筛选，在线→节点页）；告警铃铛接现有告警体系（只读最近 + 未读计数）；账户菜单显用户名 / 角色 + **退出登录（LogOut 图标 + 文字，接管 FR-132）**
  - [ ] 面包屑 / 标题与 FR-134 协同（本期自带轻量标题，不展开 FR-134 的 P3 全量）；响应式不挤垮工作区
  - [ ] i18n 完整 + 暗 / 亮色正常 + 真机

### 开源合规

#### FR-135: 开源许可与依赖清单页（独立页，照参考图布局）
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #55 + 2026-06-26 需求（照参考图丰富度 + 全覆盖三源）
- **描述**: 侧栏底部版本旁加「开源许可」入口（FR-132 右下），新建独立页 `/licenses` 聚合 web(npm)+bot-worker(npm)+Go(go.mod) 直接与传递依赖的名称/版本/许可证/作者/链接
- **验收**:
  - [ ] 版本右侧「开源许可」链接 → 独立页 `/licenses`（带返回）；依赖与许可证在构建期扫描生成（不手维护）
  - [ ] **照参考图布局**：顶部搜索框（按包名过滤）+ 按类型分区「运行时依赖(N)/开发依赖(N)」计数 + 表格列[包名·版本·许可证·作者] + 行内可展开查看许可证全文
  - [ ] 三处依赖来源全覆盖（web npm + bot-worker npm + Go go.mod）；i18n + 暗/亮色正常

### 实例列表 / 操作

#### FR-136: 实例列表汇总头 + 节点/端口列 + 角色徽标 + proxy↔backend inline
- **优先级**: P2 | **依赖**: 无（受益于 FR-128） | **来源**: 走查 #126,127,128,130
- **描述**: 列表加顶部汇总头（运行/停止/崩溃计数，可点=设筛选）、节点·端口列、角色统一徽标、proxy 行 inline 展开已注册 backend
- **验收**:
  - [ ] sticky 汇总 chip（运行 N/停止 N/崩溃 M/总数）可点设状态筛选
  - [ ] 表加「节点:端口」列（serverPort 已有数据）；角色三态统一语义色徽标；proxy 行可 inline 展开 backend 摘要

#### FR-137: 实例列表搜索/排序 + 筛选吸顶可折叠 + 分组单表
- **优先级**: P2 | **依赖**: 无（受益于 FR-128 筛选进 URL） | **来源**: 走查 #25,114,115,129
- **描述**: 加名称搜索与列排序、筛选区吸顶可折叠、分组视图改单表 sticky 组头（不再每组重复整套表头）
- **验收**:
  - [x] 名称模糊搜索与排序字段（名称/状态/创建时间/节点）已接后端分页并写 URL；列表表头名称/节点/状态可点排；筛选工具条已 sticky 可折叠
  - [x] 卡片/平铺列表已虚拟渲染；分组视图已改为单个虚拟表 + sticky 组头，不再每组重复表头

#### FR-138: 单实例操作可发现性与反馈
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #19,26,27,29,31,33,35
- **描述**: 次要操作收 kebab、per-row loading/disabled、乐观更新、过渡态动效、运行态禁用+tooltip、详情页操作对齐列表、按钮 aria
- **验收**:
  - [ ] 行内仅留启停/重启主操作，余项收「⋯」菜单（删除标红）；操作中行按钮 loading+disabled 防连点
  - [ ] 运行态克隆/删除改禁用+tooltip（非消失）；详情页头部补齐与列表一致操作；过渡态徽章脉冲

#### FR-139: 批量操作增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #21,22,23,24,28,32
- **描述**: 批量部分失败展开明细+保留失败选择、按选中集状态感知禁用、命令下发二次确认、选择范围（可见/不可见）提示、停止防回车直提
- **验收**:
  - [ ] 部分失败列出失败实例名+原因并保留其选择供重试；无意义动作按状态分布禁用+tooltip
  - [ ] 批量命令下发前复述「将向 N 个实例下发：<命令>」确认；批量停止/强杀需明确确认不被回车直提

### 实例详情面板

#### FR-140: 终端体验增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #39,40,41,45
- **描述**: 终端加重连按钮、全屏、字号调节、搜索（不新增 xterm addon 的轻量搜索）、读写态指示、右键菜单 a11y + i18n
- **验收**:
  - [x] 重试耗尽/断开显示「重新连接」按钮；全屏 + 字号 +/− 已落地；Ctrl/Cmd+F 搜索框支持 buffer 匹配计数、当前定位、上一条/下一条切换与可见区轻量高亮
  - [x] 常驻可写/只读徽标；右键菜单语义化（role/Esc/方向键）+ 文案 i18n

#### FR-141: 资源管理器与配置编辑器增强
- **优先级**: P2 | **依赖**: 无（与 FR-129/130 协同） | **来源**: 走查 #49,60,62,63,64,65,66,67,68
- **描述**: 暗色 CodeMirror、大文件/二进制防护、文件树虚拟化+键盘导航、搜索高亮+范围限定、串行批量进度、配置表单字段分组校验、文本↔表单切换保护、diff 着色
- **验收**:
  - [x] CodeMirror 随暗色模式切主题；超大/二进制文件拦截为下载降级；文件树已补 role=tree 键盘导航与虚拟化
  - [x] 搜索片段高亮 + 目录/类型范围已落地（HTTP `rootPath`/`extensions` + CP/mock 过滤）
  - [x] 版本 diff 增删着色已落地（文件历史版本抽屉按 unified diff 语义行渲染）
  - [x] 粘贴/移动串行批量给进度+部分失败汇报已落地（失败项保留可重试）
  - [x] 配置表单字段分组+即时校验已落地（schema group 元数据、前端分组渲染、整数/布尔/枚举即时拦截）

#### FR-142: 详情页监控与探针增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #38,47,48
- **描述**: 详情页引入实例历史曲线（复用 TimeSeriesChart）、探针富指标阈值着色、探针未安装可操作指引、指标卡停机折叠
- **验收**:
  - [x] 监控面板展示 TPS/内存/玩家历史曲线（带 RangePicker）；当前 TPS/MSPT/CPU 阈值绿/黄/红与历史曲线阈值标记已落地
  - [x] 探针未安装提示带安装指引/入口；停机时指标卡折叠不占大块空白

#### FR-143: 插件/模组/资源包/数据包管理增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #50,69,70
- **描述**: 按类型分区管理（插件/模组/资源包/数据包）、jar/zip 元信息解析、拖拽+进度上传、同名覆盖确认、运行态需重启提示、市场入口预留
- **验收**:
  - [x] 区分 plugins/mods/datapacks/resourcepacks 分区与后缀校验；真实四目录后端校验、单服覆盖语义、基础元信息解析与展示版本/作者/依赖已落地
  - [x] 启禁/删除提示「重启后生效」已落地；拖拽上传 + 进度、同名覆盖二次确认与市场入口预留已落地

### 节点

#### FR-144: 节点页直观化
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #118,119,72,120,73,74
- **描述**: 节点页加集群汇总头、主表精简列（次要信息进详情）、操作收 kebab、详情展开分段+可见才轮询
- **验收**:
  - [ ] 顶部 sticky 集群概览（在线/离线/维护 + 集群 CPU/内存/磁盘聚合水位）
  - [ ] 主表保留名称/状态/CPU/内存/磁盘/操作（网络/系统/IP 进详情）；操作收 kebab（排空/下线标危险）
  - [ ] 详情区分段（概览/趋势/实例对比）；展开/不可见时不起多余轮询

### 群组（网络）

#### FR-145: 群组管理可寻址双栏 + proxy↔backend 拓扑
- **优先级**: P2 | **依赖**: 无（受益于 FR-128） | **来源**: 走查 #121,122,123,124,125
- **描述**: 群组详情从滚动模态改为可寻址双栏（左成员/右候选）、加 proxy↔backend 拓扑视图、列表成员健康分布、候选补节点/状态/端口、成员状态用 StatusBadge
- **验收**:
  - [ ] 群组详情可深链（路由/searchParams）；管理改左右双栏消嵌套滚动
  - [ ] proxy↔backend 注册关系拓扑/缩进可视；列表行显示成员状态分布（运行/崩溃）；候选项含节点·状态·端口可筛

### 玩家

#### FR-146: 玩家管理增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #75,76,77
- **描述**: 在线玩家跨服全局封禁 + 多选批量踢/封、实时事件流暂停+类型过滤+清空、操作按钮统一语义色
- **验收**:
  - [x] 行勾选批量踢/封 + 跨服全局封禁（同名多服一键）；按子服分组/筛选
  - [x] 事件流暂停滚动开关 + 类型过滤 + 清空已落地；踢/封按钮用 destructive 语义色

### Bot

#### FR-147: Bot 规模化管理增强
- **优先级**: P2 | **依赖**: FR-040（Bot 全局管理页，已交付） | **来源**: 走查 #78,79,80
- **描述**: Bot 健康条多段着色（兑现 FR-040 备注「细分 connecting/error」）、多组批量删除二次确认+进度、行为参数化入口
- **验收**:
  - [ ] 健康条按 connected/connecting/error/stopped 多段着色（数据来自 summary byStatus）
  - [ ] 多组批量删除前 DangerConfirm + 串行进度提示；行为下拉旁「配置」入口暴露巡逻路径/跟随目标等参数

### 监控 / 告警 / 日志

#### FR-148: 趋势图增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #84,85
- **描述**: TimeSeriesChart 加多序列图例、Y 轴自适应 domain（非占比指标）、空态 i18n
- **验收**:
  - [ ] 多序列常驻图例区分；TPS/MSPT 等窄幅指标 Y 轴 `domain=['auto','auto']` 显波动；空态走 i18n

#### FR-149: 告警增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #81,82,86
- **描述**: 规则行内启停 Switch（补 enabled 字段）、测试通道结果反馈、事件时间范围+规则+关键字筛选+分页、静默跨夜可视+时区标注
- **验收**:
  - [ ] 规则表 Switch 直接 toggle enabled（编辑弹窗补 enabled 字段）；测试通道弹「✓已送达/✗错误详情」
  - [ ] 事件加时间范围+规则+关键字筛选与分页；静默时段跨夜可视化 + 时区标注

#### FR-150: 日志中心增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #87
- **描述**: 日志加实时跟随（tail）、时间范围筛选、虚拟滚动、级别本地化+统一配色、导出范围/格式
- **验收**:
  - [ ] 「实时跟随」开关 + 时间范围；长列表虚拟滚动；级别 Badge i18n + 与告警页统一配色
  - [ ] 导出可选范围（当前页/全部匹配/时间段）与格式

### 备份 / 存储 / 计划 / 模板

#### FR-151: 备份页增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #89,93,94
- **描述**: 进行中状态自动刷新进度、总占用+份数汇总、校验和展示、增量备份链父子可视+删除依赖警告
- **验收**:
  - [ ] 存在「进行中」备份时条件轮询进度直至完成；顶部汇总（总占用/份数/最近成功）
  - [ ] 展示校验和；增量行显示父备份，删父备份警告「N 个增量依赖」

#### FR-152: 备份存储测试连接 + 容量展示
- **优先级**: P2 | **依赖**: FR-057（远程存储后端） | **来源**: 走查 #90
- **描述**: 备份存储后端列表展示每个远程后端的已完成备份份数与已用空间，并提供行内「测试连接」按钮；测试由在线 Worker 通过新增 `TestStorageBackend` gRPC 对 S3/SFTP/WebDAV 做小对象上传→下载校验→删除探测，CP 不落明文凭证、不直接访问远程后端。
- **契约**:
  - `GET /api/v1/backup-storages` 返回项新增只读聚合字段 `backupCount` / `usedBytes`；远程后端统计来自 CP 已落库 `backups.storage_id` + `status=completed` + `file_size_mb` 聚合。
  - `GET /api/v1/backup-storages/:id/stats` 返回单个远程后端 `{backupCount, usedBytes}`；`GET /api/v1/backup-storages/local/stats` 返回本地备份统计，份数来自本地备份记录，容量优先扫描 CP 数据根 `var/backups` 实际文件大小。
  - `POST /api/v1/backup-storages/:id/test` 返回 `{ok,message,errorCode?,latencyMs,nodeUuid?}`；无在线 Worker、凭证环境变量缺失、Worker 探测失败均返回 200 + `ok=false` 供前端即时展示失败原因。
- **验收**:
  - [x] 表单与行各有「测试连接」即时反馈成功/失败原因；列表显示每后端备份数/已用空间

#### FR-153: 计划任务增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #95 + #91 可视化部分（校验 bug 见 BUG-020）
- **描述**: cron 人类可读翻译 + 下次执行预览 + 常用预设、时区标注、编辑 command 回填保护
- **验收**:
  - [ ] 合法表达式显示可读描述 + 接下来 3-5 次执行时间；常用预设快捷按钮；cron 与「上/下次执行」标注时区
  - [ ] 编辑 command 任务时回填或显式提示「留空将清除原命令」

#### FR-154: 模板应用到实例 + 变量填充预览
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #92
- **描述**: 模板页加「用此模板创建实例」入口 + 变量填充预览、startCommand 复制、版本/更新时间
- **验收**:
  - [ ] 模板卡片「用此模板创建」跳转创建流程并预填；含占位变量时填充预览
  - [ ] startCommand 一键复制 + 展开查看；显示版本/更新时间

### 平台 / 资产 / 更新 / 数据库

#### FR-155: 平台资产与更新页增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #96,97,98,99
- **描述**: 制品/JDK 下载进度、被引用制品禁删前移、拉取密钥过期预警、系统更新前置检查+回滚+金丝雀、数据库敏感列防护
- **验收**:
  - [~] refCount>0 时删除按钮禁用+tooltip 已落地；拉取密钥按 expiresAt 显示「已过期 / 即将过期」预警已落地（7 天内即将过期）；制品/JDK 导入下载进度待补
  - [~] 系统更新展示 changelog/回滚说明、CP 自更新强确认、全网升级金丝雀分批待补；数据库敏感列禁排序/过滤+标注已落地（后端/mock/前端三层保护）

### 账户 / 设置 / 认证

#### FR-156: 用户与组管理能力补齐
- **优先级**: P1 | **依赖**: 无（归真 FR-003） | **来源**: 走查 #100,101
- **描述**: 补齐缺失的管理能力——用户改密/启停/编辑；用户组编辑/删除/配额修改/成员增删 UI 接入（兑现 FR-003 归真）
- **验收**:
  - [ ] 用户行可改密/启停/编辑；组可编辑/删除（接通已实现的 useDeleteGroup）/改配额/管成员
  - [ ] 创建用户角色选项带权限差异说明；**真机**：改密后新密码可登录、停用账户被拒登录

#### FR-157: 认证体验增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #104
- **描述**: 登录密码显隐切换、Setup 密码强度提示+实时规则、字段级错误聚焦
- **验收**:
  - [ ] 密码框 eye 显隐；Setup 实时强度条 + 规则 checklist；密码不一致错误绑定到 confirm 字段并聚焦

#### FR-158: 设置与审计页增强
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #105 + 审计
- **描述**: 设置切分类未保存拦截 + 生效时机说明 + 安全项隔离；审计导出 + 行详情展开 + 真实总数分页
- **验收**:
  - [ ] 设置切分类有未保存草稿时拦截确认；非即时项 tooltip 说明生效时机；security 项视觉隔离
  - [ ] 审计支持导出 + 行展开看变更详情 + 显示真实命中总数与分页

### 一致性 / 响应式（ref + 基线）

#### FR-159: 共享对话框统一
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #102,107,103,88
- **描述**: 裸 div 模态框（CreateUser/CreateGroup/Provision* 及节点/玩家/告警弹窗）统一迁 Radix Dialog（role/aria-modal/焦点陷阱/Esc/autofocus）；一次性 secret 改持久可复制展示
- **验收**:
  - [~] 玩家踢/封确认、资源编辑器未保存确认、更新说明外链确认、搭建向导（代理/后端子服）、实例复制、群组网络、代理后端注册、告警规则/通道弹窗、实例标签/配置/限额、用户创建/编辑与用户组创建/编辑/成员管理已走 shadcn/Radix Dialog；剩余页面继续收敛
  - [x] forwarding secret 等关键凭据用带复制的持久面板展示（非 15s toast）

#### FR-160: 共享基件统一
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #108,77,2,125 | **备注**: ref 重构，行为不变，走 sdd-refactor-code
- **描述**: 表格/按钮/危险确认/状态徽标/语义色收敛到单一来源（消除原生 table、原生 button、原生 confirm()、裸状态文本三套并存）
- **验收**:
  - [~] 列表/按钮/危险确认已首批收敛，原生 `window.confirm()` 调用已移除；仍有页面级原生按钮待继续替换
  - [~] 统一控制台状态徽标已切共享 StatusBadge，日志表头已 i18n；其他页面裸枚举与硬编码语义色待继续收敛

#### FR-161: 全局响应式与防翻屏基线
- **优先级**: P2 | **依赖**: 无 | **来源**: 走查 #7,116,117,114
- **描述**: 控制台外壳与各页响应式（移动断面 + 抽屉式侧栏）、列表分页/虚拟滚动、页头工具栏吸顶、模态去嵌套滚动
- **验收**:
  - [x] 窄屏侧栏抽屉与主实例页工具条折叠已落地；关键页面桌面/移动端防横向翻屏 benchmark 已补
  - [~] 实例列表/卡片与实例分组树已虚拟渲染，页头/工具条 sticky 首批落地；复杂模态/双栏替代仍待逐页收敛

## 3. 缺陷（BUG-013~023，走 sdd-fix-bug，修复后入 CHANGELOG）

- **BUG-013**: 实例详情「备份」标签永久「加载中」空壳——`BackupsTab` 硬编码 loading 从不调 `useBackups`，备份/恢复/下载/删除全不可用（走查 #1）
- **BUG-014**: 实例详情头部状态显原始英文 `CRASHED`，与列表本地化「崩溃」不一致（走查 #2）
- **BUG-015**: i18n 泄漏——英文模式图表硬编码中文「暂无数据」+ 单实例操作 toast 中文硬编码（走查 #3,30）
- **BUG-016**: 主题首帧 FOUC——`useThemeStore` 初值固定 system 不读 localStorage，保存为 dark 者首屏闪浅色（走查 #5）
- **BUG-017**: 离线实例详情仍重复触发 422——未在节点/实例离线时短路 token/discover 请求（走查 #6）
- **BUG-018**: 配置/文件编辑器切换/关闭丢弃未保存草稿、无拦截，造成数据丢失（走查 #36,37）
- **BUG-019**: 节点列表离线节点显示 0% 资源条而非「无数据」，误导为在线空载（走查 #71）
- **BUG-020**: 计划任务 cron 非法表达式（如 `,,,`）通过前端 `FIELD_RE` 校验（走查 #91 校验部分）
- **BUG-021**: 告警规则编辑弹窗缺 enabled 字段，前端无法切换规则启用态（走查 #81 部分）
- **BUG-022**: 创建用户密码下限(6)与 Setup 初始化(8)不一致（走查 #106）
- **BUG-023**: 仪表盘负载环判级**待复核**——走查 #83 据代码推断「倍数被当百分比致判级失效」，但 FR-108 在 v0.9.1 走查已验证「分级配色本就生效、全红是机器确实满载」，二者冲突，需先真机复核再定是否为缺陷（不擅自归真 FR-108）

## 4. 归真（父 FR 标 done 实则缺，退「开发中」+ sdd-fix-bug）

- **FR-003**（用户组与配额）：验收「创建/编辑/删除用户组」标 done，但 `GroupsPage` 当前只读、`useDeleteGroup` 未接入 UI → 由 FR-156 兑现，状态退回开发中（走查 #101）
- **FR-059**（危险操作保护体系化）：验收「删除/强制关服…均接入」，但单实例「强杀」零二次确认（与批量要求 FORCE 自相矛盾）→ 由 FR-139/138 兑现，状态退回开发中（走查 #20）
