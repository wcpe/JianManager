...
```json
{
  ...
  "nodes": [{
    ...
    "botWorker": {"rssBytes": null, "cpuPct": null, "activeCount": null, "connectingCount": null, "eventLoopP95Ms": null, "available": false, "reason": "未启动"}
  }],
...
所有不可用数值为 `null`；`freshness/status` 取 `fresh|stale|offline|unavailable`。响应不含未受管进程、原始命令行、密钥或 Bot 配置。
