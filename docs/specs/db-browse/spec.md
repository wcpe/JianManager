...
- 支持点击列头排序：排序列必须存在于当前表列白名单内，且不得是敏感列。
- 支持简单过滤：过滤列必须存在于当前表列白名单内且不得是敏感列，过滤值作为参数绑定，不拼接 SQL。
...
- `TableRows(table, params)`：
  ...
  - 排序列与过滤列必须命中列白名单，且不得命中敏感列；非法列静默忽略，回退默认查询。
...
- `DatabaseExplorer`：
  ...
  - 点击非敏感列列头在未排序、升序、降序、取消排序之间切换；敏感列列头禁用排序并标注「敏感列」。
  - 过滤条包含过滤列选择、过滤值输入、应用与清除；过滤列下拉不提供敏感列。
...
- [x] 补充 DatabasePage DOM 测试：普通成员拒绝、不泄露表数据、过滤/排序交互、敏感列不可排序/过滤且列头标注。
...
- 排序列和过滤列必须通过列白名单，且不得是敏感列；非法列不得造成 SQL 注入或 500。
- 未知表返回 404 `TABLE_NOT_FOUND`。
- 敏感列在服务端响应中已脱敏；前端也不会展示敏感列原值，且敏感列列头标注「敏感列」并禁用排序/过滤入口。
- 页面无任何写操作入口。
- 自动化验证：
