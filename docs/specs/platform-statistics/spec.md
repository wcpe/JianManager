...
> 状态：开发中　·　关联 PRD：FR-220　·　分支：feat/fr-220-platform-stats
...
- [ ] 写本 spec（平台级维度 + 数据来源缺口论证 + 端点契约 + gate-api 自检）
- [ ] 新增前端 hook `useClientDistObservability`（消费既有 `/client-dist/observability`，定义 TS 类型）
- [ ] 抽 `lib/platform-stats.ts` 纯函数（分桶/占比）+ 单测
- [ ] 改造 `StatisticsPage.tsx`：KPI 行 + 构成分布区 + 分发概览区（管理员降级）+ 局部空/错误态
- [ ] i18n zh/en 新增本页维度键
- [ ] mock `observ.ts` 新增 `/client-dist/observability` handler
- [ ] vitest：纯函数 + `StatisticsPage.dom.test.tsx`（含 CRASHED 分布、403 降级、500 不崩）
- [ ] 文档同步：PRD FR-220 状态 → 🔨 开发中（done 由用户确认）、ARCHITECTURE §8 统计子项描述更新为「平台级聚合（FR-220 已补）」、本 spec 任务勾选；**API.md 无需改**（不动端点）
- [ ] 真机验收（需用户确认）
