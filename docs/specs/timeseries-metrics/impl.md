...
> 关联 FR: FR-060 | 优先级: P1 | 状态: 🔨 in-progress（后端 + 前端图表均完成、真机验收通过；待用户确认标 done） | 关联 ADR: ADR-013（分级降采样）；ADR-014（ServerProbe 实例指标源）
...
### 前端（随 FR-061 实施完成）
- [x] `web/src/api/metrics.ts`：`useMetricSeries`、`useMetricOverview`。
- [x] `OverviewPage`（环形仪表盘 + 聚合曲线）、节点详情（CPU/内存/磁盘/网络曲线 + 各实例 TPS/MSPT/堆/线程对比）、实例「监控」段（TPS/MSPT/堆 used·max/线程/CPU + 分世界 区块/实体/方块实体）。
- [x] `web/src/i18n/{zh,en}.json`：`metrics.*`、`dashboard.*` 键。
...
- [x] `cd web && npx tsc --noEmit && npm run lint && npm run build`（0/0 通过）+ `npx vitest run`（52 过）
- [x] 真机：真 Paper + 探针，CP 连续运行历史曲线累积、CP 重启后历史不丢、5m 降采样卷积均验证；48h/30d/≥1y 过期清理由单测覆盖（长跑未做）
