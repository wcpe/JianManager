...
export type MetricRange = '1h' | '6h' | '24h' | '7d' | '30d' | '90d' | '1y'
export type MetricResolution = 'auto' | 'raw' | '5m' | '1h'

export const METRIC_RANGES: MetricRange[] = ['1h', '6h', '24h', '7d', '30d', '90d', '1y']
...
export function RangePicker({
  ...
  return (
    <div className={cn('jm-toolbar-surface inline-flex items-center gap-1 p-1', className)} role="tablist">
      {METRIC_RANGES.map((r) => (
        <Button
          key={r}
          type="button"
          ...
          className={cn(
            'h-7 px-2.5',
            value === r ? 'shadow-soft' : 'text-muted-foreground hover:text-foreground',
          )}
        >
          {r}
        </Button>
...
export function ResolutionPicker({
  ...
  return (
    <div className={cn('jm-toolbar-surface inline-flex items-center gap-1 p-1', className)} role="tablist" aria-label={t('monitor.resolution')}>
