 ...
 * 加载新 jar 的 {@code top.wcpe.mc.jm.updater.core.Core}，校验 {@code run(Map)} 入口存在、并调用其
...
final class UrlClassLoaderSelfTest implements CoreSelfTest {
    ...
    public boolean test(Path coreJar) {
        try {
            ...
            // 平台加载器无应用类 → top.wcpe.mc.jm.updater.core.* 必从新 jar 加载（非委派旧版）。
            ClassLoader parent = ClassLoader.getSystemClassLoader().getParent();
            try (URLClassLoader loader = new URLClassLoader(urls, parent)) {
                Class<?> coreClass = Class.forName("top.wcpe.mc.jm.updater.core.Core", true, loader);
                // 入口必须存在（ABI 完整）。
