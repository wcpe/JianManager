...
func (s *ImportServerService) Import(ctx context.Context, req ImportServerRequest) (*model.Instance, error) {
	...
	// 工作目录定型：就地=原目录绝对路径（托管区外例外）；搬迁=先搬再登记托管区相对路径（便携）。
	workDir := strings.TrimSpace(req.Path)
	inPlace := req.Mode == "in_place"
	if req.Mode == "migrate" {
		rel := allocWorkDirRel(req.Name) // var/servers/<slug>-<shortid>
		slug := rel[strings.LastIndex(rel, "/")+1:]
		// 搬迁同步执行（V1）：大目录跨盘拷贝耗时，超时给足；向导侧已提示大目录建议就地。
		...
		mv, err := client.Worker.ImportServerDir(moveCtx, &workerpb.ImportServerDirRequest{
			Path: req.Path, Mode: "migrate", TargetSlug: slug,
		})
		if err != nil {
			return nil, fmt.Errorf("Worker ImportServerDir RPC 失败: %w", err)
		}
		if !mv.Success {
			return nil, fmt.Errorf("%w: %s", ErrImportRejected, mv.Error)
		}
		workDir = rel
	}
	...
	// 建实例走结构化启动同 provision（ADR-008）；角色取 backend（与 provision 一致，
	// 可注册进代理 / 可克隆）；端口沿用探测值、不改原目录任何文件。
	inst, err := s.instance.Create(CreateInstanceRequest{
		...
		ServerPort:    inspect.ServerPort,
		AutoRestart:   true,
		importWorkDir: workDir,
		importInPlace: inPlace,
	})
	if err != nil {
		return nil, err
	}
