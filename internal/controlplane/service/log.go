...
type LogService struct {
	...
	cache *resolveCache
	// cutover FR-480 日志入库切换开关与逐 Worker 水位。恒非 nil；默认关闭（旧行为不变）。
	cutover *LogCutover
	// legacy FR-480 Legacy 只读访问器与独立保留预算。
	legacy *LegacyLogReader
	// dual FR-480 Federated vs Legacy 双路径查询门面。
	dual *LogDualPath
}

// NewLogService 创建日志服务。root 用于解析归档目录 var/log，可为 nil（此时归档落盘跳过，仅做表内保留删除）。
//
// FR-480：cfg.Cutover 接线切换开关与 Legacy 独立保留预算。零值嵌套结构走 Default*：
...
// FR-480：cutover 打开时 instance/worker 来源不再写入 CP logs；platform 来源不受影响。
