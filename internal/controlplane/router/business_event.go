...
func (h *BusinessEventHandler) requireRead(c *gin.Context) (*service.UserAccess, bool) {
	access := getAccess(c)
...
func (h *BusinessEventHandler) visibleInstanceUUIDs(c *gin.Context, access *service.UserAccess) ([]string, bool) {
	...
	uuids, err := h.svc.InstanceUUIDsByIDs(ids)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "INTERNAL_ERROR", "message": "查询可访问实例失败"})
		return nil, false
	}
...
func (h *BusinessEventHandler) ListEvents(c *gin.Context) {
	access, ok := h.requireRead(c)
	if !ok {
		return
	}
	instanceUUIDs, ok := h.visibleInstanceUUIDs(c, access)
	if !ok {
		return
	}
...
func (h *BusinessEventHandler) AggregateEconomy(c *gin.Context) {
	...
	rows, err := h.svc.AggregateEconomyByZoneScoped(player, c.Query("currency"), instanceUUIDs)
	if err != nil {
