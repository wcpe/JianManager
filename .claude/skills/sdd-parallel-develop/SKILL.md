...
10. **按依赖顺序 rebase 到 main**：
    ...
    git checkout feat/node-reg
    git rebase main
    git checkout main
    git merge --ff-only feat/node-reg

    # 有依赖的后合
    git checkout feat/instance
    git rebase main
    git checkout main
    git merge --ff-only feat/instance
    ```

    rebase 保证 main 上的提交历史是线性的，没有 merge commit。

11. **Rebase 冲突处理**：
    - rebase 过程中遇到冲突，尝试自动解决
    - 如果自动解决失败，暂停并通知用户：
      ```
      ⚠️ rebase feat/xxx 到 main 时发生冲突，无法自动解决。
      冲突文件: <列出>
      请手动解决后运行: git rebase --continue
      解决后告诉我继续。
      ```
    - 如果自动解决成功：
      ```bash
      ...
      git rebase --continue
      ```
    - rebase 完成后回到 main 执行 `git merge --ff-only`
